...
async function handleNodeRegistration(req, res) {
  const { chainID, ip: providedIp, port, type = "client" /* 'client' or 'bootnode' */, peerID } = req.body;
  ...
  if (type === "bootnode" && !bootnodeList.find(bn => bn.chainID === chainID)) {
    // Ensure peerID is stored for bootnodes if provided
    bootnodeList.push({ chainID, ip: registrationIp, port: portNum, peerID }); 
  ...
  const responsePayload = {
      ...
      details: { chainID, type, peerID, port: portNum }
  };
...
// Registers a node using its chainID, listening port, peerID and type.
...
app.get('/nodes', (req, res) => {
    res.status(200).json(chainRegistry); // chainRegistry already contains all fields including peerID and type
});
  ...
  if (isHtmlRequest) {
    ...
    const html = `
      ...
      <head>
        ...
        <title>KNIRVCHAIN Bootnode Registry Status</title>
        <style>
          body {
...
const turnServer = new Turn({
  ...
  //   'knirv': Turn.generateLongTermCredentials("knirvchain_secure_password", "knirv.com") // Long-term
  // },
  // realm: 'knirv.com',
