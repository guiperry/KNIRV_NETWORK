...
class KNIRVGatewayLinks {
  ...
  async loadConfiguration() {
    try {
      ...
      // Transform portal config to oracle config format
      this.config = {
        deployment_mode: this.detectDeploymentMode(),
        oracle_services: this.transformGatewayServices(portalConfig.oracle_services),
  ...
  /**
   * Transform oracle services configuration
   */
  transformGatewayServices(oracleServices) {
    if (!oracleServices) return {};
    
    const mode = this.detectDeploymentMode();
    const services = oracleServices[mode] || oracleServices.private_testnet || {};
  ...
  /**
   * Get URL for a oracle service
   */
  async getServiceUrl(serviceName) {
    await this.loadPromise;
    
    const service = this.config.oracle_services[serviceName];
    return service ? service.base_url : null;
  }

  /**
   * Get health check URL for a oracle service
   */
  ...
  /**
   * Get all oracle service URLs
   */
  async getAllServiceUrls() {
    await this.loadPromise;
    return this.config.oracle_services;
  }
  ...
  async updateDOMLinks() {
    ...
    // Update oracle service links
    for (const [serviceName, serviceConfig] of Object.entries(this.config.oracle_services)) {
      const elements = document.querySelectorAll(`[data-oracle-service="${serviceName}"]`);
