...
export default function APIPage() {
  ...
  const features = [
    ...
    {
  icon: <Globe className="h-6 w-6 knirv-text-primary" />,
      title: "Global CDN",
  ...
  return (
    <div className="dve-page">
      ...
      <h2 className="text-3xl font-bold text-white text-center mb-12">API Features</h2>
        ...
        <Card key={index} className="bg-white/5 border-white/10 knirv-card-gradient">
...
const knirv = new KnirvClient({
  apiKey: 'your-api-key',
  baseURL: 'https://api.knirv.network'
});

// Create a new agent
const agent = await knirv.agents.create({
...
const response = await knirv.chat.send({
  agentId: agent.id,
