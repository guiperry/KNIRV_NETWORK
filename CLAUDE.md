...
| `packages/KNIRVHASHER` | Go | Repurposed ASIC mining hardware doing neural-network inference. Pipeline stages: `pipeline/0_DATA_CONNECTOR`, `pipeline/1_DATA_MAPPER`, `pipeline/2_DATA_ENCODER`, `pipeline/3_DATA_SEEDER` — each its own `go.mod` (module names `data-connector`, `data-mapper`, `data-encoder`, `data-seeder`), built/tested from inside its own subdirectory only. `3_DATA_SEEDER` (renamed from `3_DATA_TRAINER`) mines PoW-witnessed assertions, it does not train weights. |
