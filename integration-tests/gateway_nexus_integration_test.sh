...
test_authentication() {
    ...
    # Test admin token
    local response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer TESTNET_ADMIN_TOKEN" \
    ...
    response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer TESTNET_VALIDATOR_TOKEN" \
    ...
    # Test observer token
    response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer TESTNET_OBSERVER_TOKEN" \
    ...
    # Test invalid token
    response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer definitely-not-valid" \
        "$gateway_url/.netlify/functions/gateway-sse/nexus/system/status")
    http_code="${response: -3}"
