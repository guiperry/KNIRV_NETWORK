// WebSocket utility for real-time updates in the Agentic Engine
...
export class WebSocketManager {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      try {
        // Choose appropriate WebSocket endpoint based on environment
        const wsEndpoint = this.isElectron
          ? `${this.baseUrl}/api/v1/desktop/secure-ws`  // Desktop secure endpoint
          : `${this.baseUrl}/api/v1/ws`;                // Standard endpoint for cloud
...
export const subscribeToAgentStatus = (handler: (update: AgentStatusUpdate) => void) => {
  wsManager.subscribe('agent_status', (message) => {
