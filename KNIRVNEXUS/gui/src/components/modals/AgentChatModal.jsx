...
const AgentChatModal = ({ isOpen, onClose, agent }) => {
  ...
  const loadChatHistory = async () => {
    try {
      // In a real implementation, this would fetch chat history from the API
  ...
  const loadAgentLogs = async () => {
    try {
      // In a real implementation, this would fetch agent logs from the API
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      // In a real implementation, this would send the message to the agent API
      ...
      const agentResponse = {
        id: Date.now() + 1,
        type: 'agent',
        content: `I received your message: "${userMessage.content}". This is a simulated response from ${agent.name}.`,
        timestamp: new Date().toISOString()
      };
