...
echo "🚀 Testing MCP Server Integration in Agentic Engine"
echo "=================================================="
...
check_server() {
    echo -e "\n${BLUE}Checking if Agentic Engine server is running...${NC}"
    if curl -s "$BASE_URL/servers" > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Server is running${NC}"
        return 0
    else
        echo -e "${RED}❌ Server is not running${NC}"
        echo "Please start the server with: ./agentic-engine --production"
        exit 1
    fi
}
...
echo ""
echo "🎉 Your Agentic Engine now has access to 689+ MCP servers!"
echo ""
echo "Next Steps:"
