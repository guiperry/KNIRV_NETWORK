...
func NewDeepseekProvider(apiKey, model string, extraHeaders map[string]string) providers.Provider {
	...
	// Priority: 1. Explicitly provided model, 2. Environment variable, 3. Default
	if provider.model == "" {
		// Check environment variable
		envModel := os.Getenv("DEEPSEEK_MODEL_NAME")
		if envModel != "" {
			provider.model = envModel
			log.Printf("Deepseek model set from DEEPSEEK_MODEL_NAME: %s", provider.model)
		} else {
			// Use default
...
func (p *DeepseekProvider) Endpoint() string {
	...
	if endpoint := os.Getenv("DEEPSEEK_BASE_URL"); endpoint != "" {
		log.Printf("Using Deepseek endpoint from environment: %s", endpoint)
		return endpoint
	}
