...
// NewActiveMemoryService creates a new ActiveMemoryService
func NewActiveMemoryService(v *vault.VaultService, g *graph.ReasoningEngine, s *mdstorage.MarkdownStorageDriver) *ActiveMemoryService {
	return &ActiveMemoryService{
		vault:    v,
		graph:    g,
		storage:  s,
		executor: vault.NewSolutionExecutor(),
