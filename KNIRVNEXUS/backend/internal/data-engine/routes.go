...
// RegisterRoutes registers data-engine routes with the provided router
func (de *DataEngine) RegisterRoutes(router *mux.Router) {
	// Create handlers
	handlers := NewDataEngineHandlers(de)

	// Create data-engine subrouter
	dataEngineRouter := router.PathPrefix("/api/data-engine").Subrouter()
...
// RegisterBuntDBRoutes registers BuntDB data-engine routes with the provided router
