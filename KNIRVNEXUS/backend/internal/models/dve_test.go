	...
	testCases := []TestCase{
		{
			...
			Expected:    map[string]interface{}{"output": "expected"},
			Weight:      1.0,
		},
	}
...
func TestTestCase_StructFields(t *testing.T) {
	...
	if testCase.Input["prompt"] != "test prompt" {
		t.Errorf("Expected input prompt 'test prompt', got '%v'", testCase.Input["prompt"])
	}
	if testCase.Expected["response"] != "expected response" {
		t.Errorf("Expected response 'expected response', got '%v'", testCase.Expected["response"])
	}
}
	...
	testResults := []TestResult{
		{
			TestCaseID:    "test-1",
			Status:        "passed",
			ActualOutput:  map[string]interface{}{"output": "actual"},
			Score:         0.9,
			ErrorMessage:  "",
...
func TestTestResult_StructFields(t *testing.T) {
	...
	if testResult.ActualOutput["prediction"] != "positive" {
		t.Errorf("Expected prediction 'positive', got '%v'", testResult.ActualOutput["prediction"])
	}
}
