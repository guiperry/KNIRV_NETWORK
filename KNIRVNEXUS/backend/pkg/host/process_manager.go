...
func (pm *ProcessManager) identifyKNIRVProcess(process *Process) {
	...
	for _, keyword := range knirvKeywords {
		if strings.Contains(commandLower, keyword) || strings.Contains(nameLower, keyword) {
			...
			} else if strings.Contains(commandLower, "agent-server") {
				process.ServiceType = "agent-server"
			} else if strings.Contains(commandLower, "data-engine") {
				process.ServiceType = "data-engine"
...
func (pm *ProcessManager) setDefaultKNIRVLimits() {
	...
	pm.resourceLimits.ProcessLimits["agent-server"] = &ProcessLimit{
		MaxCPUPercent:    30.0,
		MaxMemoryPercent: 15.0,
		MaxInstances:     1,
	}
	
	pm.resourceLimits.ProcessLimits["data-engine"] = &ProcessLimit{
		MaxCPUPercent:    40.0,
