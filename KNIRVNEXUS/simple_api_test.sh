...
# Simple API Test Script for Phase 1.3 Standardization
# Tests core endpoints with standardized response validation
...
validate_response() {
    ...
    if ! echo "$response" | jq . >/dev/null 2>&1; then
        print_result "$test_name - JSON Format" "FAIL" "Response is not valid JSON: $response"
        return 1
    fi
...
echo -e "${BLUE}=== Simple API Test Suite ===${NC}"
echo -e "${BLUE}Testing Phase 1.3 Standardized API Endpoints${NC}\n"

# Start the server in background on port 8082
echo -e "${YELLOW}Starting server on port 8082...${NC}"
API_PORT=8082 ./agentic-engine-test &
SERVER_PID=$!
sleep 8
