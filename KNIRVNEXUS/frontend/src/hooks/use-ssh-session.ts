...
export const useSSHSession = () => {
  ...
  // Create SSH session for a rental
  const createSession = useCallback(async (rentalId: string): Promise<SSHSession | null> => {
    setIsLoading(true);
    setError(null);
  ...
  const getSession = useCallback(async (rentalId: string): Promise<SSHSession | null> => {
    setIsLoading(true);
    setError(null);
