...
async function globalSetup(config: FullConfig) {
  console.log('🚀 Global setup: Preparing KNIRV-NEXUS E2E tests...');
  ...
  try {
    ...
    console.log('⏳ Waiting for KNIRV-NEXUS backend API to be ready...');
    ...
    console.log('⏳ Waiting for KNIRV-NEXUS frontend to be ready...');
    ...
    while (!frontendReady && attempts < maxAttempts) {
      try {
        ...
        const titleElement = await page.locator('text=KNIRV NEXUS').first();
