...
function startBackendServer() {
  return new Promise((resolve, reject) => {
    ...
    if (app.isPackaged) {
      ...
      backendPath = path.join(process.resourcesPath, 'backend', 'agentic-engine');
      if (process.platform === 'win32') {
        backendPath += '.exe';
      }
    } else {
      // In development, use the backend from the project root
      backendPath = path.join(__dirname, '..', 'agentic-engine');
      if (process.platform === 'win32') {
        backendPath += '.exe';
      }
    }
...
function createTray() {
  ...
  // Set tooltip
  tray.setToolTip('Agentic Engine - NFT-Agent Platform');

  // Create context menu
  const contextMenu = Menu.buildFromTemplate([
    {
      label: 'Show Agentic Engine',
      click: () => {
        showMainWindow();
      }
    },
...
function createMenu() {
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'About Agentic Engine',
          click: () => {
            dialog.showMessageBox(mainWindow, {
              type: 'info',
              title: 'About Agentic Engine',
              message: 'Agentic Engine',
        ...
        },
        {
          label: 'Documentation',
          click: () => {
            shell.openExternal('https://github.com/your-repo/agentic-engine');
          }
        }
      ]
    }
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.log('Agentic Engine desktop app started successfully');
  } catch (error) {
...
app.setAsDefaultProtocolClient('agentic-engine');
