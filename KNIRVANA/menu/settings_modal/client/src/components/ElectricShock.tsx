...
const ElectricShock = ({ radius }: ElectricShockProps) => {
  ...
  const { playElectricShock, isInitialized } = useAudio();
  
  const lastShockTime = useRef<number>(0);
  ...
  useFrame((state) => {
    ...
    if (shockRef.current && materialRef.current) {
      ...
      // Play electric shock sound when shock becomes visible and enough time has passed
      if (shouldShow && !wasVisible && isInitialized && time - lastShockTime.current > 1.5) {
        playElectricShock();
        lastShockTime.current = time;
      }
