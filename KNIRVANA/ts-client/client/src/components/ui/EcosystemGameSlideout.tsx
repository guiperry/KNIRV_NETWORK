...
import { Bot, Wallet, Trophy, Star, ChevronLeft, ChevronRight, Hammer, Zap, Shield, Brain, Map as MapIcon, Award, Plus, Minus, X } from "lucide-react";
...
interface EcosystemGameSlideoutProps {
  isOpen: boolean;
  onClose: () => void;
}

// ----------------------- Main Slideout Component -----------------------
export default function EcosystemGameSlideout({ isOpen, onClose }: EcosystemGameSlideoutProps) {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full max-w-6xl bg-slate-950 text-slate-100 z-50 overflow-hidden"
            ...
            <div className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-slate-950/70 border-b border-slate-800">
              <div className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-xl md:text-2xl font-semibold tracking-tight">Ecosystem Menu</h1>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-4">
                      <span className="text-slate-300">
                        Clusters: <span className="text-emerald-400">{clusters.filter(c => c.state === "resolved").length}</span>/{clusters.length}
                      </span>
                      <span className="text-slate-300">
                        Mode: <span className="text-purple-400">{effectiveMode === "minimal" ? "Step 1" : "Step 2"}</span>
                      </span>
                    </div>
        ...
        {SKILL_CATALOG.map((s) => (
          <div key={s.id} className="flex items-center justify-between rounded-xl border border-slate-800 p-3 bg-slate-900/60">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => onBuy(s.id)}
                disabled={knerv < Math.round(s.baseCost * Math.pow(1.6, botSkills[s.id] || 0))}
                className="text-xs px-2 py-1 rounded-lg bg-emerald-600/80 hover:bg-emerald-600 transition border border-emerald-500/40 disabled:opacity-50 disabled:cursor-not-allowed"
                title={`Cost: ${Math.round(s.baseCost * Math.pow(1.6, botSkills[s.id] || 0))} Knerv`}
              >
                {Math.round(s.baseCost * Math.pow(1.6, botSkills[s.id] || 0))}
              </button>
            </div>
          </div>
        ))}
    ...
    <Panel title="Ecosystem Map" icon={MapIcon}>
      <div className="mb-4 p-3 rounded-lg bg-slate-800/50 border border-slate-700">
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-gradient-to-br from-rose-500 to-red-600"></div>
