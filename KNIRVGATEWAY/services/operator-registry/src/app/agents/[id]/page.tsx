...
export async function generateMetadata({ params }: AgentProfilePageProps) {
  ...
  const agent = await getOperatorById(decodedId);
  if (!agent) {
    return {
  ...
  return {
    title: `${agent.name} - Operator Profile | Operator Registry`,
    description: agent.description || `Profile page for operator ${agent.name}.`,
  };
}
