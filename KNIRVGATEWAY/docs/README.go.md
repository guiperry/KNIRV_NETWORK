# KNIRVGATEWAY - Go Implementation

This is the Go implementation of the KNIRVGATEWAY server, replacing the Node.js server.js implementation.
...
- **Service Management**: Manages Node.js services (payment-gateway, tunnel-registry, webgui, operator-registry)
...
- `PAYMENT_GATEWAY_PORT` - Payment gateway port (default: `3001`)
...
# Start on testnet
KNIRV_CHAIN_ID=testnet ./bin/knirvgateway
```
...
- `GET /api` - Mock central API gateway
...
- ✅ All Node.js services (payment-gateway, tunnel-registry, webgui, operator-registry)
...
- ⚠️ Serverless mode proxy to persistent gateway
