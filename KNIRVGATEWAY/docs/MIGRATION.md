...
This guide explains how to migrate from the Node.js server to the Go implementation of KNIRVGATEWAY.
...
go build -o bin/knirvgateway ./cmd/gateway
```
...
- Automatically start all Node.js services (payment-gateway, tunnel-registry, webgui)
...
2. **Serverless Mode**: Proxy to persistent gateway not yet implemented
...
curl -X POST http://localhost:8080/services/payment-gateway/start
```
...
**Node.js:**
```bash
docker build -t knirvgateway:latest .
```
...
ls -la services/payment-gateway/server.js
ls -la services/tunnel-registry/server.js
ls -la services/webgui/server.js

# Install service dependencies
cd services/payment-gateway && npm install
...
The Go implementation provides immediate benefits in performance and deployment while maintaining full API compatibility. The migration is straightforward and can be done with minimal downtime. DHT functionality will be added in future updates, but all core gateway functionality is available now.
