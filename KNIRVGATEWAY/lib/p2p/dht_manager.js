/**
 * KNIRVGATEWAY DHT Manager
 *
 * Implements a simplified DHT functionality for KNIRVGATEWAY to listen for network changes
...
const DHT_CONFIG = {
  DISCOVERY_NAMESPACE: 'knirvgateway',
...
class DHTManager extends EventEmitter {
  constructor(chainID = 'testnet', serviceID = 'knirvgateway') {
    super();
  ...
  async initialize(bootstrapPeers = []) {
    try {
      ...
      console.log(`KNIRVGATEWAY DHT Manager initialized with PeerID: ${this.peerId}`);
      console.log(`Bootstrap peers: ${this.bootstrapPeers.length}`);
      return true;
    } catch (error) {
      console.error('Failed to initialize KNIRVGATEWAY DHT Manager:', error);
      return false;
    }
  }
  ...
  async start() {
    ...
    try {
      this.isStarted = true;
      console.log('KNIRVGATEWAY DHT Manager started');
      console.log(`PeerID: ${this.peerId}`);

      // Connect to bootstrap peers
      await this.connectToBootstrapPeers();

      // Announce our presence as KNIRVGATEWAY
      await this.announceService();

      console.log('KNIRVGATEWAY DHT Manager fully operational');
      return true;
    } catch (error) {
      console.error('Failed to start KNIRVGATEWAY DHT Manager:', error);
      this.isStarted = false;
      return false;
    }
  }
  ...
  async announceService() {
    try {
      ...
      console.log(`Announced KNIRVGATEWAY service: ${this.serviceID}`);
    } catch (error) {
  ...
  async stop() {
    if (this.isStarted) {
      try {
        ...
        this.isStarted = false;
        console.log('KNIRVGATEWAY DHT Manager stopped');
      } catch (error) {
