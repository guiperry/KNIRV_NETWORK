...
**Source**: KNIRVROUTER/docs/Decentralized_Network.md

# KNIRVCHAIN Decentralized Discovery Implementation Plan (DHT)

## 1. Goal

To replace the current centralized Node.js registry and STUN service with a decentralized peer discovery mechanism using a Distributed Hash Table (DHT), enhancing network resilience and removing reliance on a central authority (`knirv.com`) for lookups.
...
*   **Data Storage:** While primarily designed for peer discovery, consider how the DHT could also store additional metadata about chains/nodes (e.g., version info, health status) if needed.
...
*   **`/<OptionalSubPath>`**: Specifies sub-resources or actions related to the primary resource type. Examples: `/block`, `/transaction`, `/account`, `/status`, `/peers`.  Can be omitted if not needed (defaults to `/`).
...
*   Get chain peers: `knirv://<chainID>.chain/peers`
*   Get specific NRN content: `knirv://<contentID>.nrn/content?version=1.2`
*   Get peers for a specific NRN content: `knirv://<contentID>.nrn/peers`
*   Get peers for a specific chain network: `knirv://<chainID>.chain/peers`
...
2.  Upon DHT initialization, trigger the `dht.Bootstrap` process to connect to the specified bootstrap peers and join the network. Implement retry logic for robustness.
...
1.  **Self-Address Discovery:** Nodes behind NAT need to determine their public IP.
    ...
    *   **Option B (libp2p AutoNAT):** Leverage libp2p's AutoNAT service, which uses other peers to determine reachability.
...
1.  Once a target peer's multiaddr is obtained from the DHT lookup, use the libp2p Host's `host.Connect(ctx, peerInfo)` method (where `peerInfo` contains the PeerID and multiaddrs) to establish a direct P2P connection.
