...
**Source**: KNIRVROUTER/docs/SYNC_Protocol.md

## Knirvchain P2P Chain Synchronization Protocol v1.0.0

**1. Overview**

This document specifies the protocol used by Knirvchain nodes to synchronize their blockchain state directly with peers over the libp2p network. This mechanism allows nodes to discover longer valid chains from their peers and update their local state accordingly.
...
**3.5. ErrorResponse (Optional but Recommended)**
...
1.  **Peer Discovery:** The Initiator node uses the `DiscoveryManager` (`FindResource`) to identify other Knirvchain peers for its `ChainID`.

2.  **Stream Initiation:** The Initiator selects a peer and attempts to open a new libp2p stream using the `ChainSyncProtocolID`.
...
*   Handle validation failures (log error, potentially penalize peer score if implemented, close stream).
...
*   The `requestChainFromPeers` function will act as the Initiator. It needs to:
    *   Use `DiscoveryManager` to find peers.
...
Your verifier peers simply need to run the same Knirvchain node software as your root node.
...
*   **Discovery:** The `DiscoveryManager` (running in the background via `go discoveryMgr.Run(...)`) will connect to bootstrap peers and use the DHT to find other nodes announcing the same `ChainID`.
*   **Synchronization:** The `P2PConsensusManager`'s `runForkResolution` goroutine periodically calls `requestChainFromPeers`. This function will:
    *   Find other Knirvchain peers via the `DiscoveryManager`.
    *   Attempt to open streams to those peers using `/knirv/chain-sync/1.0.0`.
    *   If a peer responds (because it also has the `handleSyncStream` handler registered), the protocol described above is executed.
*   The verifier node acts as both an Initiator (when `requestChainFromPeers` runs) and a Responder (when `handleSyncStream` is invoked by another peer).
