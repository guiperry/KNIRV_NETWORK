...
**Source**: KNIRVROOT/documentation/docsify/core-concepts/uri-scheme.md
...
This document outlines the design, structure, resolution process, and implementation considerations for the `agent://` Uniform Resource Identifier (URI) scheme used within the KNIRVROOT network. This scheme provides a standardized way to locate and interact with various resources, such as specific blockchain instances or tokenized content, in a decentralized manner.

## 2. URI Structure

The standard format for a KNIRVROOT URI is defined as follows, adhering to common URI patterns:
...
*   **Scheme:** `agent`
    *   **Description:** The mandatory protocol identifier, indicating that the URI pertains to the KNIRVROOT network and its resolution mechanisms.

*   **Authority:** `<ID>.<ResourceType>`
    *   **Description:** This part functions similarly to the host/authority in standard URLs. It uniquely identifies the primary resource within the KNIRVROOT ecosystem and specifies its general category. This component is critical for discovery via the Distributed Hash Table (DHT).
...
*   `.chain`
    *   **Description:** Represents a specific KNIRVROOT blockchain instance.
...
*   `.nrn`
    *   **Description:** Represents tokenized content or data assets managed within the KNIRVROOT ecosystem (assuming NRN is the token/content system).
...
*   `address=<agent_address>`: Specifies a KNIRVROOT wallet or contract address (Base58Check encoded).
...
5.  **Make Request (Over P2P Stream):**
    *   Once a connection is established, the client opens a new stream to the provider using a KNIRVROOT-specific protocol ID (e.g., `/agent/chain/1.0`, `/agent/nrn/1.0`).
...
Creating new, globally unique identifiers and associating them with resources involves the root KNIRVROOT network and the DHT.
