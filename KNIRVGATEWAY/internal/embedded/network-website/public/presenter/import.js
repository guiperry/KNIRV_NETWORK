class PresentationImporter {
    ...
    async submitImport() {
        ...
        try {
            // Check if we're running with the standalone presenter server or through KNIRVGATEWAY
            ...
            if (isStandalonePresenter) {
                ...
                // KNIRVGATEWAY or deployed version - try Netlify functions first
                this.addLog('☁️ KNIRVGATEWAY/Netlify environment detected');
    ...
    async checkIfStandalonePresenter() {
        try {
            ...
            // If this fails, we're probably in KNIRVGATEWAY mode
            return false;
        }
    }
    ...
    getBasePath() {
        // Detect if we're running in KNIRVGATEWAY context
        const currentPath = window.location.pathname;
        if (currentPath.includes('/presenter')) {
            // We're in KNIRVGATEWAY, maintain the presenter path
            return '/presenter/';
        } else {
