...
const DeploymentPanel = ({ repoUrl, agentConfig, onDeployComplete, compiledPluginUrl, compilationJobId }: DeploymentPanelProps) => {
  ...
  const deploymentPlatforms = [
    ...
    { id: "knirvengine", name: "Desktop: KNIRVENGINE", icon: "🖥️", description: "Deploy to Go desktop runtime", type: "knirv" },
    ...
    try {
      ...
      const response = await fetch('/api/deploy', {
        ...
        body: JSON.stringify({
          ...
          deploymentType: isBlockchainDeployment ? 'blockchain-aws' : (deploymentPlatforms.find(p => p.id === selectedPlatform)?.type || 'cloud'),
      ...
      // Simulate deployment process with different timings for different platform types
      const selectedPlatformData = deploymentPlatforms.find(p => p.id === selectedPlatform);
      const platformType = selectedPlatformData?.type || 'cloud';
      ...
      } else if (platformType === 'knirv') {
        steps = [
          { status: "building" as const, progress: 15, message: "Preparing KNIRV runtime..." },
          { status: "building" as const, progress: 30, message: "Compiling for target platform..." },
          { status: "deploying" as const, progress: 60, message: `Deploying to ${selectedPlatformData?.name}...` },
          { status: "deploying" as const, progress: 85, message: "Configuring KNIRV agent..." },
          { status: "success" as const, progress: 100, message: `Successfully deployed to ${selectedPlatformData?.name}!` }
        ];
      } else {
        steps = [
              ...
              {deploymentPlatforms.map((platform) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className={`font-medium ${platform.type === 'knirv' ? 'knirv-text-primary' : 'text-white'}`}>
                        {platform.name}
                        {platform.type === 'knirv' && <span className="ml-2 text-xs bg-blue-500/20 px-2 py-1 rounded">KNIRV</span>}
                      </div>
