 ...
 * 
 * Provides DHT status information for the serverless oracle instance.
 * In serverless mode, this proxies to the persistent oracle.
 */
...
export default async function handler(req, res) {
  ...
  try {
    // In serverless mode, we proxy to the persistent oracle
    ...
    if (!RENDER_GATEWAY_URL) {
      return res.status(200).json({
        ...
        timestamp: Date.now(),
        persistent_oracle: 'not_configured'
      });
    }

    // Try to get status from persistent oracle
    try {
      ...
      return res.status(503).json({
        status: 'unavailable',
        mode: 'vercel-serverless',
        error: 'Persistent oracle unavailable',
