# KNIRVCHAIN Solution Design Document
...
* **KNIRVCHAIN Core:** A Proof-of-Authority (PoA) or private consensus ledger that stores blocks containing GLB files.
...
* **Why GLB?** While traditionally for 3D data, GLB allows for a self-contained binary package. In KNIRVCHAIN, the GLB can encapsulate spatial context, metadata, and even 3D representations of conceptual "memory spaces."
...
When a memory is classified as one of the three types above, the **KNIRVCHAIN Bridge** initiates a transaction:

> `POST /api/v1/transaction`
> **Payload:** `{ source: "KNIRVCHAIN", type: "IDEA", data: [GLB_REF], timestamp: 1734612252 }`
...
* **Immutability:** Once a memory is "hashed" into KNIRVCHAIN, it cannot be altered, ensuring a truthful audit log of the AI's "thought process."
...
* Zero-Knowledge Proofs (ZKP) for sharing memories between different users' KNIRVCHAINs without revealing raw data.
