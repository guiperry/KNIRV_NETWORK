...
*   **Plugin System (Preferred):** Use Go's plugin system (`plugin` package) to dynamically load and execute the code templates. This allows you to load the templates at runtime without recompiling the Inference Engine.
    ...
    *   **Execution:** The Inference Engine calls on the KNIRVORACLE to process NRN transactions within the capability (e.g., `ValidateAsset`, `ImportAsset`) when parsing functions.
...
7.  **Transaction Processed (or Rejected):** If the asset is valid, the Inference Engine sends the transaction to the KNIRVORACLE for an NRN URI minted. If the asset is invalid, the blockchain core rejects the transaction.

### agent Client (Inference Engine):

An application that enables llm inference within custom capability functionalities.

*   **Discovers capabilities:** Queries the KNIRVORACLE API.
...
*   **Logs capability usage:** Submits an `MCPInvokeCapabilityTransaction` to the KNIRVORACLE, including a `ContextRecord` and the NRN fee.
*   **Capability Hosting Client:** Makes the capability binary/code available for download at the `LocationHints` specified in the `ResourceDescriptor` via the KNIRVORACLE DHT. Receives NRN token fees when clients log usage of their registered capabilities.
...
2.  **Registration (On-Chain):**
    ...
    *   `Owner`: The developer's KNIRVORACLE address.
...
3.  **Discovery (By Client):**
    ...
    The agent Client queries the KNIRVORACLE API (e.g., `GET /mcp/capabilities?resourceType=TOOL&name=AssetValidator`) to find suitable capabilities.
...
6.  **Usage Logging & Fee Payment (By Client):**
    ...
    *   `Initiator`: The client's KNIRVORACLE address.
    ...
    *   `Details`: Other relevant metadata (e.g., capability version used, execution duration).
    ...
    The client signs and submits this transaction to the KNIRVORACLE network. The fee is ultimately credited to the capability's `Owner`.
...
## System Design Document: KNIRVORACLE MCP Capabilities

### 1. Introduction

#### 1.1. Purpose:

This document outlines the design for a capability system within the KNIRVORACLE ecosystem, leveraging the Model Context Protocol (MCP).

Capabilities are plugins registered on the KNIRVORACLE, hosted and executed by client applications (e.g., "Inference Engines").
...
It does not cover the internal implementation of the KNIRVORACLE node or the client application beyond their interaction with capabilities.
...
#### 2.2. KNIRVORACLE Hosting Client (Inference Engine):

*   Interacts with KNIRVORACLE nodes to discover and log usage of capabilities.
...
*   Registers capabilities on KNIRVORACLE via `MCPRegisterCapabilityTransaction`.
...
*   **Capability Registration:** Developers must be able to register Capabilities on KNIRVORACLE. The `ResourceDescriptor` must include metadata, `ContentHash` of the Capability package, and NRN usage fee. Its `Schema` field should contain:
...
*   **Capability Discovery:** Clients must be able to query KNIRVORACLE to find registered Capabilities.
...
```
+-----------------------+ +-------------------------+ +-----------------------+
| agent Client          |----->| KNIRVORACLE Node         |<-----| Capability Developer/       |
...
*   **KNIRVORACLE Node:** As described in `MCP_Implementation.md`. Manages Decentralized Model Context Protocol (MCP) transactions and state.
...
*   **KNIRVORACLE Node API:** As defined in `MCP_Implementation.md` (e.g., `POST /mcp/register_capability`, `POST /mcp/invoke/resource/{resource_id}` (where resource\_id is capability\_id), `GET /mcp/capability/{capability_id}`, `GET /mcp/capabilities`).
...
1.  **Developer:** Creates capability, hosts it, registers it on KNIRVORACLE (submits `MCPRegisterCapabilityTransaction`).
2.  **Client:**

    a. Discovers capability via KNIRVORACLE API.
    ...
    f. Submits `MCPInvokeCapabilityTransaction` (with `ContextRecord` and NRN fee) to KNIRVORACLE API.
...
*   **5.6. Blockchain Security:** Standard blockchain security practices apply to KNIRVORACLE nodes and transactions.
...
*   **6.2. Blockchain-Side Errors:**
    ...
    *   Handled by KNIRVORACLE node validation logic.

### 7. Performance Considerations

*   **7.1. Client-Side Performance:** Capability loading and execution time are client-side concerns.
*   **7.2. Blockchain Performance:** Throughput of registration and invocation transactions depends on KNIRVORACLE's underlying performance.
...
*   **8.1. Unit Tests:**
    ...
    *   For KNIRVORACLE node's MCP transaction handling.
...
*   **8.3. Security Tests:**
    ...
    *   Test KNIRVORACLE API security.

### 9. Deployment

*   **9.1. Deployment Process:**

Capability Developer: Compiles capability, makes it available for download, registers it on KNIRVORACLE.

agent Client: Deployed independently. Users interact with it to use capabilities.

KNIRVORACLE Network: Nodes are deployed and run the blockchain.
