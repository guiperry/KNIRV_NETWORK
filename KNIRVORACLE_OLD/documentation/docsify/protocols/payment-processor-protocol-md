# KNIRVORACLE Payment Processor

The KNIRVORACLE Payment Processor is a component that enables root nodes to accept payments in fiat currencies (USD, EUR, etc.) and cryptocurrencies (ETH, BTC, etc.) and automatically disburse KNIRVORACLE tokens to customers.

## Overview

The payment processor integrates with popular payment gateways like Stripe and Coinbase Commerce to process payments and automatically disburse tokens to customers' KNIRVORACLE wallets. This enables roots to monetize their content and services using KNIRVORACLE tokens without having to handle payment processing themselves.
...
To enable the payment processor, start your KNIRVORACLE node with the `--root` flag:
...
1. Configure your payment gateways (Stripe, Coinbase Commerce) to send webhook events to your KNIRVORACLE node
2. Include the customer's KNIRVORACLE wallet address in the payment metadata
...
This component is part of the KNIRVORACLE project and is subject to the same license terms.