# KNIRVORACLE URI Scheme Implementation Design
...
*   **Scheme:** `agent`
    *   **Description:** The mandatory protocol identifier, indicating that the URI pertains to the KNIRVORACLE network and its resolution mechanisms.

*   **Authority:** `<ID>.<ResourceType>`
    *   **Description:** This part functions similarly to the host/authority in standard URLs. It uniquely identifies the primary resource within the KNIRVORACLE ecosystem and specifies its general category. This component is critical for discovery via the Distributed Hash Table (DHT).
...
*   `.chain`
    *   **Description:** Represents a specific KNIRVORACLE blockchain instance.
...
*   `.nrn`
    *   **Description:** Represents tokenized content or data assets managed within the KNIRVORACLE ecosystem (assuming NRN is the token/content system).
...
*   `address=<agent_address>`: Specifies a KNIRVORACLE wallet or contract address (Base58Check encoded).
...
5.  **Make Request (Over P2P Stream):**
    *   Once a connection is established, the client opens a new stream to the provider using a KNIRVORACLE-specific protocol ID (e.g., `/agent/chain/1.0`, `/agent/nrn/1.0`).
...
Creating new, globally unique identifiers and associating them with resources involves the root KNIRVORACLE network and the DHT.
