...
func initNodeJSServices(cfg *config.Config, discoveryMgr *DiscoveryManager) (*NodeJSManager, error) {
	...
	if cfg.NodeJSServices.TunnelRegistry.Enabled && cfg.NodeJSServices.TunnelRegistry.ScriptPath == "" {
		cfg.NodeJSServices.TunnelRegistry.ScriptPath = "embedded/nodejs/tunnel/agent-tunnel-registry/server.js"
	...
	if cfg.NodeJSServices.PaymentGateway.Enabled && cfg.NodeJSServices.PaymentGateway.ScriptPath == "" {
		cfg.NodeJSServices.PaymentGateway.ScriptPath = "embedded/nodejs/payment/agent-payment-gateway/server.js"
	...
	if cfg.NodeJSServices.BootnodeRegistry.Enabled && cfg.NodeJSServices.BootnodeRegistry.ScriptPath == "" {
		cfg.NodeJSServices.BootnodeRegistry.ScriptPath = "embedded/nodejs/operator/operator-registry/server.js"
	...
	if cfg.NodeJSServices.NotarySystem.Enabled && cfg.NodeJSServices.NotarySystem.ScriptPath == "" {
		cfg.NodeJSServices.NotarySystem.ScriptPath = "embedded/nodejs/operator/operator-registry/notary-server.js"
	...
	if cfg.NodeJSServices.NetworkMonitor.Enabled && cfg.NodeJSServices.NetworkMonitor.ScriptPath == "" {
		cfg.NodeJSServices.NetworkMonitor.ScriptPath = "embedded/nodejs/webgui/webGUI/server.js"
...
func initEconomicsIntegration(cfg *config.Config) (*EconomicsIntegration, error) {
	...
	// Enable local mode for root nodes by default and set embedded binary path
	if cfg.IsRoot {
		os.Setenv("ECONOMICS_LOCAL_MODE", "true")
		os.Setenv("ECONOMICS_BINARY_PATH", "embedded/binaries/economics/economics-service")
		log.Printf("[%s] Economics service running in local mode (embedded binary)", cfg.ChainID)
	} else {
