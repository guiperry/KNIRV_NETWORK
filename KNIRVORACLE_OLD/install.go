...
// URIResponse represents the response from the KNIRVORACLE Root's /uriGenerator endpoint
...
func Install(configPath string, IsBootnode bool, role config.Role, nonInteractive bool, walletPath string) (*config.Config, error) {
	...
	if role == config.Root {
		fmt.Println("=== KNIRVORACLE Root Node Setup ===")
		...
		if role != config.Root {
			fmt.Println("1. Connect to the KNIRVORACLE Bootnode")
			fmt.Println("2. Generate a unique chain URI for this node")
		}

		if role == config.RoleBootnode {
			fmt.Println("3. Register with KNIRVORACLE Bootnode Registry")
		}
	...
	if role != config.Root && role != config.RoleClient { // Skip URI generation for Root AND Client roles
		// Step 1: Connect to KNIRVORACLE Root
	...
	case config.Root:
		...
		if chainID == "" {           // Fallback if not set in currentCfg
			chainID = fmt.Sprintf("KNIRVORACLE-ROOT:%d", currentCfg.Port) // Default for root
		}
	...
	// Step 4: Register with KNIRVORACLE Node Registry (only for Bootnode role)
	if role == config.RoleBootnode {
		...
		fmt.Println("Registering bootnode with KNIRVORACLE Node Registry...")
		fmt.Printf("Registering node %s at %s:%s (PeerID: %s) with KNIRVORACLE Node Registry\n",
		...
		if err != nil {
			...
			fmt.Println("Successfully registered with KNIRVORACLE Node Registry")
		}
	...
	if role != config.Root {
		...
		_ = fmt.Sprintf("KNIRVORACLE-%s", strings.ToLower(role.String())) // Used in log messages
	...
	case config.Root:
		...
		configToSave.ChainID = chainID // This is KNIRVORACLE-ROOT<port> or similar
	...
	switch role {
	case config.Root:
		fmt.Println("Your KNIRVORACLE ROOT Node is now configured.")
		fmt.Println("This node will be able to create and manage chains.")
	case config.RoleBootnode:
		fmt.Println("Your KNIRVORACLE Bootnode is now configured with a unique chain URI.")
		fmt.Println("This node will help with dev discovery and network coordination.")
	case config.RolePeer:
		fmt.Println("Your KNIRVORACLE Peer Node is now configured with a unique chain URI.")
		fmt.Println("This node will participate in the network and process transactions.")
	case config.RoleClient:
		fmt.Println("Your KNIRVORACLE Client Node is now configured.")
		fmt.Println("This node will connect to the network with reduced resource usage.")
	}

	fmt.Println("Finalizing installation and launching KNIRVORACLE Node...")
...
// promptForRootEndpoint prompts the user for the KNIRVORACLE Root endpoint
...
func promptForRootEndpoint(nonInteractive bool, currentCfg *config.Config) string {
	...
	fmt.Printf("Enter the KNIRVORACLE Root endpoint [default: %s]: ", defaultEndpoint)
	var endpoint string
	fmt.Scanln(&endpoint)
...
// GenerateChainURI connects to the KNIRVORACLE Root and generates a unique chain URI
...
func GenerateChainURI(rootEndpoint string, desiredURI string) (string, string, string, error) {
	...
	req.Header.Set("User-Agent", "KNIRVORACLE-Node-Installer/1.0")

	// Send the request
	resp, err := client.Do(req)
	if err != nil {
		log.Printf("HTTP request failed: %v", err)
		return "", "", "", fmt.Errorf("failed to connect to KNIRVORACLE Root: %w", err)
	}
...
func RegisterURIHandlers(chainURI string) error {
	...
	schemes := []uri.URIScheme{
		{
			Name:        "agent",
			Description: "KNIRVORACLE Decentralized MCP Network",
		},
	}
...
// RegisterWithNodeRegistry registers this node with the KNIRVORACLE Node Registry
...
func configureLinuxService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	serviceUser := "KNIRVORACLE"
	workingDir := "/var/lib/KNIRVORACLE"
	logPath := "/var/log/KNIRVORACLE.log"

	// Determine service name and command line based on service type
	serviceName := fmt.Sprintf("KNIRVORACLE-%s", strings.ToLower(role.String()))
	...
	if err != nil {
		executablePath = "/usr/local/bin/KNIRVORACLE" // Fallback if we can't determine the path
	}

	if role == config.RoleBootnode {
		description = fmt.Sprintf("KNIRVORACLE Bootnode Service for chain %s", cfg.ChainID)
		...
		description = fmt.Sprintf("KNIRVORACLE Peer Node Service for chain %s", cfg.ChainID)
...
`, description, serviceUser, execStart, workingDir, logPath, logPath)
	...
	if os.Geteuid() != 0 {
		...
		if err != nil {
			...
			dataDir = filepath.Join(homeDir, ".config", "KNIRVORACLE")
		}
...
func configureWindowsService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	displayName := "KNIRVORACLE Node"
	description := "KNIRVORACLE Node Service"
	logPath := "C:\\ProgramData\\KNIRVORACLE\\logs\\KNIRVORACLE.log"
	...
	executablePath, err := os.Executable()
	if err != nil {
		executablePath = os.Getenv("ProgramFiles") + "\\KNIRVORACLE\\KNIRVORACLE.exe" // Fallback
	}

	var binPath string

	if role == config.RoleBootnode {
		displayName = "KNIRVORACLE Bootnode"
		description = fmt.Sprintf("KNIRVORACLE Bootnode Service for chain %s", cfg.ChainID)
	...
	if err != nil || !isAdmin {
		...
		if err != nil {
			...
			dataDir = filepath.Join(appDataDir, "AppData", "Local", "KNIRVORACLE")
		}
...
echo Installing KNIRVORACLE %s Service
...
func configureMacOSService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	workingDir := "/Library/KNIRVORACLE"
	logPath := "/Library/Logs/KNIRVORACLE/KNIRVORACLE.log"

	// Determine service name and command line based on service type
	serviceName := fmt.Sprintf("com.KNIRVORACLE.%s", strings.ToLower(role.String()))

	// Get the executable path
	executablePath, err := os.Executable()
	if err != nil {
		executablePath = "/usr/local/bin/KNIRVORACLE" // Fallback
	}
...
<dict>
	...
	if os.Geteuid() != 0 {
		...
		if err != nil {
			...
			dataDir = filepath.Join(homeDir, "Library", "Application Support", "KNIRVORACLE")
		}
