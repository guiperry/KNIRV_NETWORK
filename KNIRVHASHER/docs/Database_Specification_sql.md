# Hasher Database Specification (KNIRVBASE Edition)
...
This document specifies the complete database schema for the Hasher quantum-resistant password vault system using **KNIRVBASE**, a local-first distributed NoSQL database with CRDT-based conflict resolution. The design leverages KNIRVBASE's strengths in offline-first operation, eventual consistency, and P2P synchronization while meeting Hasher's demanding security and performance requirements.
...
**Why KNIRVBASE is Perfect for Hasher:**
...
4. **Operational Simplicity**
   ...
   - Compatible with Hasher monolithic architecture

**Configuration:**
```go
// KNIRVBASE Configuration for Hasher
...
networkID, err := db.CreateNetwork(types.NetworkConfig{
    NetworkID:   "hasher-production",
    Name:        "Hasher Vault Network",
...
### 1.2 KNIRVBASE Architecture for Hasher
...
CREATE TABLE credentials (
    ...
    -- Credential Data (PQC-encrypted)
    hash                BYTEA NOT NULL,              -- PQC-encrypted KDF output (32 bytes hash + PQC overhead)
    salt                BYTEA NOT NULL,              -- 256-bit random salt (32 bytes)
    iterations          INTEGER NOT NULL,            -- KDF iteration count (100M-500M)
    algorithm           VARCHAR(50) NOT NULL DEFAULT 'PBKDF2-SHA256-ASIC',

    -- Post-Quantum Cryptography
    pqc_algorithm       VARCHAR(50) NOT NULL DEFAULT 'Kyber-768',
    pqc_key_id          BIGINT REFERENCES pqc_keys(id),

    -- Metadata
    metadata            JSONB DEFAULT '{}',          -- Flexible metadata (department, role, etc.)
    ...
    -- Status
    status              VARCHAR(20) NOT NULL DEFAULT 'active',  -- active, locked, expired, disabled
    failed_attempts     INTEGER NOT NULL DEFAULT 0,
    locked_until        TIMESTAMP WITH TIME ZONE,

    -- Constraints
    CONSTRAINT chk_credentials_iterations_min CHECK (iterations >= 10000000),
    CONSTRAINT chk_credentials_iterations_max CHECK (iterations <= 500000000),
    CONSTRAINT chk_credentials_salt_size CHECK (octet_length(salt) = 32),
    CONSTRAINT chk_credentials_status CHECK (status IN ('active', 'locked', 'expired', 'disabled'))
);
...
|---------|------|--------|---------|
| 1.0 | 2024-12-24 | Hasher Team | Initial database specification |
