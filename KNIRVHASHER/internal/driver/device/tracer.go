...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc clang -target bpfel hasher ../../ebpf/hasher.bpf.c -- -I/usr/include/bpf -I.
...
// Must match struct hash_event in hasher.bpf.c
...
// Must match struct hash_stats in hasher.bpf.c
...
func (t *Tracer) attachUprobes() error {
	...
	attachments := []struct {
		...
		{"hasher_compute_start", t.objs.TraceComputeStart},
		{"hasher_compute_end", t.objs.TraceComputeEnd},
		{"hasher_batch_start", t.objs.TraceBatchStart},
		{"hasher_batch_end", t.objs.TraceBatchEnd},
	}
...
// hasherObjects contains eBPF programs and maps
// This is a placeholder - actual implementation is generated by bpf2go
type hasherObjects struct {
...
func (o *hasherObjects) Close() error {
	if o.TraceComputeStart != nil {
		o.TraceComputeStart.Close()
	}
	if o.TraceComputeEnd != nil {
		o.TraceComputeEnd.Close()
	}
	if o.TraceBatchStart != nil {
		o.TraceBatchStart.Close()
	}
	if o.TraceBatchEnd != nil {
		o.TraceBatchEnd.Close()
	}
	if o.ComputeStartTimes != nil {
		o.ComputeStartTimes.Close()
	}
	if o.Events != nil {
		o.Events.Close()
	}
...
// loadHasher loads the embedded eBPF bytecode
// This function should be generated by bpf2go, but we provide a fallback
func loadHasher() (*ebpf.CollectionSpec, error) {
	// Try to load from the compiled .o file in the ebpf directory
	paths := []string{
		"internal/ebpf/hasher.bpf.o",
		"../../ebpf/hasher.bpf.o",
		"/usr/share/hasher/hasher.bpf.o",
	}
	...
	return nil, errors.New("eBPF object file not found - run 'make ebpf' to compile")
}
