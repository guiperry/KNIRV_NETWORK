...
func main() {
	log.Printf("Testing gRPC connectivity to hasher-server at %s", serverAddress)
	...
	// Test 1: GetStatus - should work even without hardware
	log.Printf("Test 1: Getting server status...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	status, err := c.GetStatus(ctx, &pb.StatusRequest{})
	if err != nil {
		log.Printf("GetStatus failed: %v", err)
	} else {
		log.Printf("✅ GetStatus SUCCESS: %s", status.String())
	}
