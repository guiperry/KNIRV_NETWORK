...
export const GraphChainProvider: React.FC<GraphChainProviderProps> = ({ children }) => {
  ...
  const fetchCurrentHeight = async () => {
    try {
      setError(null);
      const height = await graphChainApi.getCurrentHeight();
      setCurrentHeight(height);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch GraphChain height');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchCurrentHeight, 10000);
