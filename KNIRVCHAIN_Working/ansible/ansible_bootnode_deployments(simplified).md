...
# Simplified Ansible Playbook Approach for KNIRVCHAIN Bootnode Deployment on AWS

This document outlines a simplified Ansible playbook approach for deploying KNIRVCHAIN bootnodes on AWS, focusing on non-interactive installation and automated deployment triggered by a webhook.
...
*   **Play 2: Software Installation:** Simplified to install prerequisites, copy the KNIRVCHAIN executable, and execute it in a non-interactive bootnode installation mode.

## Assumptions

*   Your `KNIRVCHAIN_executable` has a non-interactive installation mode for setting up a bootnode.  For example: `./KNIRVCHAIN_executable install --role bootnode --non-interactive` (The exact command might differ based on your application's CLI).
*   This installer, when run, correctly:
    ...
    *   Creates, enables, and starts the systemd service (e.g., `KNIRVCHAIN-bootnode.service`) configured to run the application as a bootnode (e.g., with the `--bootnode` flag).
...
```yaml
  ...
  vars:
    ...
    # The installer within KNIRVCHAIN_executable is expected to create user-specific config/data dirs
  ...
  tasks:
    ...
    - name: Run KNIRVCHAIN installer to set up bootnode
      ansible.builtin.command:
        cmd: "{{ remote_KNIRVCHAIN_bin_dir }}/KNIRVCHAIN_executable install --role bootnode --non-interactive"
    ...
    - name: Ensure KNIRVCHAIN bootnode service is enabled and started
...
*   **Removed Vars:** `KNIRVCHAIN_config_local_path`, `KNIRVCHAIN_wallet_local_path`, `KNIRVCHAIN_master_wallet_local_path` are removed as the installer will generate these.
*   **Removed Directory Creation:** Tasks for creating `remote_KNIRVCHAIN_app_config_dir` and `remote_KNIRVCHAIN_data_dir_base` are removed, assuming your installer handles this under the user's home directory (e.g., `/home/ubuntu/.config/KNIRVCHAIN`).
...
*   **Simplified Service Management:** The handlers are removed. A task `Ensure KNIRVCHAIN bootnode service is enabled and started` is added as a safeguard. If your installer reliably enables and starts the service, you might even remove this task. The `service_name` variable should match the name your installer uses for the service.
*   **No `templates/KNIRVCHAIN-bootnode.service.j2` needed:** Since the installer creates the service file, this template is no longer required for the playbook.
*   **Permissions and `become: true`:** When Ansible runs the installer command with `become: true`, the installer will execute as root. Your installer logic needs to:
    *   Create any user-specific directories (like `~/.config/KNIRVCHAIN`) and ensure they are owned by the `remote_user` (e.g., `ubuntu`).
...
This simplified Ansible playbook, combined with a webhook-triggered mechanism, will give you a powerful automated deployment system for your bootnodes. Remember to thoroughly test your `KNIRVCHAIN_executable`'s installer to ensure it behaves as expected in a non-interactive, automated context.
```