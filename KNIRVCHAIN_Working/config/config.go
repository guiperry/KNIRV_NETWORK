...
type PaymentProcessorConfig struct {
	Enabled               bool    `json:"enabled" mapstructure:"enabled"`
	NodeRPC               string  `json:"node_rpc" mapstructure:"nodeRPC"` // URL of the KNIRVCHAIN node
...
type NetworkMonitorConfig struct {
	...
	AutoStart bool `json:"auto_start" mapstructure:"auto_start"` // Auto-start with KNIRVCHAIN
}
...
func DefaultConfig() *Config {
	return &Config{
		NodeName: "KNIRVCHAIN Node", // Default node name
		...
		DataEngine: DataEngineConfig{
			...
			KafkaClientID:    "KNIRVCHAIN-client",
			ChromaDBURL:      "http://localhost:8000",
			ChromaCollection: "KNIRVCHAIN-events",
			EnableKafka:      false,
		...
		NodeJSServices: NodeJSServicesConfig{
			...
			TunnelRegistry: struct {
				...
				Enabled:          true, // Changed from false to true to enable data engine by default for all roles
				ScriptPath:       "embedded/nodejs/tunnel/agent-tunnel-registry/server.js",
				HTTPPort:         3003,
				ControlPort:      4001,
			...
			PaymentGateway: struct {
				...
				Enabled:    false,
				ScriptPath: "embedded/nodejs/payment/agent-payment-gateway/server.js",
				HTTPPort:   3004,
			...
			BootnodeRegistry: struct {
				...
				ScriptPath: "embedded/nodejs/operator/operator-registry/server.js",
				HTTPPort:   3006,
			},
			NotarySystem: struct {
				Enabled    bool   `json:"enabled"`
				...
				Enabled:    true, // Enable by default for root nodes
				ScriptPath: "embedded/nodejs/operator/operator-registry/notary-server.js",
				HTTPPort:   3007,
			},
			NetworkMonitor: struct {
				Enabled    bool   `json:"enabled"`
				...
				Enabled:    true, // Enable by default for root nodes
				ScriptPath: "embedded/nodejs/webgui/webGUI/server.js",
				HTTPPort:   3008,
			},
		},
...
func CreateRootConfigFromMatrixAndConstants() *Config {
	...
	if rootConstants.BlockchainAddress != "" {
		...
		cfg.MinersAddress = ""
		cfg.MasterAddress = ""
		cfg.ChainID = fmt.Sprintf("KNIRVCHAIN-Faucet%d", cfg.Port)
	}
...
// - Root: ~/.config/KNIRVCHAIN/root_data/config.json
// - Bootnode: ~/.config/KNIRVCHAIN/bootnode_data/config.json
// - Peer: ~/.config/KNIRVCHAIN/dev_data/config.json
// - Client: ~/.config/KNIRVCHAIN/client_data/config.json
...
// - Root: ~/.config/KNIRVCHAIN/root_data
// - Bootnode: ~/.config/KNIRVCHAIN/bootnode_data
// - Peer: ~/.config/KNIRVCHAIN/dev_data
// - Client: ~/.config/KNIRVCHAIN/client_data
...
// - Bootnode: ~/.config/KNIRVCHAIN/bootnode_data/wallet.dat
// - Peer: ~/.config/KNIRVCHAIN/dev_data/wallet.dat
// - Client: ~/.config/KNIRVCHAIN/client_data/wallet.dat
...
// - Bootnode: ~/.config/KNIRVCHAIN/bootnode_data/master_wallet.dat
// - Peer: ~/.config/KNIRVCHAIN/dev_data/master_wallet.dat (typically not used)
// - Client: ~/.config/KNIRVCHAIN/client_data/master_wallet.dat (typically not used)
...
const BLOCKCHAIN_ADDRESS_CONSTANT_FROM_MAIN_OR_CONFIG = "KNIRVCHAINb53c1e30b8a578c091dd40612bfd1433991b4e09" // Replace with actual access
...
func DefaultTerminalIntegration() *TerminalIntegration {
	...
	return &TerminalIntegration{
		Enabled:           true,
		ZshCompletionsDir: filepath.Join(homeDir, ".KNIRVCHAIN", "completions"),
		Theme:             "dark",
	}
}
