...
func (ws *WalletServer) handleGetBalance(w http.ResponseWriter, r *http.Request) {
...
func (ws *WalletServer) handleGenerateWalletGet(w http.ResponseWriter) {
...
func (ws *WalletServer) saveWalletToFile(wallet *Wallet, path string) error {
...
func (ws *WalletServer) handlePostTransactionPost(w http.ResponseWriter, r *http.Request) {
	...
	txn := NewTransaction(transactionRequest.FromAddress, transactionRequest.ToAddress, transactionRequest.Value, []byte{})
...
func (ws *WalletServer) handleSendSignedTransactionPost(w http.ResponseWriter, r *http.Request) {
	...
	unsignedTxn := NewTransaction(transactionRequest.FromAddress, transactionRequest.ToAddress, transactionRequest.Value, []byte{}) // Assuming data is empty for now
	...
	log.Printf("Successfully forwarded signed transaction %s to blockchain node", signedTxn.TransactionHash)
...
func (ws *WalletServer) handleRegisterMCPCapability(w http.ResponseWriter, r *http.Request) {
	...
	txn := NewTransaction(wallet.GetAddress(), "", 0, nil) // ToAddress and Value not used for MCP
	txn.Type = "register_capability_txn"
	...
	json.NewEncoder(w).Encode(MCPTransactionResponse{
		TransactionID: signedTxn.TransactionHash,
		Status:        "pending",
		FeePaid:       req.Fee,
	})
}
...
func (ws *WalletServer) handleInvokeMCPCapability(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	txn := NewTransaction(wallet.GetAddress(), "", 0, nil) // ToAddress and Value not used for MCP
	txn.Type = "invoke_capability_txn"
...
func (ws *WalletServer) sendTransactionToBlockchain(txn *Transaction) (*http.Response, error) {
	payload := map[string]interface{}{
		"transaction": txn,
	}
	data, err := json.Marshal(payload)
