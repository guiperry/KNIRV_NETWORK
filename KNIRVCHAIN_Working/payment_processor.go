...
func (pp *PaymentProcessor) disburseTokens(recipientAddress string, amount *big.Int, paymentID string) (string, error) {
	...
	if !isValidKNIRVCHAINAddress(recipientAddress) {
		return "", fmt.Errorf("invalid recipient address: %s", recipientAddress)
	}

	// Convert big.Int to uint64 for KNIRVCHAIN transaction
...
// isValidKNIRVCHAINAddress checks if an address is a valid KNIRVCHAIN address
...
func LoadPaymentProcessorConfig(cfg *config.PaymentProcessorConfig) {
	...
	if nodeRPC := os.Getenv("KNIRVCHAIN_NODE_RPC"); nodeRPC != "" {
		cfg.NodeRPC = nodeRPC
	}
