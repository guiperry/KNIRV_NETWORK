...
func (e *EmbeddedWebGUI) Start(ctx context.Context) error {
	...
	nodeModulesPath := filepath.Join(e.workingDir, "webGUI", "node_modules")
	...
	// Check if Next.js application is already built
	nextBuildPath := filepath.Join(e.workingDir, "webGUI", ".next")
	if _, err := os.Stat(nextBuildPath); os.IsNotExist(err) {
		// Build the Next.js application only if .next directory doesn't exist
		...
		log.Printf("[%s] Next.js application already built, skipping build", e.name)
	}
