...
/**
 * Server wrapper for KNIRV Operator Registry System
...
console.log(`[OperatorRegistry] Starting Operator Registry System on port ${port}...`);
...
nextProcess.on('error', (error) => {
    console.error(`[OperatorRegistry] Failed to start: ${error.message}`);
    process.exit(1);
});

nextProcess.on('close', (code) => {
    console.log(`[OperatorRegistry] Process exited with code ${code}`);
    process.exit(code);
});

// Handle graceful shutdown
process.on('SIGTERM', () => {
    console.log('[OperatorRegistry] Received SIGTERM, shutting down gracefully...');
    nextProcess.kill('SIGTERM');
});

process.on('SIGINT', () => {
    console.log('[OperatorRegistry] Received SIGINT, shutting down gracefully...');
    nextProcess.kill('SIGINT');
});

console.log(`[OperatorRegistry] Operator Registry server wrapper started`);

// Create a TURN/STUN server instance
...
app.get('/status', (req, res) => {
  ...
  const highlight = req.query.highlight ? String(req.query.highlight) : null;
  ...
  if (isHtmlRequest) {
    ...
    const nodeRows = Object.entries(chainRegistry).map(([chainID, info]) => {
      ...
      const isHighlighted = highlight && highlight === chainID;
      const rowClass = isHighlighted ? 'style="background: linear-gradient(90deg, rgba(253,240,210,0.6), rgba(255,255,255,0.6)); border-left: 4px solid #f59e0b;"' : '';
    ...
    const html = `
      ...
      <head>
        ...
        <title>KNIRV Network Operator Registry Status</title>
        <style>
          body {
      ...
      <body>
        <h1>KNIRVCHAIN Bootnode Registry Status</h1>
        ...
        <div style="margin-top: 12px;">
          <a href="/" onclick="window.location.href='http://localhost:3007'; return false;" class="refresh-button">Back to Operator Registry</a>
        </div>
        <script>
