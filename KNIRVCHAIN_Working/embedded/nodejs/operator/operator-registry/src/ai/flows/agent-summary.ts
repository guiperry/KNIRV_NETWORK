...
const agentSummaryPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant specializing in summarizing the functionality of network operators and services.

  Given the following information about an operator, create a concise summary (1-2 sentences) that new users can use to quickly understand its purpose and capabilities:

  Operator Name: {{{agentName}}}
  Operator Capabilities: {{{agentCapabilities}}}
  {{#if agentDescription}}
  Operator Description: {{{agentDescription}}}
  {{/if}}
