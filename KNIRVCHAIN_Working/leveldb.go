...
func NewLevelDB(dbPath string) (*LevelDB, error) {
	...
	return &LevelDB{Client: db, path: dbPath}, nil
}
...
func (db *LevelDB) GetContextRecord(contextID string) (*pb.ContextRecordProto, error) { // contextID is the transaction hash
...
func (db *LevelDB) GetContextRecordsForCapability(capabilityID string) ([]*pb.ContextRecordProto, error) {
	...
	for _, key := range keys {
		ctx, err := db.GetContextRecord(strings.TrimPrefix(key, KeyPrefixContextCapabilityIdx))
		if err != nil {
...
func (db *LevelDB) GetAllContextRecords() ([]*pb.ContextRecordProto, error) {
	...
	var contexts []*pb.ContextRecordProto
	for _, key := range keys {
		ctx, err := db.GetContextRecord(strings.TrimPrefix(key, KeyPrefixContext))
		if err != nil {
			return nil, fmt.Errorf("failed to get context record: %w", err)
		}
		contexts = append(contexts, ctx)
...
func (db *LevelDB) SaveCapability(capability interface{}) error {
	...
	capProto, err := ConvertToCapabilityDescriptorContainerProto(capability)
	if err != nil {
...
func (db *LevelDB) UpdateCapability(capabilityID string, updates interface{}) error {
	...
	updatesProto, err := ConvertToCapabilityDescriptorContainerProto(updates)
	if err != nil {
