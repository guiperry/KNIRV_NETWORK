...
cd integration-tests && go test -v -run TestKNIRVCHAIN
...
cd KNIRVCHAIN
go build -o KNIRVCHAIN_node ./cmd/KNIRVCHAIN/main.go
./KNIRVCHAIN_node --port 8080 --p2p_port 6001
```
...
The KNIRVCHAIN Makefile was updated to manage embedded Node.js and binary services in the `embedded/` directory. New targets were added for building, cleaning, and managing these services.  The update handles both Go binaries and Node.js services, includes automatic dependency installation, and provides error handling.
...
The KNIRVCHAIN WebGUI (Next.js application) was failing to start due to a missing CSS import and an incompatible start command.  The CSS import was fixed, the start command was updated to use `npx serve`, and the Makefile build target was enhanced.
