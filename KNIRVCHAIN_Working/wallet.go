...
func (w *Wallet) SignTransaction(tx *Transaction) error {
	// Create unsigned transaction copy for signing
	...
	txProtoForSigning, err := unsignedTx.ToProtoForHashing()
	if err != nil {
	...
	// Update transaction with signature and public key
	tx.Signature = sig
...
func (w *Wallet) GetSignedTxn(unsignedTxn Transaction) (*Transaction, error) {
	// Convert the transaction to its Protobuf representation for hashing
	txProtoForSigning, err := unsignedTxn.ToProtoForHashing()
	if err != nil {
		return nil, fmt.Errorf("failed to convert transaction to proto for signing: %w", err)
	}

	// Get canonical bytes for hashing
	bs, err := GetCanonicalBytesForHashing(txProtoForSigning)
	if err != nil {
		return nil, fmt.Errorf("failed to get canonical bytes for transaction signing: %w", err)
	}
	...
	signedTxn := NewTransaction(unsignedTxn.From, unsignedTxn.To, unsignedTxn.Value, unsignedTxn.Data)
