...
type SyncConfig struct {
	...
	APIPatterns    []APIPattern    `json:"api_patterns"`
...
// APIPattern defines API reference synchronization
type APIPattern struct {
	Name           string   `json:"name"`
	Pattern        string   `json:"pattern"`
	SourceDirs     []string `json:"source_dirs"`
	TargetDir      string   `json:"target_dir"`
	CentralAPIPath string   `json:"central_api_path"`
	Enabled        bool     `json:"enabled"`
}
...
func (sm *SyncManager) LoadConfig(configPath string) error {
	...
	sm.Logger.Printf("Loaded sync configuration with %d script patterns, %d test patterns, and %d API patterns",
		len(config.ScriptPatterns), len(config.TestPatterns), len(config.APIPatterns))
...
// SyncAPIPatterns synchronizes API reference patterns to point to central API spec
func (sm *SyncManager) SyncAPIPatterns() ([]SyncResult, error) {
	var results []SyncResult

	sm.Logger.Println("Starting API reference synchronization...")

	for _, pattern := range sm.SyncConfig.APIPatterns {
		if !pattern.Enabled {
			sm.Logger.Printf("Skipping disabled API pattern: %s", pattern.Name)
			continue
		}
		...
		sm.Logger.Printf("Synchronizing API pattern: %s", pattern.Name)

		// Process each source directory
		for _, sourceDir := range pattern.SourceDirs {
			sourcePath := filepath.Join(sm.ProductionRoot, sourceDir)

			err := sm.syncAPIReferences(sourcePath, pattern)
			if err != nil {
				result.Errors = append(result.Errors, err.Error())
				result.Success = false
			} else {
	...
	sm.Logger.Printf("API reference synchronization completed. %d patterns processed", len(results))
	return results, nil
}
...
func (sm *SyncManager) syncPatternFiles(sourcePath, targetPath string, pattern ScriptPattern) error {
	...
	return filepath.WalkDir(sourcePath, func(path string, d fs.DirEntry, err error) error {
		...
		if matched, _ := filepath.Match(pattern.Pattern, d.Name()); matched {
			...
			// Skip scripts that are unique to production (don't exist in testnet)
			if _, err := os.Stat(targetFile); os.IsNotExist(err) {
				sm.Logger.Printf("Skipping script unique to production: %s", path)
				return nil
			}
...
// syncAPIReferences synchronizes API references to point to the central API spec
func (sm *SyncManager) syncAPIReferences(sourcePath string, pattern APIPattern) error {
	if _, err := os.Stat(sourcePath); os.IsNotExist(err) {
		return fmt.Errorf("source path does not exist: %s", sourcePath)
	}

	// Ensure target directory exists
	targetPath := filepath.Join(sm.ProductionRoot, pattern.TargetDir)
	if err := os.MkdirAll(targetPath, 0755); err != nil {
		return fmt.Errorf("failed to create target directory: %w", err)
	}
	...
	return filepath.WalkDir(sourcePath, func(path string, d fs.DirEntry, err error) error {
		...
		if matched, _ := filepath.Match(pattern.Pattern, d.Name()); matched {
			...
			for _, exclude := range sm.SyncConfig.ExcludeFiles {
				if strings.Contains(path, exclude) {
					sm.Logger.Printf("Skipping excluded API file: %s", path)
					return nil
				}
			}

			// For API files, copy the central API spec to the target location
			centralAPIPath := filepath.Join(sm.ProductionRoot, pattern.CentralAPIPath)
			return sm.copyCentralAPISpec(centralAPIPath, targetFile)
		}
...
// copyCentralAPISpec copies the central API specification to the target location
func (sm *SyncManager) copyCentralAPISpec(centralAPIPath, targetPath string) error {
	// Ensure target directory exists
	targetDir := filepath.Dir(targetPath)
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		return fmt.Errorf("failed to create target directory: %w", err)
	...
	// Read central API spec
	content, err := os.ReadFile(centralAPIPath)
	if err != nil {
		return fmt.Errorf("failed to read central API spec: %w", err)
	}

	// Write to target file
	if err := os.WriteFile(targetPath, content, 0644); err != nil {
		return fmt.Errorf("failed to write API spec to target: %w", err)
	}

	sm.Logger.Printf("API spec synchronized: %s -> %s", centralAPIPath, targetPath)
	return nil
}
