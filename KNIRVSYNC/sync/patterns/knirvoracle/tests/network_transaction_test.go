...
// verifyMiningStatus checks if mining is active by verifying block testnet
func verifyMiningStatus(t *testing.T, nodeURL string, expectedActive bool) error {
	...
	t.Log("Mining status API reports active, now verifying block testnet...")
	...
	// Wait for new block testnet with longer timeout
	...
	for {
		select {
		case <-timeout:
			return fmt.Errorf("timeout waiting for block testnet")
		case <-tick:
			...
			if len(currentBlocks) > initialBlocks {
				t.Logf("Block testnet verified: initial=%d, current=%d", initialBlocks, len(currentBlocks))
