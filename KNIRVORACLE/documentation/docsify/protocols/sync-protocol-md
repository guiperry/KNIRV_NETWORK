## KNIRVORACLE P2P Chain Synchronization Protocol v1.0.0

**1. Overview**

This document specifies the protocol used by KNIRVORACLE nodes to synchronize their blockchain state directly with devs over the libp2p network. This mechanism allows nodes to discover longer valid chains from their devs and update their local state accordingly.
...
1.  **Peer Discovery:** The Initiator node uses the `DiscoveryManager` (`FindResource`) to identify other KNIRVORACLE devs for its `ChainID`.
...
**NEVER TRUST DATA FROM PEERS.** All received blocks and transactions MUST be independently validated according to the KNIRVORACLE consensus rules before being integrated into the local chain.
...
Your verifier devs simply need to run the same KNIRVORACLE node software as your root node.
...
*   **Synchronization:** The `P2PConsensusManager`'s `runForkResolution` goroutine periodically calls `requestChainFromPeers`. This function will:
    *   Find other KNIRVORACLE devs via the `DiscoveryManager`.
