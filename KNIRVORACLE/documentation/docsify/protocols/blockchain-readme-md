...
*   **MCP:** A set of standards for defining, discovering, and interacting with AI capabilities. KNIRVORACLE serves as a decentralized backend for MCP.
*   **Capabilities:** These are the core building blocks of the MCP ecosystem on KNIRVORACLE:
...
The `ContextRecord` is central to KNIRVORACLE's MCP, providing an on-chain, verifiable log for:
...
3.  User config directory (`~/.config/KNIRVORACLE/config.json` on Linux).
...
Resolution involves parsing the URI, deriving a resource key (e.g., `myPeer.chain`), generating a CID, querying the private KNIRVORACLE DHT for providers using the CID, connecting to a provider via libp2p, and making the request over a P2P stream.
