...
func monitorFlowProgress(service *XIONIntegrationService, flowID string, timeout time.Duration) {
	...
	for range ticker.C {
		...
		// Print latest step
		if len(flow.Steps) > 0 {
			latestStep := flow.Steps[len(flow.Steps)-1]
			log.Printf("     Latest step: %s (%s)", latestStep.StepName, latestStep.Status)
		}
...
func printFlowSummary(flow *PaymentFlow) {
	...
	for i, step := range flow.Steps {
		...
		if step.CompletedAt != nil {
			...
			status += fmt.Sprintf(" (%s)", duration.String())
		}
		log.Printf("     %d. %s: %s", i+1, step.StepName, status)
...
func demonstrateXIONFeatures() {
	...
	features := []struct {
		...
		},
		{
			...
			description: "Automated NRN minting via KNIRVORACLE treasury",
			implemented: true,
		},
		{
