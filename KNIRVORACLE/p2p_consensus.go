...
// requestChainFromPeers requests blockchain data from KNIRVORACLE nodes
func (pcm *P2PConsensusManager) requestChainFromPeers() {
	...
	if pcm.discoveryManager == nil {
		log.Printf("[%s][%s] DiscoveryManager not available, cannot find KNIRVORACLE nodes.", pcm.nodeRole.String(), pcm.blockchain.ChainID)
		return
	}
	...
	if err != nil {
		...
		if !strings.Contains(err.Error(), "no providers found") {
			log.Printf("[%s][%s] Error finding KNIRVORACLE nodes via DHT: %v", pcm.nodeRole.String(), pcm.blockchain.ChainID, err)
		} else {
			log.Printf("[%s][%s] No other KNIRVORACLE nodes found via DHT for chain sync.", pcm.nodeRole.String(), pcm.blockchain.ChainID)
		}
		return
	}

	if len(agentPeersInfo) == 0 {
		log.Printf("[%s][%s] No relevant KNIRVORACLE nodes found to sync with.", pcm.nodeRole.String(), pcm.blockchain.ChainID)
		return
	}

	log.Printf("[%s][%s] Starting P2P chain sync check with %d potential KNIRVORACLE node(s)", pcm.nodeRole.String(), pcm.blockchain.ChainID, len(agentPeersInfo))

	// --- Iterate through KNIRVORACLE nodes only ---
