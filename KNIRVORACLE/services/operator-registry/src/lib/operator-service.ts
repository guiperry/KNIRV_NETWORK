...
const KNIRV_ORACLE_URL = process.env.NEXT_PUBLIC_KNIRVORACLE_URL || 'http://localhost:1317';

// Interface for KNIRV-ORACLE registration response
interface KnirvOracleRegistrationResponse {
...
// Function to register operator with KNIRV-ORACLE and get transaction hash
export const registerOperatorWithKnirvOracle = async (operator: Operator): Promise<KnirvOracleRegistrationResponse> => {
  try {
    const registrationData = {
      ...
      factsUrl: operator.addr_facts_url,
...
const generateMockSignatureForOperator = (operatorId: string, createdDate?: string, issuer?: string, type?: string): OperatorSignature => {
  const creationDate = createdDate || new Date().toISOString();
  return {
    type: type || "RsaSignature2018",
    created: creationDate,
    verificationMethod: `${operatorId}#key-pki-1`,
...
// The mockOperators array can be kept for fallback or initial seeding.
export const mockOperators: Operator[] = [
  {
    ...
    signature: generateMockSignatureForOperator('did:nanda:agent-translator-001', new Date(2024, 0, 14).toISOString(), "PolyglotAI CA")
  },
  {
    ...
    signature: generateMockSignatureForOperator('did:nanda:agent-dataprotector-002', new Date(2023, 10, 19).toISOString(), "SecureDataCorp CA", "EcdsaSecp256k1Signature2019")
  },
  {
    ...
    signature: generateMockSignatureForOperator('did:nanda:agent-artcreator-003', new Date(2024, 1, 28).toISOString(), "ArtifyInc CA")
  },
  {
    ...
    signature: generateMockSignatureForOperator('did:nanda:agent-codehelper-004', new Date(2024, 1, 9).toISOString(), "CodeGenius Community CA")
  },
  {
    ...
    signature: generateMockSignatureForOperator('did:nanda:agent-dbquery-005', new Date(2023, 8, 4).toISOString(), "QueryWorks Trusted CA", "JsonWebSignature2020")
  }
];


export const getAgentById = async (id: string): Promise<Operator | undefined> => {
  try {
    const agentDocRef = doc(db, "agents", id);
    const agentSnap = await getDoc(agentDocRef);

    if (agentSnap.exists()) {
      return { id: agentSnap.id, ...agentSnap.data() } as Operator;
    } else {
      console.warn(`Agent with ID ${id} not found in Firestore. Checking mock data.`);
      // Fallback to mock data if not found in Firestore
      const mockAgent = mockOperators.find(agent => agent.id === id);
      if (mockAgent) {
        return mockAgent;
      }
...
export const getAllAgents = async (): Promise<Operator[]> => {
  ...
  const firestoreFetchAndCombineOperation = async (): Promise<Operator[]> => {
    let agentsListFromFirestore: Operator[] = [];
    const agentsCollectionRef = collection(db, "agents");

    try {
        const agentSnapshot = await getDocs(agentsCollectionRef);
        agentsListFromFirestore = agentSnapshot.docs.map(docSnap => ({ id: docSnap.id, ...docSnap.data() } as Operator));

        // Seeding logic (only if Firestore is completely empty and mockOperators has items)
        if (agentsListFromFirestore.length === 0 && mockOperators.length > 0) {
                console.log("No operators found in Firestore. Seeding database with mock data...");
            const batch = writeBatch(db);
            mockOperators.forEach(agent => {
                ...
                const agentData = {
                  ...
                  signature: agent.signature || generateMockSignatureForOperator(agent.id, agent.registeredAt)
                };
                batch.set(agentRef, agentData);
            });
            await batch.commit();
            console.log("Mock data seeded successfully to Firestore.");
            // After seeding, Firestore now contains the mockOperators.
            // It's important to reflect this in the data returned by this operation.
            agentsListFromFirestore = mockOperators.map(agent => ({
                ...agent,
                registeredAt: agent.registeredAt || new Date().toISOString(),
                signature: agent.signature || generateMockSignatureForOperator(agent.id, agent.registeredAt)
            }));
    ...
    // Start with a deep copy of mockOperators to ensure all hardcoded agents are present.
    const combinedAgents = mockOperators.map(agent => ({...agent}));
    const mockAgentIds = new Set(mockOperators.map(agent => agent.id));

    // Add agents from Firestore that are not already in mockOperators (based on ID)
    ...
    agentsListFromFirestore.forEach(dbAgent => {
        if (!mockAgentIds.has(dbAgent.id)) {
            ...
            // For now, let's keep it simple: mockOperators are the base, unique DB agents are added.
  ...
  try {
    ...
    // If timeout or any other error, return only a copy of mockOperators
    return mockOperators.map(agent => ({...agent}));
  }
};
...
export const registerNewAgent = async (agentData: Partial<Operator>): Promise<{ agent: Operator; transactionHash: string; capabilityId: string }> => {
  try {
    ...
    const agent: Operator = {
      ...
      signature: generateMockSignatureForOperator(agentId)
    };

    // Register with KNIRV-ORACLE first to get transaction hash
    console.log('Registering agent with KNIRV-ORACLE:', agent.name);
    const oracleResponse = await registerOperatorWithKnirvOracle(agent);
