...
class KNIRVGatewayClient {
  ...
  /**
   * Connect to oracle events SSE stream
   */
  connectGatewayEvents() {
    const url = `${this.baseUrl}/oracle/events`;
    return this.createEventSource('oracle', url, {
  ...
  handleGatewayEvent(data) {
    ...
    switch (data.type) {
      case 'connected':
        this.emit('oracle_connected', data);
        break;
      case 'metrics':
        this.updateMetricsDisplay(data.data);
        break;
      default:
        console.log('Unknown oracle event:', data);
    }
  }
  ...
  updateMetricsDisplay(metrics) {
    const metricsElement = document.getElementById('oracle-metrics');
    if (metricsElement) {
  ...
  async getGatewayMetrics() {
    return this.request('/oracle/metrics');
  }

  /**
   * Authenticate with the oracle
   */
...
// Example usage:
/*
const oracle = new KNIRVGatewayClient();

// Connect to health monitoring
oracle.connectHealthMonitor();

// Listen for events
oracle.on('health_change', (data) => {
  console.log('Service health changed:', data);
});

oracle.on('connected', (data) => {
  console.log('Connected to:', data.source);
});

// Get initial status
oracle.getHealthStatus().then(status => {
