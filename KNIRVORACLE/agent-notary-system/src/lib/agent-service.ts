...
// Function to register agent with KNIRV-ORACLE and get transaction hash
export const registerAgentWithKnirvOracle = async (agent: Agent): Promise<KnirvOracleRegistrationResponse> => {
  try {
...
export const registerNewAgent = async (agentData: Partial<Agent>): Promise<{ agent: Agent; transactionHash: string; capabilityId: string }> => {
  try {
    ...
    const oracleResponse = await registerAgentWithKnirvOracle(agent);
