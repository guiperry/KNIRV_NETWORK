```markdown
# Ansible Playbook for KNIRVORACLE Bootnode Deployment on AWS

This Ansible playbook automates the deployment of a KNIRVORACLE bootnode on Amazon Web Services (AWS). It handles both the provisioning of the necessary AWS infrastructure and the configuration of the bootnode software.
...
*   The KNIRVORACLE executable (`KNIRVORACLE_executable`) is pre-compiled and available on the Ansible control node.
...
```yaml
---
- name: Provision KNIRVORACLE Bootnode Infrastructure on AWS
  hosts: localhost
  gather_facts: false
  vars:
    ...
    security_group_name: "KNIRVORACLE-bootnode-sg"
    p2p_port: 6050 # Your KNIRVORACLE P2P port
    api_port: 6000 # Your KNIRVORACLE API port
    ssh_port: 22
    instance_tags:
      Name: "KNIRVORACLE-bootnode"
      Project: "KNIRVORACLE"
  ...
  tasks:
    - name: Create Security Group for Bootnode
      amazon.aws.ec2_security_group:
        ...
        description: "Security group for KNIRVORACLE Bootnode"
        region: "{{ aws_region }}"
        rules:
          - proto: tcp
            ports:
          ...
          - proto: tcp
            ports:
              - "{{ p2p_port }}"
            cidr_ip: "0.0.0.0/0"
            rule_desc: "Allow KNIRVORACLE P2P TCP"
          - proto: udp # If your P2P also uses UDP (e.g., QUIC)
            ports:
              - "{{ p2p_port }}"
            cidr_ip: "0.0.0.0/0"
            rule_desc: "Allow KNIRVORACLE P2P UDP"
          - proto: tcp
            ports:
              - "{{ api_port }}"
            cidr_ip: "0.0.0.0/0" # Restrict if API should not be public
            rule_desc: "Allow KNIRVORACLE API"
        tags: "{{ instance_tags }}"
      register: sg_result
...
- name: Configure KNIRVORACLE Bootnode Software
  ...
  vars:
    ...
    remote_KNIRVORACLE_bin_dir: "/opt/KNIRVORACLE"
    remote_KNIRVORACLE_app_config_dir: "/home/{{ remote_user }}/.config/KNIRVORACLE"
    remote_KNIRVORACLE_data_dir_base: "/home/{{ remote_user }}/.config/KNIRVORACLE/data" # Base for data, actual path might include ChainID
    service_name: "KNIRVORACLE-bootnode"
  ...
  tasks:
    ...
    - name: Create KNIRVORACLE binary directory
      ansible.builtin.file:
        path: "{{ remote_KNIRVORACLE_bin_dir }}"
        state: directory
        mode: '0755'

    - name: Create KNIRVORACLE application config directory
      ansible.builtin.file:
    ...
    - name: Create KNIRVORACLE application data base directory
      ansible.builtin.file:
    ...
    - name: Copy KNIRVORACLE wallet.dat
      ansible.builtin.copy:
    ...
    - name: Copy KNIRVORACLE master_wallet.dat
      ansible.builtin.copy:
    ...
    - name: Create systemd service file for KNIRVORACLE bootnode
      ansible.builtin.template:
        src: templates/KNIRVORACLE-bootnode.service.j2
        dest: "/etc/systemd/system/{{ service_name }}.service"
        mode: '0644'
      notify: Reload systemd and restart KNIRVORACLE-bootnode

  handlers:
    - name: Reload systemd and restart KNIRVORACLE-bootnode
      ansible.builtin.systemd:
        daemon_reload: true

    - name: Enable and start KNIRVORACLE-bootnode service
      ansible.builtin.systemd:
        name: "{{ service_name }}"
        enabled: true
...
## Systemd Service Template (`templates/KNIRVORACLE-bootnode.service.j2`)

Create a directory named `templates` in the same directory as your playbook. Inside it, create the file `KNIRVORACLE-bootnode.service.j2`:

```jinja
[Unit]
Description=KNIRVORACLE Bootnode Service
...
1.  **Replace Placeholders:**
    ...
    *   **In `templates/KNIRVORACLE-bootnode.service.j2`:**
        *   Review `WorkingDirectory`. The default `WorkingDirectory=/home/{{ remote_user }}` should allow the application to find its config in `~/.config/KNIRVORACLE/` correctly when run as `{{ remote_user }}`.
...
1.  Save the playbook as `deploy_bootnode_aws.yml` and the template in `templates/KNIRVORACLE-bootnode.service.j2`.
...
This playbook provides a solid foundation for deploying KNIRVORACLE bootnodes on AWS. You can extend it further with more sophisticated configuration templating, logging setup, monitoring, etc., as your needs evolve. Remember to test thoroughly in a non-production environment first.  Pay close attention to the AWS region and AMI ID; these are critical for a successful deployment.
```