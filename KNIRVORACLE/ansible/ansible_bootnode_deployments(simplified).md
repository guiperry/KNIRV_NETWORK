...
# Simplified Ansible Playbook Approach for KNIRVORACLE Bootnode Deployment on AWS

This document outlines a simplified Ansible playbook approach for deploying KNIRVORACLE bootnodes on AWS, focusing on non-interactive installation and automated deployment triggered by a webhook.
...
*   **Play 2: Software Installation:** Simplified to install prerequisites, copy the KNIRVORACLE executable, and execute it in a non-interactive bootnode installation mode.
...
*   This installer, when run, correctly:
    *   Creates `~/.config/KNIRVORACLE/` (or the equivalent path for the user the service will run as).
    ...
    *   Creates, enables, and starts the systemd service (e.g., `KNIRVORACLE-bootnode.service`) configured to run the application as a bootnode (e.g., with the `--bootnode` flag).
...
```yaml
  ...
  vars:
    ...
    # e.g., /home/{{ remote_user }}/.config/KNIRVORACLE
  ...
  tasks:
    ...
    - name: Run KNIRVORACLE installer to set up bootnode
    ...
    - name: Ensure KNIRVORACLE bootnode service is enabled and started
...
*   **Removed Directory Creation:** Tasks for creating `remote_KNIRVORACLE_app_config_dir` and `remote_KNIRVORACLE_data_dir_base` are removed, assuming your installer handles this under the user's home directory (e.g., `/home/ubuntu/.config/KNIRVORACLE`).
...
*   **Added Installer Command:** A new task `Run KNIRVORACLE installer to set up bootnode` is added.
...
*   **Simplified Service Management:** The handlers are removed. A task `Ensure KNIRVORACLE bootnode service is enabled and started` is added as a safeguard. If your installer reliably enables and starts the service, you might even remove this task. The `service_name` variable should match the name your installer uses for the service.
*   **No `templates/KNIRVORACLE-bootnode.service.j2` needed:** Since the installer creates the service file, this template is no longer required for the playbook.
*   **Permissions and `become: true`:** When Ansible runs the installer command with `become: true`, the installer will execute as root. Your installer logic needs to:
    *   Create any user-specific directories (like `~/.config/KNIRVORACLE`) and ensure they are owned by the `remote_user` (e.g., `ubuntu`).
