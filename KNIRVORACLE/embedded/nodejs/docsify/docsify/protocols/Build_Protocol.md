# KNIRVORACLE Build System

This document describes the build system for KNIRVORACLE, including role-specific builds and cross-compilation.

## Node Roles

KNIRVORACLE supports multiple node roles, each with specific functionality:
...
KNIRVORACLE now supports role-specific builds using Go build tags. This allows for creating binaries that are optimized for specific roles without requiring runtime configuration.

### Building for Specific Roles

You can build KNIRVORACLE for specific roles using the Makefile targets:
...
You can cross-compile KNIRVORACLE for multiple platforms and roles:
...
go build -tags=client -o KNIRVORACLE
...
KNIRVORACLE supports multiple user interfaces:
...
KNIRVORACLE supports the following command-line flags:
