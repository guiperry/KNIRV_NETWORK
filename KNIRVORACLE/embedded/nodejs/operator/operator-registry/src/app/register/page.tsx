...
const realisticDefaults = {
  agentName: "My New Operator",
  agentDID: `did:nanda:operator-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`, // More unique default DID
  capability: "Generic Operator Capability",
  description: "This is a newly registered operator. Its purpose is to run network services such as routers or bootnodes. Specific capabilities and details will be updated in its AgentFacts/OperatorFacts. This registration includes a simulated digital signature to demonstrate PKI concepts.",
  factsUrl: "https://example.com/.well-known/operator-facts-default.jsonld",
  providerName: "Independent Operator",
  version: "0.1.0-alpha",
  ansName: "", // Optional, can be derived or set later
  capabilities: ["Generic Operator Capability"], // Default capability array
  attestations: [],
...
const agentRegistrationSchema = z.object({
  agentName: z.string().min(3, { message: "Operator name must be at least 3 characters if provided." }).optional().or(z.literal("")),
  ...
  factsUrl: z.string().url({ message: "Please enter a valid URL for OperatorFacts if provided." }).optional().or(z.literal("")),
...
export default function RegisterOperatorPage() {
  ...
  const steps = ["Entry", "Identity", "Technical", "Validation", "Contract"];
  ...
  // Register operator with KNIRV-ORACLE integration
  const result = await registerNewOperator(agentData);

  console.log("Operator registered successfully:", result);

      toast({
        title: "Operator Registered Successfully! 🎉",
        description: (
          <div className="mt-2 w-[340px] rounded-md bg-green-950 p-4">
            <p className="text-green-100 font-medium">Your operator has been registered with KNIRV-ORACLE!</p>
            ...
            <p className="mt-2 text-xs text-green-300">
              The operator is now registered in the NANDA ANS registry and recorded on KNIRV-ORACLE.
            </p>
          </div>
        ),
        variant: "default",
      ...
      toast({
        ...
        description: (
          <div className="mt-2 w-[340px] rounded-md bg-red-950 p-4">
            <p className="text-red-100">Failed to register operator with KNIRV-ORACLE.</p>
  ...
  <Card className="shadow-lg">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-3xl font-bold">Register New Operator</CardTitle>
          <CardDescription className="text-muted-foreground">
            Onboard your operator to the Operator Registry. Registration creates an `OperatorAddr` pointer to your OperatorFacts (verifiable metadata). Fill required fields to provision your operator for onboarding tests. This form demonstrates the registration flow and will integrate with KNIRV-ORACLE during operational deployment.
          </CardDescription>
          <div className="mt-4">
      ...
      <FormField
        ...
        <FormDescription>A user-friendly name for your operator. (Optional, defaults will be used if blank)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Operator DID (Decentralized Identifier)</FormLabel>
                    <FormControl>
                      <Input placeholder={realisticDefaults.agentDID} {...field} />
                    </FormControl>
                    <FormDescription>The operator's unique DID (NANDA root identity). This will be used as the database ID. (Optional, a unique default will be generated)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>The main function your operator provides (e.g., bootnode, router). (Optional, defaults will be used if blank)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
               <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Operator Name Service (ANS) string for capability-based addressing. (Optional, a default will be constructed if blank)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Detailed description for OperatorFacts. (Optional, defaults will be used if blank)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
               <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>OperatorFacts URL (NANDA Pointer Target)</FormLabel>
                    <FormControl>
                      <Input type="url" placeholder={realisticDefaults.factsUrl} {...field} />
                    </FormControl>
                    <FormDescription>Publicly accessible URL to your OperatorFacts (JSON-LD). (Optional, defaults will be used if blank)</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Operator Provider / Organization</FormLabel>
                      <FormControl>
                ...
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    Register Operator with KNIRV-ORACLE
                  </>
                )}
              </Button>
            </form>
