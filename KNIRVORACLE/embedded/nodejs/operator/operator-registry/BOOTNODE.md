# KNIRVORACLE Bootnode Setup

This document explains how to set up and run a bootnode for the KNIRVORACLE network.

## What is a Bootnode?

A bootnode is a special node in the KNIRVORACLE network that helps other nodes discover each other. It acts as a rendezvous point for new nodes joining the network. Bootnodes register themselves with the KNIRVORACLE Node Registry, which allows other nodes to discover them.

## Prerequisites

- Node.js (v14 or later) for running the registry service
- Go (v1.16 or later) for running the KNIRVORACLE node
...
To run a KNIRVORACLE node as a bootnode:

1. Start the KNIRVORACLE node with the `--bootnode` flag:
   ```
   ./KNIRVORACLE --bootnode --p2p.port=5050
   ```
...
3. Check the logs of the KNIRVORACLE node for any error messages related to bootnode registration.
