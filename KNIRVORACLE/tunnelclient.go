...
// TunnelClient manages the connection to a central relay server for NAT traversal
...
func NewTunnelClient(cfg *config.TunnelClientConfig, nodeID, chainID, internalIP string, internalP2PPort uint, nodeType string) *TunnelClient {
	return &TunnelClient{
		config:          cfg,
		nodeID:          nodeID,
		chainID:         chainID,
...
func (c *TunnelClient) attemptReconnect() {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *TunnelClient) startPingTicker() {
	c.mutex.Lock()
	defer c.mutex.Unlock()
