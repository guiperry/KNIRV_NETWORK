...
import (
	...
	"KNIRVORACLE/config"
	"KNIRVORACLE/pkg/api"
	"KNIRVORACLE/pkg/integrations/economics"
...
func initEmbeddedNodeJSServices(cfg *config.Config) (*nodejs.EmbeddedNodeJSManager, error) {
	...
	// Create embedded Node.js manager
	embeddedManager := nodejs.NewEmbeddedNodeJSManager(&cfg.NodeJSServices)

	// Start all embedded services
	if err := embeddedManager.StartAllServices(context.Background()); err != nil {
		return embeddedManager, fmt.Errorf("failed to start embedded Node.js services: %w", err)
	}
...
func initEconomicsIntegration(cfg *config.Config) (*EconomicsIntegration, error) {
	...
	_ = economics.NewEconomicsIntegration() // Use underscore to avoid unused variable warning
	...
	// TODO: Start background sync when methods are implemented
	// economicsIntegration.StartBackgroundSync(context.Background())

	// TODO: Add economics endpoints when methods are implemented
