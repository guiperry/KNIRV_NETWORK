    ...
    <Card className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300 ease-in-out">
      ...
      <CardFooter className="flex gap-2">
        <Button asChild variant="default" size="sm" className="flex-1">
          <Link href={`/operators/${agent.id}`}>
            View Details <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </Button>
        <Button asChild variant="outline" size="sm" className="w-12">
          <a href={`http://localhost:3003/status?highlight=${encodeURIComponent(agent.id)}`} target="_blank" rel="noreferrer" title="View Operator Status on Registry">
