...
// CerebrasEmbeddingClient handles deterministic embedding generation
// Since Cerebras doesn't have a dedicated embeddings endpoint, we create high-quality
// deterministic embeddings that provide good semantic similarity for KNIRVORACLE memory
type CerebrasEmbeddingClient struct {
	APIKey string // Kept for compatibility, but not used in deterministic mode
}

// NewCerebrasEmbeddingClient creates a new Cerebras embedding client
...
func (c *CerebrasEmbeddingClient) GenerateEmbedding(ctx context.Context, text string) ([]float32, error) {
...
func (c *CerebrasEmbeddingClient) GenerateEmbeddings(ctx context.Context, texts []string) ([]float32, error) {
	if len(texts) == 0 {
	...
	fmt.Printf("DEBUG: KNIRVORACLE generated deterministic embedding for text: %.50s... (dimension: %d)\n", text, len(embedding))
	return embedding, nil
}

// CreateCerebrasEmbeddingFunction creates an embedding function for chromem-go
// This function provides deterministic, high-quality embeddings for KNIRVORACLE memory
func CreateCerebrasEmbeddingFunction(apiKey string) func(context.Context, string) ([]float32, error) {
...
// EmbedDocuments provides compatibility with the cerebras.EmbeddingFunction interface
func (c *CerebrasEmbeddingClient) EmbedDocuments(ctx context.Context, texts []string) ([][]float32, error) {
	var results [][]float32
