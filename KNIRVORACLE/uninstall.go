...
func Uninstall() {
	...
	fmt.Printf("KNIRVORACLE %s Node has been uninstalled.\n", role.String())
}
...
func UnregisterURIHandlers() error {
	...
	schemes := []uri.URIScheme{
		{
			Name:        "agent",
			Description: "KNIRVORACLE Decentralized Protocol",
		},
	}
...
func getServiceName(role config.Role) string {
	return fmt.Sprintf("KNIRVORACLE-%s", strings.ToLower(role.String()))
}
