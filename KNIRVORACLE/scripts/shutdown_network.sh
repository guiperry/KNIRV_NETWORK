...
# Comprehensive KNIRVORACLE process shutdown script

# Stage 1: Identify all KNIRVORACLE-related processes
# - By process name/args
# - By working directory
# - By network ports
PIDS=$(
    # Processes with KNIRVORACLE in command line
    pgrep -f "KNIRVORACLE"
...
if [ -z "$PIDS" ]; then
    echo "No KNIRVORACLE processes found"
    exit 0
fi

echo "Found KNIRVORACLE processes with PIDs:"
ps -fp $PIDS
...
if [ -n "$REMAINING" ]; then
    ...
    echo "All KNIRVORACLE processes terminated"
    exit 0
fi