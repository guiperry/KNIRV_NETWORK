...
# Verification script for KNIRVORACLE Node.js dependencies
# Checks if the manually created dependency files are in place and functional

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KNIRVORACLE_DIR="$(dirname "$SCRIPT_DIR")"

echo "🔍 Verifying KNIRVORACLE Node.js dependencies..."

# Check axios.cjs for agent-tunnel-registry
AXIOS_FILE="$KNIRVORACLE_DIR/agent-tunnel-registry/node_modules/axios/dist/node/axios.cjs"
echo ""
...
PSL_FILE="$KNIRVORACLE_DIR/agent-payment-gateway/node_modules/psl/dist/psl.cjs"
echo ""
...
cd "$KNIRVORACLE_DIR/agent-tunnel-registry"
...
cd "$KNIRVORACLE_DIR/agent-payment-gateway"
