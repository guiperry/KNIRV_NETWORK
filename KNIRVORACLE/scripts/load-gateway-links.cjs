 ...
 * 
 * Loads and processes oracle service links based on deployment environment.
 * Provides a unified interface for accessing oracle service URLs throughout the application.
 */
...
class GatewayLinksLoader {
  ...
  /**
   * Get oracle service URLs based on deployment mode
   */
  getGatewayServiceUrls() {
    ...
    const oracleServices = this.config.oracle_services;
    if (!oracleServices || !oracleServices[this.deploymentMode]) {
      console.warn(`[Links Loader] No oracle services configuration found for mode: ${this.deploymentMode}`);
      return {};
    }

    const services = oracleServices[this.deploymentMode];
    const urls = {};
   ...
   */
  getAllLinks() {
    const oracleServices = this.getGatewayServiceUrls();
...
window.getGatewayServiceUrl = function(serviceName) {
  const service = window.KNIRV_GATEWAY_CONFIG.oracle_services[serviceName];
...
`;
  ...
  saveJSConfig(outputPath = null) {
    const defaultPath = path.join(__dirname, '..', 'network-website', 'public', 'js', 'oracle-config.js');
  ...
  printSummary() {
    ...
    console.log('\nGateway Services:');
    for (const [serviceName, serviceConfig] of Object.entries(allLinks.oracle_services)) {
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('Usage: node load-oracle-links.js [command]');
