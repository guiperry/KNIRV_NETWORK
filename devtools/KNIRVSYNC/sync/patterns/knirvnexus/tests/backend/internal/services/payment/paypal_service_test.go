...
func TestPayPalService_GetBaseURL(t *testing.T) {
	...
	prodService := NewPayPalService("test_client", "test_secret", "testnet", "USD")
	prodURL := prodService.GetBaseURL()
	expectedProdURL := "https://api.paypal.com"
	if prodURL != expectedProdURL {
		t.Errorf("Expected testnet URL %s, got %s", expectedProdURL, prodURL)
	}
}
...
func TestPayPalService_GetCheckoutURL(t *testing.T) {
	...
	if prodURL != expectedProdURL {
		t.Errorf("Expected testnet checkout URL %s, got %s", expectedProdURL, prodURL)
	}
}
