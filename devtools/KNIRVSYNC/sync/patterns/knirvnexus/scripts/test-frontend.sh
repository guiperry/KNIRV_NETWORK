...
test_nexus_portal_build() {
    log "Testing NEXUS Portal build..."
    
    local portal_dir="$PROJECT_ROOT/../KNIRVGATEWAY/nexus-portal"
    
    if [ ! -d "$portal_dir" ]; then
        error "NEXUS Portal directory not found"
        return 1
    fi
    ...
    # Check if package.json exists
    if [ ! -f "package.json" ]; then
        error "package.json not found in NEXUS Portal"
        return 1
    fi
    ...
    if npm run build >/dev/null 2>&1; then
        success "NEXUS Portal build completed"
    else
    ...
    # Check if build output exists
    if [ -d "dist" ]; then
        success "NEXUS Portal build output exists"
    else
        error "NEXUS Portal build output not found"
        return 1
    fi
}
...
test_realtime_integration() {
    ...
    # Check for SSE integration in NEXUS Portal
    local portal_dir="$PROJECT_ROOT/../KNIRVGATEWAY/nexus-portal"
    if [ -f "$portal_dir/src/lib/realtime-service.ts" ]; then
        success "NEXUS Portal real-time service exists"
    else
        error "NEXUS Portal real-time service not found"
        return 1
    fi
    
    if [ -f "$portal_dir/src/hooks/use-realtime.ts" ]; then
        success "NEXUS Portal real-time hooks exist"
    else
        error "NEXUS Portal real-time hooks not found"
        return 1
    fi
}
