...
# 6. Start KNIRVWALLET (auto-detect real or demo)
print_status "Starting KNIRVWALLET..."
if ./scripts/start-knirvwallet.sh auto; then
    # Wait for either real (8088) or demo (8089) controller
    if curl -s --max-time 5 "http://localhost:8088/health" > /dev/null 2>&1; then
        print_success "Real KNIRVWALLET started on port 8088"
    elif curl -s --max-time 5 "http://localhost:8089/health" > /dev/null 2>&1; then
        print_success "Demo KNIRVWALLET started on port 8089"
    else
        print_warning "KNIRVWALLET health check failed, but continuing..."
    fi
else
    print_warning "Failed to start KNIRVWALLET, but continuing with testnet..."
fi
