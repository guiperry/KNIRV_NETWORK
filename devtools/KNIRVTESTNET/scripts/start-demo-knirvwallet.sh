...
# KNIRV TESTNET - Demo KNIRVWALLET Service
# Provides a lightweight demo KNIRVWALLET for testnet development
...
DEMO_CONTROLLER_PID_FILE="$TESTNET_ROOT/data/demo-knirvwallet.pid"
DEMO_CONTROLLER_LOG_FILE="$TESTNET_ROOT/logs/demo-knirvwallet.log"
...
# Create demo KNIRVWALLET service
create_demo_service() {
    print_info "Creating demo KNIRVWALLET service..."
    
    # Create demo service directory
    mkdir -p "$TESTNET_ROOT/data/demo-knirvwallet"
    
    # Create simple Node.js demo service
    cat > "$TESTNET_ROOT/data/demo-knirvwallet/demo-controller.js" << 'EOF'
...
const server = http.createServer((req, res) => {
    ...
    if (pathname === '/' || pathname === '/dashboard') {
        ...
        res.end(JSON.stringify({ error: 'Not found', service: 'demo-knirvwallet' }));
    }
});

server.listen(PORT, () => {
    console.log(`Demo KNIRVWALLET running on port ${PORT}`);
...
// Graceful shutdown
process.on('SIGTERM', () => {
    console.log('Demo KNIRVWALLET shutting down...');
    server.close(() => {
        process.exit(0);
    });
});
EOF
    ...
    cat > "$TESTNET_ROOT/data/demo-knirvwallet/package.json" << 'EOF'
{
  "name": "demo-knirvwallet",
  "version": "1.0.0",
  "description": "Demo KNIRVWALLET service for testnet",
...
EOF
    ...
    print_success "Demo KNIRVWALLET service created"
}

# Start demo KNIRVWALLET
start_demo_controller() {
    print_info "🎮 Starting Demo KNIRVWALLET service..."
    
    # Check if real controller is running
    if check_real_controller; then
        print_warning "Real KNIRVWALLET detected - skipping demo service"
        print_info "To use demo service, stop the real KNIRVWALLET first"
        return 0
    fi
    ...
    if [ -f "$DEMO_CONTROLLER_PID_FILE" ]; then
        ...
        if kill -0 "$pid" 2>/dev/null; then
            print_warning "Demo KNIRVWALLET already running (PID: $pid)"
            return 0
        else
    ...
    if [ ! -f "$TESTNET_ROOT/data/demo-knirvwallet/demo-controller.js" ]; then
        create_demo_service
    fi
    
    # Start demo service
    cd "$TESTNET_ROOT/data/demo-knirvwallet"
...
case "${1:-start}" in
    ...
    "stop")
        if [ -f "$DEMO_CONTROLLER_PID_FILE" ]; then
            ...
            if kill -0 "$pid" 2>/dev/null; then
                kill "$pid"
                rm -f "$DEMO_CONTROLLER_PID_FILE"
                print_success "Demo KNIRVWALLET stopped"
            else
    ...
    "status")
        if [ -f "$DEMO_CONTROLLER_PID_FILE" ]; then
            pid=$(cat "$DEMO_CONTROLLER_PID_FILE")
            if kill -0 "$pid" 2>/dev/null; then
                print_success "Demo KNIRVWALLET running (PID: $pid)"
                curl -s "http://localhost:$DEMO_CONTROLLER_PORT/health" | jq . 2>/dev/null || echo "Health check failed"
            else
                print_warning "Demo KNIRVWALLET not running (stale PID file)"
                rm -f "$DEMO_CONTROLLER_PID_FILE"
            fi
        else
            print_warning "Demo KNIRVWALLET not running"
