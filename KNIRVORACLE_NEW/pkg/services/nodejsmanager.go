...
func InitEmbeddedNodeJSServices(cfg *config.Config) (*nodejs.EmbeddedNodeJSManager, error) {
	...
	if cfg.NodeJSServices.PaymentGateway.Enabled {
		...
		log.Printf("[%s] [KNIRVORACLE-Faucet%d] Payment processor started successfully", cfg.ChainID, cfg.Port)
		log.Printf("[%s] [KNIRVORACLE-Faucet%d] Payment processor initialized successfully", cfg.ChainID, cfg.Port)
	}
