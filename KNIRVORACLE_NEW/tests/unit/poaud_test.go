...
func CreateTestBlockchain() (*BlockchainStruct, error) {
	...
	// Initialize the blockchain with database and MCP processor
	// We'll use a helper function to set these unexported fields
	initializeBlockchainForTest(bc, db)
...
// initializeBlockchainForTest initializes the blockchain with database and MCP processor
func initializeBlockchainForTest(bc *BlockchainStruct, db *database.LevelDB) {
	// Use setter methods to initialize unexported fields
	bc.SetDB(db)
	bc.SetMCPProcessor(NewMCPProcessor(db))
}

// CleanupTestBlockchain cleans up test blockchain resources
func CleanupTestBlockchain(bc *BlockchainStruct) {
	// Use the getter method to access the database
	if db := bc.GetDB(); db != nil {
		db.Close()
...
func TestPoAuDIntegration(t *testing.T) {
	...
	err = bc.GetMCPProcessor().GetDB().SaveCapability(testCapability)
	if err != nil {
