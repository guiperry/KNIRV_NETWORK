...
func NewEventProducer(config EventProducerConfig) *EventProducer {
	...
	if config.ClientID == "" {
		config.ClientID = "KNIRVCHAIN-terminal"
	}
...
func ConvertBlockchainEventMsg(msg messages.BlockchainEventMsg) Event {
	// Create a data map from the interface{}
	dataMap := make(map[string]interface{})
	if msgData, ok := msg.Data.(map[string]interface{}); ok {
		dataMap = msgData
	} else if msg.Data != nil {
		// If it's not a map, add it as a single value
		dataMap["value"] = msg.Data
	}
