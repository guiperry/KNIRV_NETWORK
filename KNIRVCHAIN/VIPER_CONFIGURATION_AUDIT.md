...
- **KNIRVCHAIN**: Uses `agent_` prefix (e.g., `agent_HTTPPORT=5001`)
...
- **KNIRVCHAIN**: Uses `default_config.json` + `[role]_config.json` strategy
...
### Primary Configuration System (KNIRVCHAIN)
...
- **Search Paths**: Current dir, user config dir, `/etc/KNIRVCHAIN`, role data dir
...
- **KNIRVCHAIN**: Uses `v.MergeInConfig()` for role-specific overrides
...
This audit provides the foundation for implementing Phase 5 of the KNIRVCHAIN refactor plan, ensuring consistent and maintainable configuration management across the entire system.
