...
func loadConfig() *Config {
	...
	listenAddr := getEnv("KNIRVBASE_LISTEN_ADDR", "")
	if listenAddr == "" {
	...
	config := &Config{
		NodeID:     getEnv("KNIRVBASE_NODE_ID", "knirvbase-node-1"),
		ListenAddr: listenAddr,
		DataDir:    getEnv("KNIRVBASE_DATA_DIR", getDefaultDataDir()),
		WalletKey:  getEnv("KNIRVBASE_WALLET_KEY", "mock"), // Use "mock" for development
		NetworkURL: getEnv("KNIRVBASE_NETWORK_URL", "https://api.xion.network"),
		ChainID:    getEnv("KNIRVBASE_CHAIN_ID", "xion-mainnet-1"),
		LogLevel:   getEnv("KNIRVBASE_LOG_LEVEL", "info"),
	}
	return config
}
...
func getDefaultDataDir() string {
	...
	if err != nil {
		return filepath.Join(os.TempDir(), "knirvbase") // fallback to temp dir
	}
	switch runtime.GOOS {
	case "windows":
		return filepath.Join(home, "AppData", "Roaming", "knirvbase")
	case "darwin":
		return filepath.Join(home, "Library", "Application Support", "knirvbase")
...
func main() {
	...
	// Setup logging
	logger := log.New(os.Stdout, "[KNIRVBASE] ", log.LstdFlags|log.Lshortfile)
	logger.Printf("Starting KNIRVBASE node: %s", config.NodeID)
	...
	logger.Println("")
	logger.Println("KNIRVBASE is running")
