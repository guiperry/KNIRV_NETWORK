...
```bash
git clone https://github.com/KNIRV_NETWORK/knirvbase.git
cd knirvbase
```
...
# Or manually
./bin/knirvbase --config config.yaml
```
...
WORKDIR /app
COPY . .
RUN go build -o knirvbase ./cmd/node
...
- `knirvbase_query_latency_seconds`
- `knirvbase_cache_hits_total`
...
```bash
tail -f /var/log/knirvbase/node.log
```

## 📞 Support

- **Documentation**: https://docs.knirv.network
- **Issues**: https://github.com/KNIRV_NETWORK/knirvbase/issues
- **Discussions**: https://github.com/KNIRV_NETWORK/knirvbase/discussions
