...
func showIntegrationArchitecture() {
	...
	fmt.Println("├─────────────────────────────────────────────────────────────┤")
	fmt.Println("│  KNIRVROUTER    │  KNIRVCHAIN    │  KNIRVCONTROLLER      │")
...
func demonstratePaymentFlow() {
	...
	steps := []struct {
		...
		{"4. Treasury Processing", "KNIRVCHAIN", "Validates NRV, mints NRN with bonuses", 1 * time.Second},
...
func showNextSteps() {
	...
	fmt.Printf("   cd KNIRVCHAIN/network-monitor\n")
