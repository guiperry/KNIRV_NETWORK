...
func TestBootnodeConnection(t *testing.T) {
	...
	bootNodeInfo := peer.AddrInfo{
		ID:    bootNodeDM.GetHost().ID(),
		Addrs: bootNodeDM.GetHost().Addrs(),
	}

	// Connect the client to the bootnode
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	err = clientDM.GetHost().Connect(ctx, bootNodeInfo)
	require.NoError(t, err)

	// Verify the connection
	connected := false
	for _, conn := range clientDM.GetHost().Network().Conns() {
		if conn.RemotePeer() == bootNodeDM.GetHost().ID() {
			connected = true
			break
		}
	}
