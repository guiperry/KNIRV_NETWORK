...
func TestMCPTransactionProcessing(t *testing.T) {
	...
	walletTx := BlockchainToWalletTransaction(txn)
	err = wallet.SignTransaction(walletTx)
	if err != nil {
	...
	err = utils.WaitForChromemDB(30*time.Second, func() (bool, error) {
		...
		if bc.ChromemDBSyncManager != nil {
			capCollection = bc.ChromemDBSyncManager.GetCapabilityDescriptorCollection()
		} else {
			// Fallback to legacy ChromemSync for backward compatibility
			capCollection = bc.ChromemSync.GetCapabilityDescriptorCollection()
		}
...
func TestCapabilityInvocation(t *testing.T) {
	...
	// Sign the transaction
	walletInvokeTx := BlockchainToWalletTransaction(invokeTxn)
	err = wallet.SignTransaction(walletInvokeTx)
	if err != nil {
		t.Fatalf("Failed to sign transaction: %v", err)
	}
	...
	err = utils.WaitForChromemDB(10*time.Second, func() (bool, error) {
		...
		if bc.ChromemDBSyncManager != nil {
			...
			actualCtxCollection = bc.ChromemDBSyncManager.GetContextRecordCollection()
		} else {
			// Fallback to legacy ChromemSync for backward compatibility
			actualCtxCollection = bc.ChromemSync.GetContextRecordCollection()
		}
...
func TestMCPCapabilityUpdate(t *testing.T) {
	...
	// Sign the transaction
	walletUpdateTx := BlockchainToWalletTransaction(txn)
	err = wallet.SignTransaction(walletUpdateTx)
	if err != nil {
		t.Fatalf("Failed to sign transaction: %v", err)
	}
	...
	// Sign the transaction
	walletUpdateTxn := BlockchainToWalletTransaction(updateTxn)
	err = wallet.SignTransaction(walletUpdateTxn)
	if err != nil {
		t.Fatalf("Failed to sign transaction: %v", err)
	}
	...
	err = utils.WaitForChromemDB(60*time.Second, func() (bool, error) {
		...
		if bc.ChromemDBSyncManager != nil {
			// Ensure the sync manager is initialized
			ctxCollection = bc.ChromemDBSyncManager.GetContextRecordCollection()
		} else {
			// Fallback to legacy ChromemSync for backward compatibility
			ctxCollection = bc.ChromemSync.GetContextRecordCollection()
		}
...
func TestValidateTransactionInBlockContext_DuplicateCapability(t *testing.T) {
	...
	walletTxn1 := BlockchainToWalletTransaction(txn1)
	err = wallet.SignTransaction(walletTxn1)
	if err != nil {
	...
	walletTxn2 := BlockchainToWalletTransaction(txn2)
	err = wallet.SignTransaction(walletTxn2)
	if err != nil {
	...
	walletTxn2_2 := BlockchainToWalletTransaction(txn2)
	err = wallet.SignTransaction(walletTxn2_2)
	if err != nil {
		t.Fatalf("Failed to sign second transaction: %v", err)
	}
...
func TestChromemDBSkipsInvalidTransactions(t *testing.T) {
	...
	walletInvalidTxn := BlockchainToWalletTransaction(invalidTxn)
	err = wallet.SignTransaction(walletInvalidTxn)
	if err != nil {
		t.Fatalf("Failed to sign transaction: %v", err)
	}
