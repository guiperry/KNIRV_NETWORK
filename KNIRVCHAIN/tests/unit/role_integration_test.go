...
func TestRoleBasedConfigPaths(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "KNIRVCHAIN-role-test")
	if err != nil {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.role != config.Root && (tc.testWallet == "miner" || tc.testWallet == "both") {
				...
				if testConfig.MinersAddress != "" {
					devWalletImpl, err := wm.LoadWallet(testConfig.MinersAddress, tc.role)
					if err == nil {
						devWalletObj = devWalletImpl
					}
					if tc.expectSuccess {
						if err != nil {
				...
				if tc.role == config.RoleBootnode && (tc.testWallet == "master" || tc.testWallet == "both") {
					if testConfig.MasterAddress != "" {
						masterWalletImpl, err := wm.LoadMasterWallet(testConfig.MasterAddress, tc.role)
						if err == nil {
							masterWalletObj = masterWalletImpl
						}
						if tc.expectSuccess {
							if err != nil {
...
func TestGUIRoleBasedFeatures(t *testing.T) {
	...
	testCases := []struct {
		name              string
		role              config.Role
		wallet            Wallet // Change to interface type
		paymentEnabled    bool
		expectMining      bool
