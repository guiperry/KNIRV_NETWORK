...
func newTestConsensusManager(_ *BlockchainStruct) *MockP2PConsensusManager {
	return NewMockP2PConsensusManager()
}
...
func TestTransactionFlow(t *testing.T) {
	...
	fundingWalletTx := ConvertForSigning(fundingTxn)
	signedFundingWalletTxn, err := blockchainWallet.GetSignedTxn(fundingWalletTx)
	if err != nil {
		t.Fatalf("Failed to sign funding transaction: %v", err)
	}

	// Convert back to blockchain transaction
	signedFundingTxn := WalletToBlockchainTransaction(signedFundingWalletTxn)
	...
	mockCM := newTestConsensusManager(bc)
	var cm p2p.ConsensusManager = mockCM // Cast to p2p.ConsensusManager interface
	bc.Lock()
	...
	txnWalletTx := ConvertForSigning(txn)
	signedWalletTxn, err := senderWallet.GetSignedTxn(txnWalletTx)
	if err != nil {
		bc.StopMining = true
		<-miningDone
	...
	// Convert back to blockchain transaction
	signedTxn := WalletToBlockchainTransaction(signedWalletTxn)
