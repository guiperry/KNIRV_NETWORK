...
*   **Capabilities:** These are the core building blocks of the MCP ecosystem on KNIRVCHAIN. They include:
...
1.  **Capability Registration:**
    *   Developers register their capabilities (e.g., a new Plugin) by submitting an `MCPRegisterCapabilityTransaction` to the KNIRVCHAIN.
...
2.  **Capability Discovery:**
    *   agent Clients query the KNIRVCHAIN API (e.g., `GET /mcp/capabilities?resourceType=PLUGIN`) to find available capabilities.
...
3.  **Capability Invocation & Plugin Execution:**
    ...
    *   **Logging Usage:** The client (or wallet server on its behalf) creates an `MCPInvokeCapabilityTransaction`.
        ...
        *   This transaction is submitted to the KNIRVCHAIN, creating an on-chain audit log.
...
The `ContextRecord` is central to KNIRVCHAIN's value, providing an on-chain, verifiable log. Here are some examples:
...
These records are crucial for billing, provenance, debugging, and analytics within the KNIRVCHAIN MCP ecosystem.
