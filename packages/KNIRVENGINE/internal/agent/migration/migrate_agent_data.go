...
import (
	...
	"KNIRVENGINE/desktop-client/internal/agent/service"
...
func MigrateAgentData() {
	...
	// Set default writable paths in the engine App Data directory. Templates
	// are source assets and remain a read-only project-relative default.
	appDataDir, err := utils.GetAppDataDir()
	if err != nil {
		log.Fatalf("resolve application data directory: %v", err)
	}
	appDatabaseDir, err := utils.GetDatabaseDir()
	if err != nil {
		log.Fatalf("resolve application database directory: %v", err)
	}
	appPluginsDir, err := utils.GetPluginsDir()
	if err != nil {
		log.Fatalf("resolve application plugins directory: %v", err)
	}

	if *pluginsDir == "" {
		*pluginsDir = appPluginsDir
	}
	if *wasmDir == "" {
		*wasmDir = appPluginsDir
	}
	if *templatesDir == "" {
		*templatesDir = "./internal/agent/templates"
	}
	if *outputDir == "" {
		*outputDir = appPluginsDir
	}
	if *dataDir == "" {
		*dataDir = appDatabaseDir
	}
	if *reportPath == "" {
		*reportPath = filepath.Join(appDataDir, "reports", "migration_report.json")
	}
	log.Printf("Using KNIRVENGINE App Data directory: %s", appDataDir)
