...
import { ChevronDown, ChevronRight, File, FileCode2, Folder, FolderOpen, Plus, RefreshCw, Save, X } from 'lucide-react';

type ProjectFile = { name: string; path: string; file: File; handle?: FileHandle };
type FileTreeNode = { name: string; path: string; file?: File; handle?: FileHandle; children: FileTreeNode[] };
...
const createTree = (files: ProjectFile[]): FileTreeNode[] => {
  ...
  files.forEach(({ path, file, handle }) => {
    ...
    parts.forEach((part, index) => {
      ...
      if (index === parts.length - 1) { child.file = file; child.handle = handle; }
      node = child;
    });
  });
...
export const Dashboard = () => {
  ...
  const chooseTarget = async () => {
    ...
    try {
      ...
      setTree(createTree(files));
      if (!files.length) setLoadError('This folder does not contain any files.');
    } catch (error) {
  ...
  const handleInput = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    loadTarget(name, files.map((file) => {
  ...
  const selectFile = async (node: FileTreeNode) => {
    ...
    try {
      if (node.file.size > TEXT_FILE_LIMIT && !isPreviewable(node.file)) setContent('This file is too large to display in the page explorer.');
  ...
  const clearTarget = () => { setTargetName(''); setTree([]); setSelected(undefined); setContent(''); setDraft(''); setSavedContent(''); setSaveMessage(''); setLoadError(''); setLoadingTarget(false); };
