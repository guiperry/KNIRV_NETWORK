...
export function ToolStreamConsole({ title, tool, inputLabel = 'Interface', defaultValue = 'any', inputKey = 'interface' }: { title: string; tool: string; inputLabel?: string; defaultValue?: string; inputKey?: string }) {
  const { session, isReady } = useSandboxSession();
  const { events, running, error, start, stop, clearEvents } = useToolStream({ sessionID: session?.id ?? '', tool });
  const [value, setValue] = useState(defaultValue);
  const run = useCallback(() => start({ [inputKey]: value }), [inputKey, start, value]);
  return <div className="h-full bg-slate-900 p-6 space-y-4"><header className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-bold text-white">{title}</h1><p className="text-sm font-mono text-slate-400">namespace-joined live stream · {tool}</p></div><div className="flex gap-2"><button onClick={run} disabled={!isReady || running} className="flex items-center gap-2 rounded-lg border border-cyan-500/40 bg-cyan-500/20 px-3 py-2 text-sm text-cyan-200 disabled:opacity-50"><Play className="h-4 w-4" />Start</button><button onClick={stop} disabled={!running} className="flex items-center gap-2 rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-200 disabled:opacity-50"><Square className="h-4 w-4" />Stop</button></div></header><SessionNotice ready={isReady} /><label className="block text-xs font-mono text-slate-400">{inputLabel}<input value={value} onChange={e => setValue(e.target.value)} className="mt-1 w-full rounded border border-slate-700/50 bg-slate-950/60 px-3 py-2 text-slate-200" /></label>{error && <div className="rounded border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">{error}</div>}<div className="flex justify-end"><button onClick={clearEvents} className="text-xs text-slate-400 hover:text-white">Clear</button></div><pre className={outputClass}>{events.length ? events.map(e => `[${new Date(e.timestamp).toLocaleTimeString()}] ${e.rawLine || JSON.stringify(e.payload)}`).join('\n') : running ? 'Waiting for events…' : 'No events recorded.'}</pre></div>;
}

export function ToolAttachConsole({ title, tool }: { title: string; tool: string }) {
  const { session, isReady } = useSandboxSession();
  const { attached, pid, log, error, attach, detach, send } = useToolAttach({ sessionID: session?.id ?? '', tool });
  const [pidInput, setPidInput] = useState('');
  const [script, setScript] = useState('');
  const connect = useCallback(() => attach(Number(pidInput) || 0, script ? { script } : {}), [attach, pidInput, script]);
  return <div className="h-full bg-slate-900 p-6 space-y-4"><header className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-white">{title}</h1><p className="text-sm font-mono text-slate-400">namespace PID attach · {attached ? `attached to ${pid}` : 'detached'}</p></div><button onClick={attached ? detach : connect} disabled={!isReady} className="rounded-lg border border-cyan-500/40 bg-cyan-500/20 px-3 py-2 text-sm text-cyan-200 disabled:opacity-50">{attached ? 'Detach' : 'Attach'}</button></header><SessionNotice ready={isReady} /><input value={pidInput} onChange={e => setPidInput(e.target.value)} placeholder="Target PID (leave empty to choose the sandbox target)" className="w-full rounded border border-slate-700/50 bg-slate-950/60 px-3 py-2 font-mono text-xs text-slate-200" /><textarea value={script} onChange={e => setScript(e.target.value)} placeholder="Frida script path inside the mounted project (optional)" className="h-36 w-full resize-none rounded border border-slate-700/50 bg-slate-950/60 p-3 font-mono text-xs text-slate-200" />{attached && <button onClick={() => send('reload', {})} className="rounded border border-slate-600 px-2 py-1 text-xs text-slate-300">Send reload</button>}{error && <div className="rounded border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">{error}</div>}<pre className={outputClass}>{log.join('\n') || 'No bridge output yet.'}</pre></div>;
}
...
export function ToolHeadlessConsole({ title, tool }: { title: string; tool: string }) {
  ...
  const analyze = useCallback(async () => { if (!session) return; setRunning(true); setError(null); try { const result = await runToolAnalysis(session.id, tool, binaryPath ? { binaryPath } : {}); setOutput(result.rawOutput || (result.structured ? JSON.stringify(result.structured, null, 2) : 'Analysis completed with no output.')); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setRunning(false); } }, [binaryPath, session, tool]);
  return <div className="h-full bg-slate-900 p-6 space-y-4"><header className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-white">{title}</h1><p className="text-sm font-mono text-slate-400">headless native analysis · {tool}</p></div><button onClick={analyze} disabled={!isReady || running} className="flex items-center gap-2 rounded-lg border border-red-500/40 bg-red-500/20 px-3 py-2 text-sm text-red-200 disabled:opacity-50"><Play className="h-4 w-4" />{running ? 'Analyzing…' : 'Analyze'}</button></header><SessionNotice ready={isReady} /><input value={binaryPath} onChange={e => setBinaryPath(e.target.value)} placeholder="Binary path (defaults to the project mount)" className="w-full rounded border border-slate-700/50 bg-slate-950/60 px-3 py-2 font-mono text-xs text-slate-200" />{error && <div className="rounded border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">{error}</div>}<pre className={outputClass}>{output}</pre></div>;
}
