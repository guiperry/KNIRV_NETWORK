...
`sandbox_deps.go` today hardcodes "package manager only." The 13 remaining tools split across acquisition strategies:

| Strategy | Tools | Notes |
|---|---|---|
| System package manager (existing pattern) | bpftrace, tshark/wireshark-cli, zeek, proxychains-ng (`proxychains4`), afl++, rizin (Cutter's engine) | Same `requiredTool.packages` map shape already used for bwrap/Xvfb/x11vnc. rizin packages cleanly on most distros (`apt install rizin`, `dnf install rizin`), which is part of why it won the Consolidation Audit over Ghidra. |
| pip (new) | semgrep, jwt_tool (`jwt_tool` PyPI or its GitHub script), frida-tools (the CLI half of Lane 3) | Needs a managed venv under the tools dir, not global pip install — see Phase 1 task |
| GitHub release binary (new) | jadx, ilspycmd (dotnet — actually a dotnet global tool, its own sub-case), rizin (fallback if the target distro's package manager lags) | Same `bundle-sandbox-tools.sh`-style vendoring as bwrap, but downloaded at build time instead of `ldd`-copied from a system install |
| Compiled in — not a runtime dependency at all | Tree-sitter (cgo binding, Lane 6), SAML Raider (pure Go, Lane 6) | No entry in `requiredTool`/`DependencyStatus` needed; Tree-sitter's only requirement is a C toolchain (`gcc`) in the **build** environment (dev machine, CI, and the `Containerfile`'s *build* stage — not its runtime stage) |

Every subprocess-based strategy still reports through the same `DependencyStatus` struct (`sandbox_deps.go:15-21`) so `Bubblewrap.tsx`'s existing missing-deps banner needs **zero frontend changes** — this is why Phase 1 is worth doing generically before any Lane work starts. Lane 6 tools never appear in that banner at all, since there's nothing to be missing at runtime.
...
- Add `githubReleaseStrategy{repo, assetPattern}` — downloads a release tarball/binary matching the current OS/arch, extracts into `sandboxToolsDir()`, chmod +x, same as a bundled binary today.
...
- Extend `scripts/bundle-sandbox-tools.sh`'s `TOOLS` array and add non-`ldd` copy steps (venv directory copy) for build-time bundling into the shipped `dist/tools/`.
- `Containerfile`: add the system-package-manager-strategy tools (including `rizin`) to the existing `microdnf install` block (same pattern as bwrap/Xvfb/x11vnc, lines 38-42); confirm the build stage already has a C toolchain for Tree-sitter's cgo build, add one if not.
...
**Tasks:**
- `api/sandbox_tool_cutter.go`: spawn `rizin`'s headless engine (`rz-core`) with JSON-emitting analysis commands — e.g. `aflj` for a function-list JSON, and a decompilation command for the selected function; parse into the same `functions[]`/`decompiled{}`/`listing{}` shape `Cutter.tsx` inherits from the retired `Ghidra.tsx` mock.
...
|---|---|---|---|---|
| Semgrep | 1 | `semgrep` | pip | `sandbox_tool_semgrep.go` |
| Tree-sitter | 6 | `go-tree-sitter` cgo binding | compiled in (`go.mod` + C toolchain) | `sandbox_tool_treesitter.go` |
| Jadx | 1 | `jadx` | GitHub release | `sandbox_tool_jadx.go` |
| ILSpy | 1 | `ilspycmd` | dotnet tool | `sandbox_tool_ilspy.go` |
| jwt_tool | 1 | `jwt_tool.py` | pip/git | `sandbox_tool_jwttool.go` |
| SAML Raider | 6 | native Go (`encoding/xml`) | none — reimplemented, no acquisition | `sandbox_tool_samlraider.go` |
| proxychains-ng | 4 | `proxychains4` | package manager | `sandbox_tool_proxychains.go` |
| bpftrace | 2 | `bpftrace` | package manager | `sandbox_tool_bpftrace.go` |
| Wireshark | 2 | `tshark` | package manager | `sandbox_tool_tshark.go` |
| Zeek | 2 | `zeek` | package manager (own repo) | `sandbox_tool_zeek.go` |
| AFL++ | 2 | `afl-fuzz` | package manager or source build | `sandbox_tool_afl.go` |
| Cutter | 5 | `rz-core` (rizin) | package manager, GitHub release fallback | `sandbox_tool_cutter.go` |
| Frida | 3 | `frida` (pip) + `frida-server` | pip + arch binary | `sandbox_tool_frida.go` |
