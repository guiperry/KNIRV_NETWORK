...
if [ ! -f "assembly/index.ts" ]; then
    echo "⚠️  assembly/index.ts not found, creating a minimal version..."
    mkdir -p assembly
...
if npx asc assembly/index.ts \
    ...
    echo "⚠️  Release WASM build failed, using fallback..."
    # Create a minimal WASM file as fallback
...
if npx asc assembly/index.ts \
    ...
    echo "⚠️  Debug WASM build failed, using fallback..."
    # Create a minimal WASM file as fallback
