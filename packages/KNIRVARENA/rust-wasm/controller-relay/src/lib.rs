...
static mut SEQUENCE_STATE: Option<SequenceState> = None;

fn sequence_state() -> &'static mut SequenceState {
    unsafe {
        SEQUENCE_STATE.get_or_insert_with(|| SequenceState::default())
    }
}
...
static mut RESULT_POOL: Option<ResultPool> = None;

fn result_pool() -> &'static mut ResultPool {
    unsafe {
        RESULT_POOL.get_or_insert_with(|| ResultPool::new())
    }
}
...
pub extern "C" fn self_test(_input_pointer: *const u8, _input_length: usize) -> i32 {
    ...
    let msg = r#"{"status":"ok"}"#;
    result_pool().push(msg.as_bytes().to_vec()) as i32
}

#[no_mangle]
pub extern "C" fn result_pointer(handle: i32) -> usize {
    let handle = handle as usize;
    match result_pool().get(handle) {
        Some(bytes) => bytes.as_ptr() as usize,
        None => 0,
    }
}
...
pub extern "C" fn free_result(handle: i32) {
    let handle = handle as usize;
    if handle < result_pool().pool.len() {
        result_pool().pool[handle].clear();
    }
}
...
pub extern "C" fn invoke(
    ...
    match op.as_str() {
        ...
        _ => {
            ...
            result_pool().push(err.as_bytes().to_vec()) as i32
        }
    }
}
...
fn __validate_envelope(ptr: *const u8, len: usize) -> i32 {
    ...
    if !sequence_state().check_and_update(&seq_key, sequence) {
...
fn __check_sequence(ptr: *const u8, len: usize) -> i32 {
    ...
    let accepted = sequence_state().check_and_update(&key, seq);
...
pub extern "C" fn wasm_init() {
    unsafe {
        SEQUENCE_STATE = Some(SequenceState::default());
        RESULT_POOL = Some(ResultPool::new());
    }
}
...
fn put_result(bytes: &str) -> i32 {
    result_pool().push(bytes.as_bytes().to_vec()) as i32
}
