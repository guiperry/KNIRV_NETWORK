#!/usr/bin/env node
/**
 * KNIRVClient Log Ingestion Client
 *
 * A Node.js client for sending logs to KNIRVClient's log ingestion API.
 *
 * Usage:
...
class KNIRVClientClient {
    /**
     * Client for KNIRVClient Log Ingestion API.
     * @param {string} baseUrl - Base URL of the KNIRVClient API
    ...
    /**
     * Tail a log file and send logs to KNIRVClient.
...
async function main() {
    ...
    const client = new KNIRVClientClient(options.url);
...
Options:
  --url <url>           KNIRVClient API base URL (default: http://localhost:3000/api/v1)
