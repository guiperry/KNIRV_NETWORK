...
# Replace 'knirvclient-releases' with your R2 bucket name.
RCLONE_REMOTE="incline"
RCLONE_BUCKET="knirvclient"
...
rclone copy "${BUILD_DIR}/knirvclient-windows-amd64.exe" "${RCLONE_REMOTE}:${RCLONE_BUCKET}/windows/"

# Upload Linux binary
echo "  -> Uploading Linux binary..."
rclone copy "${BUILD_DIR}/knirvclient-linux-amd64" "${RCLONE_REMOTE}:${RCLONE_BUCKET}/linux/"

# Upload macOS binaries
echo "  -> Uploading macOS binaries..."
rclone copy "${BUILD_DIR}/knirvclient-darwin-amd64" "${RCLONE_REMOTE}:${RCLONE_BUCKET}/mac/"
rclone copy "${BUILD_DIR}/knirvclient-darwin-arm64" "${RCLONE_REMOTE}:${RCLONE_BUCKET}/mac/"
