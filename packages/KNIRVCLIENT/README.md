...
[![Status](https://img.shields.io/badge/status-production--ready-green.svg)](https://github.com/guiperry/knirvclient)
...
KNIRV Client uses a **unified port architecture** where all services are consolidated on **port 3000** for simplified deployment and easier management.
...
graph TB
    ...
    subgraph "KNIRV Client Unified Server - Port 3000"
    ...
    subgraph "Storage Layer"
        ChromemDB[(Chromem-go Vector DB<br/>./knirvclient-data/)]
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/guiperry/knirvclient.git
   cd knirvclient
   ```
...
KNIRV Client includes an integrated **Web Baseline Compatibility Checker** that validates your web code against the [Web Platform Baseline](https://web-platform-dx.github.io/web-features/) standards to ensure cross-browser compatibility.
...
3. **Real-Time Scanning**: During each scan cycle, KNIRV Client analyzes:
...
KNIRV Client automatically selects the best AI provider based on availability and task type. Configure preferences in `.env`:
...
```env
# Database path
CHROMEM_DB_PATH=./knirvclient-data
...
Based on the comprehensive gap analysis, KNIRV Client has completed all major implementation phases:
...
- **Disclosure**: [Vulnerability Disclosure](https://github.com/guiperry/knirvclient/wiki/VULNERABILITY_DISCLOSURE_POLICY.md)
- **Documentation**: [Full documentation](https://github.com/guiperry/knirvclient/wiki)
- **Issues**: [GitHub Issues](https://github.com/guiperry/knirvclient/issues)
...
KNIRV Client is designed for performance:
