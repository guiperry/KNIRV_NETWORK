// KNIRV Client Dashboard JavaScript

class KNIRVClientDashboard {
    constructor() {
    ...
    connectWebSocket() {
        ...
        this.ws.onopen = () => {
            console.log('Connected to KNIRV Client WebSocket');
    ...
    showBrowserNotification(title, body, level = 'info') {
        ...
        if ('Notification' in window) {
            if (Notification.permission === 'granted') {
                const notification = new Notification(`KNIRV Client - ${title}`, {
                    body: body,
                    icon: '/favicon.ico',
                    tag: 'knirvclient-alert',
    ...
    async loadSettings() {
        try {
            ...
            if (settings) {
                document.getElementById('server-port').value = settings.server_port || window.location.port || '';
                ...
                this.updateEndpointsList(settings.server_port || window.location.port);
            }
        } catch (error) {
    ...
    saveSettings() {
        ...
        fetch('/api/v1/settings', {
            ...
            body: JSON.stringify({
                // The active server port is selected during startup and cannot
                // be changed by a settings request while this instance runs.
    ...
    resetSettings() {
        document.getElementById('server-port').value = window.location.port || '';
        ...
        this.updateEndpointsList(window.location.port);
    }
    ...
    // Theme Management
    loadThemePreference() {
        const savedTheme = localStorage.getItem('knirvclient-theme');
        if (savedTheme) {
    ...
    saveThemePreference() {
        localStorage.setItem('knirvclient-theme', this.currentTheme);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.dashboard = new KNIRVClientDashboard();
});
