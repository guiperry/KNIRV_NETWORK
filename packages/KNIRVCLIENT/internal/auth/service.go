...
// SetBaseURL updates the callback origin after the server has reserved its
// dynamic port. An explicitly configured APP_BASE_URL remains authoritative.
...
func (as *AuthService) HandleGitHubAuth(w http.ResponseWriter, r *http.Request) (string, string, error) {
	...
	session, _ := as.sessionStore.Get(r, "knirvclient-auth")
