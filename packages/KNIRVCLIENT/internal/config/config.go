...
type Config struct {
	...
	NRV               NRVConfig          `json:"nrv"`
}

// NRVConfig controls automatic packaging and submission of detected
// errors/vulnerabilities to KNIRVGRAPH as .nrv reports.
type NRVConfig struct {
	Enable       bool   `json:"enable"`        // NRV_REPORTING_ENABLE
	Network      string `json:"network"`       // NRV_NETWORK: "testnet" | "mainnet" | "custom"
	GatewayURL   string `json:"gateway_url"`   // NRV_GATEWAY_URL — overrides the network default; required when Network=="custom"
	ChainID      string `json:"chain_id"`      // NRV_CHAIN_ID — overrides the network default KNIRVSDK signing chain ID
	ContextLines int    `json:"context_lines"` // NRV_CONTEXT_LINES
}
...
// ResolvedGatewayURL returns the KNIRVGATEWAY base URL .nrv reports should
// be submitted to: an explicit GatewayURL override if set, otherwise the
// well-known testnet/mainnet gateway host for Network.
func (c NRVConfig) ResolvedGatewayURL() string {
	if c.GatewayURL != "" {
		return c.GatewayURL
	}
	if strings.EqualFold(c.Network, "mainnet") {
		return mainnetGatewayURL
	}
	return testnetGatewayURL
}

// ResolvedChainID returns the KNIRVSDK signing chain ID .nrv commit
// signatures are domain-separated against: an explicit ChainID override if
// set, otherwise the well-known testnet/mainnet chain ID for Network.
func (c NRVConfig) ResolvedChainID() string {
	if c.ChainID != "" {
		return c.ChainID
	}
	if strings.EqualFold(c.Network, "mainnet") {
...
func Load() *Config {
	return &Config{
		...
		RemediationBranch: getEnv("REMEDIATION_BRANCH", "knirvclient-fixes"),
		// Zero asks the operating system to select an available loopback port.
		...
		DataEngine: DataEngineConfig{
			...
			ChromaCollection: getEnv("CHROMADB_COLLECTION", "knirvclient_events"),
			WebSocketPort:    getEnvInt("WEBSOCKET_PORT", 8080),
			RESTAPIPort:      getEnvInt("RESTAPI_PORT", 7080),
		},
		NRV: NRVConfig{
			Enable:       getEnvBool("NRV_REPORTING_ENABLE", true),
			Network:      getEnv("NRV_NETWORK", "testnet"),
			GatewayURL:   getEnv("NRV_GATEWAY_URL", ""),
			ChainID:      getEnv("NRV_CHAIN_ID", ""),
			ContextLines: getEnvInt("NRV_CONTEXT_LINES", 15),
		},
	}
}
...
func getDefaultSettings() *Config {
	return &Config{
		...
		RemediationBranch: "knirvclient-fixes",
		ServerPort:        0,
		AIProviders: AIProviderConfig{
