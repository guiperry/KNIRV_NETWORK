...
func IsValidConfigFilePath(filePath string) bool {
	...
	if strings.HasPrefix(filePath, "/") {
		// Allow only specific safe directories
		safePrefixes := []string{"./", ".knirvclient/", "config/", "configs/"}
		isSafe := false
		for _, prefix := range safePrefixes {
...
// GetAppDataDir returns the OS-specific application data directory for KNIRVClient
// Returns:
//   - Linux: $XDG_DATA_HOME/knirvclient or $HOME/.local/share/knirvclient
//   - macOS: $HOME/Library/Application Support/knirvclient
//   - Windows: %APPDATA%\knirvclient
func GetAppDataDir() (string, error) {
	...
	case "windows":
		...
		appDir = filepath.Join(baseDir, "knirvclient")
	...
	case "darwin":
		...
		appDir = filepath.Join(homeDir, "Library", "Application Support", "knirvclient")

	default:
		// Linux and other Unix-like systems: Use XDG_DATA_HOME or ~/.local/share
		xdgDataHome := os.Getenv("XDG_DATA_HOME")
		if xdgDataHome != "" {
			appDir = filepath.Join(xdgDataHome, "knirvclient")
		} else {
			homeDir, err := os.UserHomeDir()
			if err != nil {
				return "", fmt.Errorf("failed to get user home directory: %w", err)
			}
			appDir = filepath.Join(homeDir, ".local", "share", "knirvclient")
		}
	}
...
// GetKNIRVClientDataPath returns the full path to the knirvclient-data directory
// in the OS-specific application data location
func GetKNIRVClientDataPath() (string, error) {
	appDir, err := GetAppDataDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(appDir, "knirvclient-data"), nil
}
