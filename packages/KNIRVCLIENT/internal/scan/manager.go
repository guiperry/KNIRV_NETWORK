...
func NewScanManager(cfg *config.Config, guardian *guardian.KNIRVClient) *ScanManager {
...
func (sm *ScanManager) UpdateJobProgress(jobID string, progress float64, message string) error {
	...
	// Broadcast progress update via WebSocket if KNIRVClient is available
	if sm.guardian != nil {
...
func (sw *scanWorker) processJob(job *ScanJob) {
	...
	// Perform the actual scan using KNIRVClient
	if sw.manager.guardian != nil {
		...
		sw.manager.FailJob(job.ID, fmt.Errorf("KNIRV Client not available"))
	}
}
