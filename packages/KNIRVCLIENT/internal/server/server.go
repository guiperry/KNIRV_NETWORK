...
// KNIRVClient represents the main application instance
type KNIRVClient struct {
	Config       *config.Config
	ProjectStore *project.ProjectStore
	AuthService  *auth.AuthService
	WSManager    *websocket.WebSocketManager
	Guardian     *guardian.KNIRVClient
...
// Start starts the server on the requested port when it is available, or an
// operating-system-selected loopback port otherwise. The selected port is sent
// to ready before requests are accepted so callers can safely launch clients.
func Start(ctx context.Context, guardian *KNIRVClient, authService *auth.AuthService, ready chan<- int) error {
	listener, err := listenOnAvailablePort(guardian.Config.ServerPort)
	if err != nil {
		return err
	}
	...
	guardian.Config.ServerPort = port
	authService.SetBaseURL(fmt.Sprintf("http://localhost:%d", port))
	...
	log.Printf("🌐 Starting KNIRV Client Consolidated Server on port %d...", port)
	...
	// Tell the launcher the definitive port only after it has been reserved.
	ready <- port
...
// listenOnAvailablePort reserves a port rather than probing and reopening it,
// preventing another process from claiming the port between selection and use.
func listenOnAvailablePort(preferredPort int) (net.Listener, error) {
	if preferredPort > 0 {
		listener, err := net.Listen("tcp", fmt.Sprintf("127.0.0.1:%d", preferredPort))
		if err == nil {
			return listener, nil
		}
		log.Printf("⚠️  Requested port %d is unavailable; selecting an open port", preferredPort)
	}
...
func setupRoutes(router *mux.Router, guardian *KNIRVClient, authService *auth.AuthService) {
...
func handleStartProjectScan(w http.ResponseWriter, r *http.Request, guardian *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
...
func handleGetAlerts(w http.ResponseWriter, _ *http.Request, _ *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
...
func handleResolveAlert(w http.ResponseWriter, r *http.Request, _ *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
...
func handleClearResolvedAlerts(w http.ResponseWriter, _ *http.Request, _ *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
...
func handleRemediateIssue(w http.ResponseWriter, r *http.Request, guardian *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Access-Control-Allow-Origin", "*")
...
func handleApplySolution(w http.ResponseWriter, r *http.Request, guardian *KNIRVClient) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Access-Control-Allow-Origin", "*")
