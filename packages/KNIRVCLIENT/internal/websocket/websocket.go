...
type WebSocketManager struct {
	...
	knirvClient *guardian.KNIRVClient // Reference to KNIRVClient for delegating broadcasts
}
...
// SetKNIRVClient sets the KNIRVClient reference for delegating broadcasts
func (wsm *WebSocketManager) SetKNIRVClient(ag *guardian.KNIRVClient) {
	wsm.knirvClient = ag
}

// HandleConnection handles a new WebSocket connection
func (wsm *WebSocketManager) HandleConnection(w http.ResponseWriter, r *http.Request, knirvClient *guardian.KNIRVClient) {
	...
	// Register with KNIRVClient for log streaming
	// KNIRVClient uses a dedicated broadcastWorker goroutine that handles all writes sequentially,
	// preventing concurrent write panics. The WebSocketManager should NOT write to these connections
	// directly - only KNIRVClient's broadcastWorker should write to them.
	if knirvClient != nil {
		knirvClient.AddDashboardConnection(conn)
	}
	...
	// Unregister from KNIRVClient
	if knirvClient != nil {
		knirvClient.RemoveDashboardConnection(conn)
	}
...
// This delegates to KNIRVClient's broadcast system to prevent concurrent writes
func (wsm *WebSocketManager) BroadcastMessage(msgType string, data interface{}) {
	if wsm.knirvClient != nil {
		// Delegate to KNIRVClient's safe broadcast system
		wsm.knirvClient.BroadcastToDashboard(msgType, data)
	} else {
		// Fallback: write directly if KNIRVClient is not available
