...
// correlate checks whether the observed action is semantically consistent with
// the registered intent by verifying that a majority of the intent's keywords
// appear in the action string.  Returns true when there is no intent to compare
// against (i.e. the agent did not pre-register an intent for this PID).
func (s *NexusMemoryServer) correlate(intent, action string) bool {
	...
	stop := map[string]bool{
		...
		"this": true, "from": true, "will": true, "have": true, "been": true,
	}
	...
	// Require at least half the intent keywords to appear in the observed action.
	return matches*2 >= len(keywords)
}
