...
func (npm *NativeProcessManager) RegisterProcess(processInfo *ProcessInfo) error {
	...
	if npm.cgroupManager.enabled {
		cgroupPath := fmt.Sprintf("knirv-objects/%s", processInfo.PluginID)
...
// GetProcessesByPlugin returns processes for a specific plugin item
func (npm *NativeProcessManager) GetProcessesByPlugin(pluginID string) []*ProcessInfo {
	npm.mu.RLock()
	defer npm.mu.RUnlock()

	var processes []*ProcessInfo
	for _, processInfo := range npm.processes {
		if processInfo.PluginID == pluginID {
			// Return copy
			info := *processInfo
			processes = append(processes, &info)
		}
...
func (npm *NativeProcessManager) GetProcessStats(pid int) (map[string]interface{}, error) {
	...
	stats := map[string]interface{}{
		"pid":        processInfo.PID,
		"plugin_id":  processInfo.PluginID,
