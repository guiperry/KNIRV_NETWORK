...
fn main() -> Result<()> {
    ...
    println!("Initial Total Supply: {}", nrn.get_total_supply());
    println!("Owner Balance: {}", nrn.get_balance(&nrn.get_owner()));

    let new_private_key = generate_private_key();
    println!("New Private Key: {}", new_private_key);
    let new_address = get_address_from_private_key(&new_private_key)?;
    println!("New Address: {}", new_address);

    println!("Minting 500 tokens to new address...");
    nrn.mint(&owner_private_key, new_address.clone(), &BigInt::from(500))?;
    println!("Total Supply after minting: {}", nrn.get_total_supply());
    println!("New Address Balance: {}", nrn.get_balance(&new_address));

    println!("Transferring 100 tokens from owner to new address...");
    nrn.transfer(&owner_private_key, new_address.clone(), &BigInt::from(100))?;
    println!(
        "Owner Balance after transfer: {}",
        nrn.get_balance(&nrn.get_owner())
    );
    println!(
        "New Address Balance after transfer: {}",
        nrn.get_balance(&new_address)
    );

    // Example of handling potential errors
    println!("Attempting an invalid transfer (negative amount)...");
    if let Err(e) = nrn.transfer(&owner_private_key, new_address.clone(), &BigInt::from(-100)) {
        println!("Transfer failed as expected: {}", e);
    }
