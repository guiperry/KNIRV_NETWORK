...
// getAppDataDir returns the OS-specific application data directory for KNIRV-SERVER
...
func (bm *BuntDBManager) createIndexes() error {
	return bm.db.Update(func(tx *buntdb.Tx) error {
		...
		// KNIRVGATEWAY indexes - TURN relay sessions
		// key pattern: gateway:turn:sessions:<session_id>
		// fields: peer_id, status (active|expired), udp_port, tcp_port, created_at, expires_at
		tx.CreateIndex("turn_sessions_by_status", "gateway:turn:sessions:*", buntdb.IndexJSON("status"))
		tx.CreateIndex("turn_sessions_by_peer", "gateway:turn:sessions:*", buntdb.IndexJSON("peer_id"))
		tx.CreateIndex("turn_sessions_by_created_at", "gateway:turn:sessions:*", buntdb.IndexJSON("created_at"))
		tx.CreateIndex("turn_sessions_by_expires_at", "gateway:turn:sessions:*", buntdb.IndexJSON("expires_at"))

		// KNIRVGATEWAY indexes - tunnel registrations
		// key pattern: gateway:tunnels:<tunnel_id>
		// fields: peer_id, status (registered|connected|disconnected), created_at, public_host
		tx.CreateIndex("tunnels_by_status", "gateway:tunnels:*", buntdb.IndexJSON("status"))
		tx.CreateIndex("tunnels_by_peer", "gateway:tunnels:*", buntdb.IndexJSON("peer_id"))
		tx.CreateIndex("tunnels_by_created_at", "gateway:tunnels:*", buntdb.IndexJSON("created_at"))

		// KNIRVGATEWAY indexes - API key records (shared auth between gateway and backend)
		// key pattern: gateway:apikeys:<key_id>
		// fields: user_id, is_active, scopes, created_at, expires_at, risk_score
		tx.CreateIndex("apikeys_by_user", "gateway:apikeys:*", buntdb.IndexJSON("user_id"))
		tx.CreateIndex("apikeys_by_active", "gateway:apikeys:*", buntdb.IndexJSON("is_active"))
		tx.CreateIndex("apikeys_by_expires_at", "gateway:apikeys:*", buntdb.IndexJSON("expires_at"))

		// KNIRVGATEWAY indexes - health snapshots (rolling history for dashboard)
		// key pattern: gateway:health:<timestamp>
		// fields: status (healthy|degraded|down), turn_running, timestamp
		tx.CreateIndex("gateway_health_by_timestamp", "gateway:health:*", buntdb.IndexJSON("timestamp"))
		tx.CreateIndex("gateway_health_by_status", "gateway:health:*", buntdb.IndexJSON("status"))
...
// SetJSON marshals and stores a JSON object
func (bm *BuntDBManager) SetJSON(key string, value interface{}) error {
	...
	if bm.knirvbase != nil && bm.knirvbase.enabled {
		err := bm.knirvbase.StoreObject(context.Background(), key, string(data))
		if err == nil {
			return nil
		}
	}
