...
type Index interface {
	Add(ctx context.Context, ref *BlockRef) error
...
func (mim *MultiIndexManager) AddBlock(ctx context.Context, block *blockchain.Block) error {
	...
	ref := &BlockRef{
		BlockID:   uuid.MustParse(string(block.BlockHash)),
	...
	for _, index := range mim.indexes {
		...
		go func(idx Index) {
			defer wg.Done()
			if err := idx.Add(ctx, ref); err != nil {
				errChan <- err
			}
		}(index)
	}
...
func (si *SemanticIndex) Add(ctx context.Context, ref *BlockRef) error {
	si.mu.Lock()
	defer si.mu.Unlock()

	if len(ref.Vector) > 0 {
		si.vectors[ref.BlockID] = ref.Vector
		return si.hnsw.Add(ref.BlockID, ref.Vector)
	}
	return nil
}
...
func (ti *TemporalIndex) Add(ctx context.Context, ref *BlockRef) error {
	ti.mu.Lock()
	defer ti.mu.Unlock()

	ti.timeline[ref.Timestamp] = append(ti.timeline[ref.Timestamp], ref.BlockID)
	return nil
}
...
func (ci *CategoryIndex) Add(ctx context.Context, ref *BlockRef) error {
	ci.mu.Lock()
	defer ci.mu.Unlock()

	if ref.Category != "" {
		ci.categories[ref.Category] = append(ci.categories[ref.Category], ref.BlockID)
	}
	return nil
}
