...
//go:embed bin/validationchain
var embeddedBinary []byte

func defaultBinaryDir() (string, error) {
	if override := strings.TrimSpace(os.Getenv("KNIRV_VALIDATION_CHAIN_BINARY_DIR")); override != "" {
		return override, nil
	}
...
// ExtractEmbeddedBinary extracts the embedded validation chain binary to the
// given destination directory (or the default system path) and returns the
// full path to the extracted binary. The binary is embedded gzip-compressed
// by the Makefile validation-chain-build target.
func ExtractEmbeddedBinary(destDir string) (string, error) {
	if len(embeddedBinary) == 0 {
		return "", fmt.Errorf("embedded validationchain binary is empty — run the Makefile validation-chain-build target first")
	}
	...
	destPath := filepath.Join(destDir, "validationchain")
