...
import (
	...
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/cleaner"
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/encoder"
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/grpc"
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/normalizer"
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/proto/v1"
	"github.com/knirvhasher/pipeline/0_DATA_CONNECTOR/internal/writer"
	"gopkg.in/yaml.v3"
)
...
func runStreamMode(ctx context.Context, client *grpc.Client, cfg *Config, norm *normalizer.SecurityNormalizer, arrowEnc *encoder.ArrowEncoder, jsonEnc *encoder.JSONEncoder, clean *cleaner.DataCleaner, writer *writer.FileWriter) {
	...
	if len(cfg.Source.DataTypes) > 0 && cfg.Source.DataTypes[0] != "ALL" {
		dataType = v1.DataType(v1.DataType_value[cfg.Source.DataTypes[0]])
	}
