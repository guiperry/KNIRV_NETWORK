...
func defaultBinaryDir() (string, error) {
	if override := strings.TrimSpace(os.Getenv("KNIRV_ORACLE_BINARY_DIR")); override != "" {
		return override, nil
	}
...
func DefaultEmbeddedBinaryPath() (string, error) {
	destDir, err := defaultBinaryDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(destDir, "knirvoracle"), nil
}
