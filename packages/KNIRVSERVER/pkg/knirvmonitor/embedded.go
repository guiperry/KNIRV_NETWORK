...
//go:embed bin/monitor
var embeddedBinary []byte

func defaultBinaryDir() (string, error) {
	if override := strings.TrimSpace(os.Getenv("KNIRV_MONITOR_BINARY_DIR")); override != "" {
		return override, nil
	}
...
func ExtractEmbeddedBinary(destDir string) (string, error) {
	if len(embeddedBinary) == 0 {
		return "", fmt.Errorf("embedded monitor binary is empty")
	}
	...
	destPath := filepath.Join(destDir, "monitor")
