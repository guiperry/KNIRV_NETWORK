...
export const useValidationSession = () => {
  ...
  // Create validation session for a creation
  const createSession = useCallback(async (creationId: string): Promise<ValidationSession | null> => {
    setIsLoading(true);
    setError(null);

    try {
      const url = `${API_BASE_URL}/api/dve-creation/nodes/${creationId}/validation-session`;
  ...
  const getSession = useCallback(async (creationId: string): Promise<ValidationSession | null> => {
    setIsLoading(true);
    setError(null);
