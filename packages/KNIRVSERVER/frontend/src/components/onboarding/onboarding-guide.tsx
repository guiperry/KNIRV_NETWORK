...
const dataCards: ConfigCard[] = [
  ...
  { id: 'cloud-pricing', icon: Cloud, label: 'Cloud Plan', desc: 'Select your cloud hosting tier for this Data Fabric', modalId: 'cloud-pricing' }
];
...
const OnboardingGuide = ({ onComplete, onReset }: OnboardingGuideProps) => {
    ...
    try {
      ...
      const orgPayload = {
        ...
        metadata: {
          ...
          cloud_plan: formData.cloudPlan || null,
        },
        value_system: {
          guidelines,
  ...
  const handleSavePreferences = (privacySettings: PrivacySettings) => {
    ...
    fetch(`${API_BASE_URL}/api/auth/preferences`, {
      method: 'PUT',
      headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({ onboarding_data: { privacy_settings: privacySettings } }),
    }).catch((err) => console.error('Failed to persist preferences:', err));
  };
  ...
  const handleSelectPlan = (planName: string) => {
    setFormData(prev => ({
      ...prev,
      cloudPlan: planName,
      selectedInputs: prev.selectedInputs.includes('cloud-pricing')
        ? prev.selectedInputs
        : [...prev.selectedInputs, 'cloud-pricing'],
      completedConnections: prev.completedConnections.includes('cloud-pricing')
        ? prev.completedConnections
        : [...prev.completedConnections, 'cloud-pricing']
    }));
    // Persist selected cloud plan to backend
    fetch(`${API_BASE_URL}/api/auth/preferences`, {
      method: 'PUT',
      headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({ onboarding_data: { cloud_plan: planName } }),
    }).catch((err) => console.error('Failed to persist cloud plan:', err));
    closeModal();
  };
  ...
  return (
    <div className="bg-[#0a0a0c] text-slate-200 font-sans selection:bg-blue-500/30 relative">
      ...
      />
      <CloudPricingModal
        isOpen={activeModal === 'cloud-pricing'}
        onClose={closeModal}
