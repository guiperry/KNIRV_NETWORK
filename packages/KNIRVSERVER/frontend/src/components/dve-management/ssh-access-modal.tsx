...
import { X, Terminal, Download, Copy, CheckCircle, AlertCircle, Monitor } from 'lucide-react';
...
export const SSHAccessModal: React.FC<SSHAccessModalProps> = ({
  ...
  const loadOrCreateSession = async () => {
    ...
    try {
      ...
      let existingSession = await sshSession.getSession(creationId);

      // If no existing session, create a new one
      if (!existingSession) {
        existingSession = await sshSession.createSession(creationId);
      }
  ...
  const handleTerminateSession = async () => {
    ...
    try {
      const success = await sshSession.terminateSession(creationId);
      if (success) {
        toast({
