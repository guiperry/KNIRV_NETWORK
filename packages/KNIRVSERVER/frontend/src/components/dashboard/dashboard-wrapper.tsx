...
function DashboardWrapperInner({ children, onRentDVE }: DashboardWrapperProps) {
  ...
  const handleSecurityReports = () => {
    generateReport('security-report', {
      ...
      kernelSecurity: securitySubsystem,
      networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics }
    });
  };
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex space-x-2">
                          ...
                          <Button variant="outline" size="sm" className="border-gray-700 text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-400 hover:border-indigo-400" onClick={() => handleShareReport('full-export', { type: 'Full Reports Export', generatedAt: new Date().toISOString(), account: { username: user?.user, role: user?.role }, teeSecurity: teeSecurityStatus, kernelSecurity: securitySubsystem, networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics } })}>
                        ...
                        <Card className="aether-bevel-dark-light rounded-xl">
                          ...
                          <CardContent className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <Button variant="outline" size="sm" className="flex-1 border-gray-700 text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-400 hover:border-indigo-400" onClick={() => handleShareReport('monthly-usage', { type: 'Monthly Usage Report', account: { username: user?.user, role: user?.role }, usage: { computeHours: 247.3, storageUsed: '1.2TB', apiCalls: 12400, nrnSpent: 1847 }, kernelSecurity: securitySubsystem, networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics } })}>
                        ...
                        <Card className="aether-bevel-dark-light rounded-xl">
                          ...
                          <CardContent className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <Button variant="outline" size="sm" className="flex-1 border-gray-700 text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-400 hover:border-indigo-400" onClick={() => handleShareReport('billing-statement', { type: 'Billing Statement', account: { username: user?.user, role: user?.role }, billing: { totalNRNSpent: 1847 }, kernelSecurity: securitySubsystem, networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics } })}>
                                <Share2 className="w-3 h-3 mr-1" />
                                Share
                              </Button>
                            </div>
                        ...
                        <Card className="aether-bevel-dark-light rounded-xl">
                          ...
                          <CardContent className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <Button variant="outline" size="sm" className="flex-1 border-gray-700 text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-400 hover:border-indigo-400" onClick={() => handleShareReport('performance-analytics', { type: 'Performance Analytics', summary: { completedTasks: 1847, successRate: 98.2, activeTasks: 127 }, kernelSecurity: securitySubsystem, networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics } })}>
                                <Share2 className="w-3 h-3 mr-1" />
                                Share
                              </Button>
                            </div>
                        ...
                        <Card className="aether-bevel-dark-light rounded-xl">
                          ...
                          <CardContent className="space-y-2">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <Button variant="outline" size="sm" className="flex-1 border-gray-700 text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-400 hover:border-indigo-400" onClick={() => handleShareReport('resource-utilization', { type: 'Resource Utilization', resources: { computeHours: 247.3, storageUsed: '1.2TB', apiCalls: 12400 }, teeMetrics: teeSecurityStatus?.performance_metrics ?? null, kernelSecurity: securitySubsystem, networkMonitor: { status: networkMonitor.status, metrics: networkMonitor.metrics } })}>
                                <Share2 className="w-3 h-3 mr-1" />
                                Share
                              </Button>
                            </div>
