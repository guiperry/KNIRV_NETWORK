...
import {
  ...
  Settings2, Info, Award, Zap, CheckCircle2, Save, Library, Copy, Trash2
} from 'lucide-react';
...
export const BadgeLabPanel: React.FC<BadgeLabPanelProps> = ({ className }) => {
  ...
  const { templates, listTemplates, createTemplate, deleteTemplate, isLoading: templatesLoading } = useBadgeTemplates();
  ...
  const handleSaveAsTemplate = async () => {
    ...
    try {
      const result = await createTemplate({
        ...
        alignment_threshold: alignmentThreshold,
        auth_credentials: {
          api_key_provider: apiKeyProvider || undefined,
          api_key_scope: apiKeyScope || undefined,
          jwt_claim: jwtClaim || undefined,
          jwt_value: jwtValue || undefined,
  ...
  const handleLoadTemplate = (tmpl: BadgeTemplate) => {
    setPrompt(tmpl.name);
    setSelectedValues(tmpl.value_signals || []);
    setSelectedOntology(tmpl.ontology_signals || []);
    setAiTextTag(tmpl.ai_text_tag || '');
    setAlignmentThreshold(tmpl.alignment_threshold || 0.75);
    setApiKeyProvider(tmpl.auth_credentials?.api_key_provider || '');
    setApiKeyScope(tmpl.auth_credentials?.api_key_scope || 'inference');
    setJwtClaim(tmpl.auth_credentials?.jwt_claim || '');
    setJwtValue(tmpl.auth_credentials?.jwt_value || '');
    setSelectedRole(tmpl.auth_credentials?.selected_role || '');
  ...
  const resetFull = () => {
    setOutputUrl(null); setSvgContent(null); setPrompt('');
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              {/* Mode toggle */}
              <div className="ml-4 flex items-center space-x-1 bg-gray-900 rounded-lg p-0.5 border border-gray-800">
                <button
                  onClick={() => setShowTemplateLibrary(false)}
                  className={`px-3 py-1.5 rounded-md text-[10px] font-bold transition-interactive ${
                    !showTemplateLibrary
                ...
                >
                  <Library size={12} className="inline mr-1" />
          ...
          {showTemplateLibrary ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-gray-300">Saved Badge Templates</h3>
                <button
                  onClick={() => { listTemplates(true); }}
              ...
              {templatesLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <Award size={40} className="text-gray-800 mb-3" />
                  <p className="text-xs text-gray-600 font-medium">No badge templates saved yet</p>
                  <p className="text-[10px] text-gray-700 mt-1">Design a badge in the Badge Studio and save it as a template</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {templates.map((tmpl) => (
                    <div
                      key={tmpl.id}
                      className="rounded-xl border border-gray-800 bg-gray-950/40 p-4 hover:border-amber-500/30 transition-interactive"
                    >
                      {/* SVG Preview */}
                      {tmpl.svg_design && (
                        <div className="mb-3 bg-[#02040a] rounded-lg overflow-hidden h-32 flex items-center justify-center">
                          <div
                      ...
                      <h4 className="text-xs font-bold text-gray-200 truncate">{tmpl.name}</h4>
                      <div className="flex flex-wrap gap-1 mt-1.5 mb-2">
                        <Badge variant="outline" className="text-[8px] text-amber-400 border-amber-400/20">
                          {tmpl.badge_type || 'capability'}
                        </Badge>
                        <Badge variant="outline" className="text-[8px] text-blue-400 border-blue-400/20">
                          {tmpl.value_signals?.length || 0} values
                        </Badge>
                        <Badge variant="outline" className="text-[8px] text-green-400 border-green-400/20">
                          {tmpl.ontology_signals?.length || 0} ontology
                        </Badge>
                      </div>
                      {tmpl.description && (
                        <p className="text-[9px] text-gray-600 line-clamp-2 mb-3">{tmpl.description}</p>
                      )}
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleLoadTemplate(tmpl)}
                          className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 bg-amber-600/20 hover:bg-amber-600/30 text-amber-400 rounded-md text-[9px] font-bold transition-interactive"
                        >
                          <Copy size={10} /> Load & Edit
                        </button>
                        <button
                          onClick={async () => {
                            await deleteTemplate(tmpl.id);
                          }}
                          className="flex items-center justify-center gap-1 px-2 py-1.5 bg-red-900/20 hover:bg-red-900/30 text-red-400 rounded-md text-[9px] font-bold transition-interactive"
                        >
                ...
                <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4 space-y-4">
                  ...
                  <div className="flex items-start gap-2 text-[9px] text-gray-600 font-medium bg-gray-900/40 p-2 rounded-lg border border-gray-800/50">
                    ...
                    <p>
                      ...
                      Save as template to reuse across DVE Workspace and WebGUI.
                    </p>
                  </div>
                </div>
              </div>
                ...
                {outputUrl && (
                  <div className="p-4 bg-gray-950/60 border-t border-gray-800/50 flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      ...
                      {svgContent && !mintedBadgeId && (
                        <button
                          onClick={handleSaveAsTemplate}
                          disabled={savingAsTemplate || templateSaved}
                          className={`flex items-center gap-1.5 px-5 py-2 rounded-lg text-[10px] font-black uppercase transition-interactive ${
                            templateSaved
