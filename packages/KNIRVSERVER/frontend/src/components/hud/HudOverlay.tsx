    ...
    <>
      ...
      <div className={`${styles.panel} ${styles.topPanel} ${className}`}>
        ...
        <div className={`${styles.panelSection} ${styles.desktopOnly}`}>
          ...
          <span className={`${styles.value} ${securitySubsystem.backend === 'unavailable' ? styles.errorVal : securitySubsystem.backend === 'native' ? styles.connecting : styles.active}`}>
            {getSecurityBackendLabel(securitySubsystem)}
          </span>
        </div>
        <div className={`${styles.panelSection} ${styles.desktopOnly}`}>
      ...
      <div className={`${styles.panel} ${styles.leftPanel}`}>
        ...
        <div className={styles.panelHeader} style={{ marginTop: '16px' }}>KERNEL SECURITY</div>
        <div className={styles.statItem}><span>BACKEND: {getSecurityBackendLabel(securitySubsystem)}</span></div>
        <div className={styles.statItem}><span>MODE: {String(securitySubsystem.mode ?? 'auto').toUpperCase()}</span></div>
        <div className={styles.statItem}><span>CONTAINERS: {securitySubsystem.isolator?.active_containers ?? 0}</span></div>
        <div className={styles.statItem}><span>DROPPED: {securitySubsystem.rate_limiter?.dropped_packets ?? 0}</span></div>
      </div>
      ...
      <div className={`${styles.panel} ${styles.rightPanel}`}>
        ...
        {securitySubsystem.backend === 'native' && (
          <div className={styles.notificationItem}>
            <div className={`${styles.notifDot} ${styles.connecting}`} />
            <span>Native security fallback active</span>
          </div>
        )}
        {securitySubsystem.guardian?.skipped && (
          <div className={styles.notificationItem}>
            <div className={`${styles.notifDot} ${styles.connecting}`} />
            <span>Guardian skipped: {securitySubsystem.guardian.reason}</span>
          </div>
        )}
        {metrics && cpuPct > 80 && (
          <div className={styles.notificationItem}>
      ...
      <div className={`${styles.panel} ${styles.mobileMetrics}`}>
        ...
        <div className={styles.mobileStat}>
          ...
          <span className={styles.mobileMetricVal}>{getSecurityBackendLabel(securitySubsystem)}</span>
        </div>
      </div>
