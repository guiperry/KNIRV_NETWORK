...
// DVE Creation types - Aligned with backend models
...
export interface DVESession {
  id: string;
  dve_creation_id: string;
  dve_node_id: string;
  owner_id: string;
  session_key: string; // base64 encoded bytes
  session_key_id: string;
  tee_binding: string;
  chain_session_id: string;
  ...
  pqc_signature: string; // base64 encoded bytes
}

export interface DVECreationRequest {
  owner_id: string;
  owner_address: string;
  name: string;
  tee_type: string;
...
export interface DVECreationResponse {
  success: boolean;
  dve_creation?: DVECreation;
  session?: DVESession;
  registration_tx_hash?: string;
...
export interface DVECreationStats {
  total_creations: number;
  active_creations: number;
  total_stake: number;
  average_stake: number;
  popular_capabilities: Array<{
    capability: string;
    count: number;
  }>;
}
