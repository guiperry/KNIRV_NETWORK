...
resolve_workspace_root() {
    ...
    if [[ -d "/home/gperry/Documents/GitHub/KNIRV/KNIRV_CORP/packages/server/backend_server/" ]]; then
        printf '%s\n' "/home/gperry/Documents/GitHub/KNIRV/KNIRV_CORP/packages/server/backend_server/"
        return
    fi
...
test_cli_flags() {
    ...
    # Test version flag
    if go run cmd/backend_server/main.go --version 2>&1 | grep -q "KNIRV-SERVER"; then
...
test_role_based_access() {
    ...
    # Test admin access
    if curl -H "Authorization: Bearer TESTNET_ADMIN_TOKEN" -f http://localhost:8080/api/admin/status >/dev/null 2>&1; then
    ...
    if curl -H "Authorization: Bearer TESTNET_VALIDATOR_TOKEN" -f http://localhost:8080/api/validation/tasks >/dev/null 2>&1; then
    ...
    if curl -H "Authorization: Bearer TESTNET_OBSERVER_TOKEN" -f http://localhost:8080/api/system/status >/dev/null 2>&1; then
