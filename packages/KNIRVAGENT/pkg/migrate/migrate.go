...
func Run(opts Options) (*Result, error) {
	...
	knirvAgentHome, err := resolveKnirvAgentHome(opts.KnirvAgentHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, knirvAgentHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to KnirvAgent")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, knirvAgentHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, knirvAgentHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(knirvAgentHome, "config.json"),
				Description: "convert OpenClaw config to KnirvAgent format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(knirvAgentHome)
...
func Execute(actions []Action, openclawHome, knirvAgentHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, knirvAgentHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, knirvAgentHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing KnirvAgent config: %w", err)
		}
...
func resolveKnirvAgentHome(override string) (string, error) {
	if override != "" {
		return override, nil
	}
	...
	return filepath.Join(home, ".knirvagent"), nil
}
