...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'deepseek' not supported in KnirvAgent, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in KnirvAgent, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.knirvagent/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.knirvagent/workspace")
		}
	})
	...
	t.Run("empty config", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Model != "gemini-2.0-flash" {
			t.Errorf("default model should be gemini-2.0-flash, got %q", cfg.Agents.Defaults.Model)
		}
	})
}
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.knirvagent/workspace"},
