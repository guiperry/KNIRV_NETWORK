...
import (
	...
	"github.com/knirvcorp/knirvagent/pkg/constants"
...
func (m *Manager) initChannels() error {
	...
	if m.config.Channels.KNIRVWallet.Enabled {
		logger.DebugC("channels", "Attempting to initialize KNIRVWallet channel")
		knirvwallet, err := NewKNIRVWalletChannel(m.config.Channels.KNIRVWallet, m.bus)
		if err != nil {
			logger.ErrorCF("channels", "Failed to initialize KNIRVWallet channel", map[string]interface{}{
				"error": err.Error(),
			})
		} else {
			m.channels["knirvwallet"] = knirvwallet
			logger.InfoC("channels", "KNIRVWallet channel enabled successfully")
		}
	}
