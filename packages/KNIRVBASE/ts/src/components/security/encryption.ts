...
export class AESEncryption {
  ...
  static encrypt(data: Buffer, key: Buffer): EncryptedData {
    ...
    try {
      ...
      const cipher = createCipheriv(AESEncryption.ALGORITHM, key, nonce) as any as GCMCipher;
  ...
  static decrypt(encryptedData: EncryptedData, key: Buffer): Buffer {
    ...
    try {
      ...
      const decipher = createDecipheriv(AESEncryption.ALGORITHM, key, Buffer.from(encryptedData.nonce, 'base64')) as any as GCMDecipher;
