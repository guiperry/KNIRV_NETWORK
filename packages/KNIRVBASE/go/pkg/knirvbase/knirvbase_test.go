...
func TestOptionsConsensusField(t *testing.T) {
	tmpDir := t.TempDir()
	opts := Options{
		DataDir: tmpDir,
	...
	if opts.Consensus.NetworkID != "test-net" {
		t.Errorf("expected test-net, got %s", opts.Consensus.NetworkID)
	}
	ctx := context.Background()
	db, err := New(ctx, opts)
	if err != nil {
		t.Fatalf("New() with consensus opts failed: %v", err)
	}
	defer db.Shutdown()
}

func TestDefaultOptionsIncludeConsensus(t *testing.T) {
	opts := DefaultOptions()
	if opts.Consensus.Enabled != true {
		t.Error("expected default consensus to be enabled")
	}
	if opts.Consensus.NetworkID != "knirvbase-default" {
		t.Errorf("expected knirvbase-default, got %s", opts.Consensus.NetworkID)
	}
}

func TestOptionsBackwardCompat(t *testing.T) {
	tmpDir := t.TempDir()
	opts := Options{
		DataDir:              tmpDir,
		DistributedEnabled:   true,
		DistributedNetworkID: "legacy-net",
		DistributedBootstrapPeers: []string{"/ip4/1.2.3.4/tcp/4001"},
	}
	ctx := context.Background()
	db, err := New(ctx, opts)
	if err != nil {
		t.Fatalf("New() with legacy opts failed: %v", err)
	}
	defer db.Shutdown()
}
