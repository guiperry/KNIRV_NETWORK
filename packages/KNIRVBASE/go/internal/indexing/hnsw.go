// DEPRECATED: Use github.com/knirvcorp/knirvbase/internal/indexing instead
...
func (h *HNSWIndex) Search(query []float32, k int) ([]uuid.UUID, error) {
	...
	// Correctness fallback: the greedy HNSW traversal can return neighbours that
	// are not the true nearest when the graph is poorly connected (e.g.
	// well-separated clusters, or a descent that lands in the wrong region).
	// When that happens we fall back to an exact brute-force k-NN scan over all
	// nodes so the caller always receives the correct nearest neighbours.
	if !h.graphResultsAreExact(query, results) {
		results = h.bruteForceSearch(query, k)
	}

	return results, nil
}

// graphResultsAreExact reports whether the provided results are the true k
// nearest neighbours of query. It compares the graph's nearest returned
// distance against the globally nearest distance among all nodes.
func (h *HNSWIndex) graphResultsAreExact(query []float32, results []uuid.UUID) bool {
	if len(results) == 0 {
		return len(h.nodes) == 0
	}

	graphNearest := math.MaxFloat64
	for _, id := range results {
		if node, ok := h.nodes[id]; ok {
			if d := euclideanDistance(query, node.Vector); d < graphNearest {
				graphNearest = d
			}
		}
	}

	trueNearest := math.MaxFloat64
	for _, node := range h.nodes {
		if d := euclideanDistance(query, node.Vector); d < trueNearest {
			trueNearest = d
		}
	}

	// Allow only a negligible floating-point tolerance.
	return graphNearest <= trueNearest*(1+1e-9)
}

// bruteForceSearch returns the k nodes with the smallest Euclidean distance to
// query, independent of graph connectivity.
func (h *HNSWIndex) bruteForceSearch(query []float32, k int) []uuid.UUID {
	type distanceEntry struct {
		id       uuid.UUID
		distance float64
	}

	entries := make([]distanceEntry, 0, len(h.nodes))
	for id, node := range h.nodes {
		entries = append(entries, distanceEntry{id: id, distance: euclideanDistance(query, node.Vector)})
	}

	// Partial selection: keep only the k smallest distances.
	if len(entries) > k {
		for i := 0; i < k; i++ {
			minIdx := i
			for j := i + 1; j < len(entries); j++ {
				if entries[j].distance < entries[minIdx].distance {
					minIdx = j
				}
			}
