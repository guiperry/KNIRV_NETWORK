...
impl Network for NetworkManager {
    ...
    async fn broadcast_message(&self, _network_id: &str, msg: ProtocolMessage) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
async fn handle_connection(
    _stream: TcpStream,
    _networks: Arc<RwLock<HashMap<String, NetworkConfig>>>,
    _peers: Arc<RwLock<HashMap<String, PeerInfo>>>,
    _connections: Arc<RwLock<HashMap<String, Arc<Mutex<TcpStream>>>>>,
    _handlers: Arc<RwLock<HashMap<MessageType, Vec<MessageHandler>>>>,
    _peer_id: String,
