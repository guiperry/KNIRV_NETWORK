# KNIRVORACLE - Embedded Services Implementation

## Overview

The KNIRVORACLE has been fully refactored to **embed all Node.js services and the network-website directly into the Go binary**. Upon startup, the binary extracts these embedded files to a temporary runtime directory, installs dependencies, and serves them.
...
### Services (487MB)
- **payment-oracle** (224MB) - Stripe/Coinbase payment processing, testnet faucet
...
```
User runs: ./bin/knirvoracle
    ↓
Runtime Manager initializes
    ↓
Extracts embedded files to: /tmp/knirvoracle-runtime/
    ├── services/
    │   ├── payment-oracle/
    │   ├── tunnel-registry/
    │   ├── webgui/
...
Stop all Node.js services (SIGTERM → SIGKILL)
    ↓
Cleanup: rm -rf /tmp/knirvoracle-runtime/
    ↓
Exit
```
...
KNIRVORACLE/
├── bin/
│   └── knirvoracle               # 580MB binary with ALL embedded files
├── cmd/oracle/main.go            # Entry point with runtime extraction
├── internal/
│   ├── embedded/
...
```go
type Runtime struct {
    BaseDir           string  // /tmp/knirvoracle-runtime
    ServicesDir       string  // /tmp/knirvoracle-runtime/services
    NetworkWebsiteDir string  // /tmp/knirvoracle-runtime/network-website
}
...
- Check if /tmp/knirvoracle-runtime exists
...
WORKDIR /app
COPY . .
RUN go build -o bin/knirvoracle ./cmd/oracle

FROM node:20-alpine
RUN apk add --no-cache ca-certificates
COPY --from=builder /app/bin/knirvoracle /usr/local/bin/
EXPOSE 8080
CMD ["knirvoracle"]
```
...
```bash
# Check binary size
ls -lh bin/knirvoracle

# Run with verbose logging
LOGLEVEL=debug ./bin/knirvoracle
```
...
ls -la /tmp/knirvoracle-runtime/
ls -la /tmp/knirvoracle-runtime/services/
ls -la /tmp/knirvoracle-runtime/network-website/public/
...
ls -la /tmp/knirvoracle-runtime/services/payment-oracle/
...
```bash
TMPDIR=/var/tmp ./bin/knirvoracle
```
...
The embedded services implementation provides a **self-contained, portable, single-binary deployment** of KNIRVORACLE. While the binary is larger (580MB), it eliminates the complexity of managing separate service directories and ensures consistency across deployments.
