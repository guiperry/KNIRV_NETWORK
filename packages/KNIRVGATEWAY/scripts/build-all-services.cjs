 ...
 * 
 * Builds all oracle services in the correct order and handles dependencies.
 * Integrates with the oracle links system for automatic configuration.
 */
...
class ServiceBuilder {
  constructor() {
    this.oracleDir = path.join(__dirname, '..');
    this.services = [
      {
        name: 'payment-oracle',
        path: 'services/payment-oracle',
        buildRequired: false,
  ...
  runCommand(command, args, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      const process = spawn(command, args, {
        stdio: 'inherit',
        cwd: options.cwd || this.oracleDir,
        ...options
      });
  ...
  async installService(service) {
    const servicePath = path.join(this.oracleDir, service.path);
  ...
  generateSummary() {
    ...
    const summaryPath = path.join(this.oracleDir, 'logs', 'build-summary.json');
    try {
