...
// StreamProcessor handles processing events (simplified for KNIRVGRAPH)
...
// Start starts the stream processor (simplified for KNIRVGRAPH)
func (p *StreamProcessor) Start() error {
	p.ctx, p.cancel = context.WithCancel(context.Background())
	p.isRunning = true
	log.Printf("KNIRVGRAPH StreamProcessor started")
	return nil
}
...
func (p *StreamProcessor) Stop() error {
	if p.cancel != nil {
		p.cancel()
		p.isRunning = false
		log.Printf("KNIRVGRAPH StreamProcessor stopped")
	}
...
// ProcessEvent processes an event directly (simplified for KNIRVGRAPH)
...
func (p *StreamProcessor) processEvent(event Event) {
	...
	for _, part := range parts {
		if string(event.Type) != string(part) && len(event.Type) > len(part) {
