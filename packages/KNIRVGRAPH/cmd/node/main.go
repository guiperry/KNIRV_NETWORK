...
func main() {
	var (
		...
		testnetMode = flag.Bool("testnet", false, "Run in testnet mode (default when -testnet is omitted and KNIRV_NETWORK_MODE is unset or \"testnet\")")
	...
	// process's manager and defaults to "testnet" unless -prod/-dev was
	// used there, so anything other than "production"/"development" here
	// still means testnet.
	if !isFlagSet("testnet") {
		...
		case "production", "development", "":
			// "production"/"development": leave *testnetMode false.
			// "": no signal from the parent process (standalone run) —
			// keep the flag's own default.
		}
	}
