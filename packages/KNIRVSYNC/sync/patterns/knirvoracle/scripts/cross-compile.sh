...
# Use BINARY_NAME from environment variable if set, otherwise default to KNIRVORACLE
BINARY_NAME=${BINARY_NAME:-KNIRVORACLE}
...
for platform in $PLATFORMS; do
  ...
  if [ "$os" = "windows" ] && [ "$NO_WEBVIEW" != "1" ]; then
    ...
    if command -v $CC_FOR_TARGET &> /dev/null && command -v $CXX_FOR_TARGET &> /dev/null; then
      ...
      CC=$CC_FOR_TARGET CXX=$CXX_FOR_TARGET GOOS=$os GOARCH=$arch CGO_ENABLED=0 \
      ...
      if [ -n "$env_prefix" ]; then
        ...
        eval "$env_prefix GOOS=$os GOARCH=$arch CGO_ENABLED=0 go build -v -tags=\"$build_tags\" -ldflags=\"-s -w -X main.AppVersion=$VERSION -X main.BuildDate=$BUILD_DATE\" -o \"$output_dir/$binary_name\" $MAIN_PACKAGE_PATH" || \
        (echo "Fyne GUI build failed, falling back to default GUI..." && \
         GOOS=$os GOARCH=$arch CGO_ENABLED=0 go build -v -ldflags="-s -w -X main.AppVersion=$VERSION -X main.BuildDate=$BUILD_DATE" -o "$output_dir/$binary_name" $MAIN_PACKAGE_PATH)
      else
