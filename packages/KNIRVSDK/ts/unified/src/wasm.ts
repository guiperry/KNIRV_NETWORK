...
function varint(value: WasmUint64): Uint8Array {
...
function uintField(field: number, value?: WasmUint64): Uint8Array {
  if (value === undefined || BigInt(value) === 0n) return new Uint8Array();
  return concat(tag(field, 0), varint(value));
}

function normalizeWasmPublicationPayload(p: WasmPublicationPayload): WasmPublicationPayload {
...
export function marshalWasmPublicationPayload(payload: WasmPublicationPayload): Uint8Array {
  const p = normalizeWasmPublicationPayload(payload);
  return concat(
    stringField(1, p.schemaVersion),
    stringField(2, p.networkId),
    stringField(3, p.networkFingerprint),
    stringField(4, p.artifactDigest),
    uintField(5, p.byteSize),
    stringField(6, p.moduleKind),
    uintField(7, p.abiVersion),
    uintField(8, p.moduleSchemaVersion),
    stringField(9, p.buildId),
    stringField(10, p.toolchainDigest),
    stringField(11, p.selfTestDigest),
    stringField(12, p.publisherAddress),
    stringField(13, p.dveTemplateId),
  );
}

function marshalWasmManifestModule(m: WasmManifestModule): Uint8Array {
  return concat(
    stringField(1, m.moduleKind),
    stringField(2, m.artifactDigest),
    uintField(3, m.byteSize),
    uintField(4, m.abiVersion),
    uintField(5, m.moduleSchemaVersion),
    stringField(6, m.capabilitiesJson),
    stringField(7, m.configurationDigest),
    stringField(8, m.downloadPath),
    stringField(9, m.publisherAddress),
    stringField(10, m.publicationStatementDigest),
  );
}

function normalizeWasmManifestPayload(p: WasmManifestPayload): WasmManifestPayload {
...
export function marshalWasmManifestPayload(payload: WasmManifestPayload): Uint8Array {
  const p = normalizeWasmManifestPayload(payload);
  return concat(
    stringField(1, p.schemaVersion),
    stringField(2, p.manifestId),
    stringField(3, p.networkId),
    stringField(4, p.chainId),
    stringField(5, p.networkFingerprint),
    stringField(6, p.userSubject),
    stringField(7, p.deviceId),
    stringField(8, p.dveId),
    uintField(9, p.leaseEpoch),
    ...p.modules.map((m) => bytesField(10, marshalWasmManifestModule(m))),
    stringField(11, p.relayTargetType),
    stringField(12, p.relayTargetId),
    stringField(13, p.assignmentId),
    uintField(14, p.assignmentVersion),
    stringField(15, p.supersedesAssignmentId),
  );
}
...
function stringOf(field?: WireField): string {
  return field?.bytesValue ? decoder.decode(field.bytesValue) : '';
}

function uintOf(field?: WireField): bigint {
  return field?.uintValue ?? 0n;
}

export function parseWasmPublicationPayload(data: Uint8Array): WasmPublicationPayload {
...
function parseWasmManifestModule(data: Uint8Array): WasmManifestModule {
  const fields = readFields(data);
  const byNumber = new Map<number, WireField>();
  for (const field of fields) if (!byNumber.has(field.number)) byNumber.set(field.number, field);
...
export function parseWasmManifestPayload(data: Uint8Array): WasmManifestPayload {
  const fields = readFields(data);
  const byNumber = new Map<number, WireField>();
  const modules: WasmManifestModule[] = [];
  for (const field of fields) {
    if (field.number === 10 && field.wireType === 2 && field.bytesValue) {
      modules.push(parseWasmManifestModule(field.bytesValue));
      continue;
    }
    if (!byNumber.has(field.number)) byNumber.set(field.number, field);
  }
