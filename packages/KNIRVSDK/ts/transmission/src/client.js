...
import { parseKnirvURI } from './parser';
/**
...
export class ClientClosedError extends KnirvClientError {
    constructor(message = 'Client is closed') {
        super(message);
...
export class KnirvClient {
     ...
     */
    async findResourceProviders(id, resourceType) {
        if (!this.libp2p) {
     ...
     * @throws {FetchFailedError} If fetching the resource fails.
     */
    async fetchFromProvider(providerId, uri) {
        if (!this.libp2p) {
            throw new KnirvClientError('libp2p not initialized');
        }
