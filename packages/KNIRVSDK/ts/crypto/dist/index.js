...
async function encryptAES(data, password) {
    ...
    try {
        ...
        return crypto_js_1.default.enc.Base64.stringify(combined);
    }
    catch (error) {
...
async function decryptAES(encryptedData, password) {
    ...
    try {
        ...
        const combined = crypto_js_1.default.enc.Base64.parse(encryptedData);
        ...
        const encrypted = crypto_js_1.default.lib.WordArray.create(combined.words.slice(8));
        ...
        // Decrypt the data
        const decrypted = crypto_js_1.default.AES.decrypt(crypto_js_1.default.enc.Base64.stringify(encrypted), key, {
            iv: iv,
            padding: crypto_js_1.default.pad.Pkcs7,
            mode: crypto_js_1.default.mode.CBC
        });
 ...
 */
async function makeCryptKey(password, salt) {
    ...
    try {
        const saltWordArray = salt
            ? crypto_js_1.default.enc.Utf8.parse(salt)
            : crypto_js_1.default.lib.WordArray.random(128 / 8);
        const key = crypto_js_1.default.PBKDF2(password, saltWordArray, {
            keySize: 256 / 32,
            iterations: 10000
        });
        return key.toString(crypto_js_1.default.enc.Hex);
    }
    catch (error) {
...
async function sha256(data) {
    ...
    try {
        const hash = crypto_js_1.default.SHA256(data);
...
function generateSalt(length = 16) {
    const salt = crypto_js_1.default.lib.WordArray.random(length);
    return salt.toString(crypto_js_1.default.enc.Hex);
}
/**
...
function generateIV(length = 16) {
    const iv = crypto_js_1.default.lib.WordArray.random(length);
    return iv.toString(crypto_js_1.default.enc.Hex);
}
