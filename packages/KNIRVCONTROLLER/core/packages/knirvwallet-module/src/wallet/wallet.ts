...
// Import and re-export Tx and TxSignature from our messages utils
...
export class KnirvWallet implements Wallet {
  ...
  get defaultHDWalletKeyring(): HDWalletKeyring | null {
  ...
  async getPrivateKey() {
    return this.getPrivateKeyByAccount(this.currentAccount);
  }

  private async getPrivateKeyByAccount(account: Account) {
    const keyring = this._keyrings.find((keyring) => keyring.id === account.keyringId);
    if (!keyring || !hasPrivateKey(keyring)) {
      throw new Error('Current account does not have a private key');
    }
    if (isHDWalletKeyring(keyring)) {
      if (account instanceof SeedAccount) {
        return keyring.getPrivateKey(account.hdPath);
      }
