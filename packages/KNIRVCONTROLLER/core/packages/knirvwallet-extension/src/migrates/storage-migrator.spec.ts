...
import { encryptAES } from '../common/utils/crypto';
import { StorageMigrator } from './storage-migrator';

let mockStorageV001: StorageModelV001;

const createMockStorageV001 = (serialized: string): StorageModelV001 => ({
  version: 1,
  data: {
    NETWORKS: [],
    CURRENT_CHAIN_ID: '',
...
describe('StorageMigrator', () => {
  beforeAll(async () => {
    ...
    mockStorageV001 = createMockStorageV001(serialized);
    const mockValue = {
