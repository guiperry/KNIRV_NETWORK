...
const { Bip39, EnglishMnemonic } = require('@cosmjs/crypto');
...
export class StorageMigration009 implements Migration<StorageModelDataV009> {
  ...
  private async migrateSerialized(
    ...
    const keyrings = wallet.keyrings.map((keyring) => {
      if (keyring.type === 'HD_WALLET' && keyring.mnemonic) {
        return {
          id: keyring.id,
          type: keyring.type,
          seed: keyring.seed,
          mnemonicEntropy: Array.from(Bip39.decode(new EnglishMnemonic(keyring.mnemonic))),
        };
      }
    ...
    return encryptAES(JSON.stringify(migratedWallet), sha256Password);
  }
