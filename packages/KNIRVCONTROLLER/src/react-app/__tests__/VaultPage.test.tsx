...
describe('VaultPage', () => {
  ...
  it('renders "Manage your Vault and DVE vault assets" description', () => {
    renderWithRouter(<VaultPage />);
    expect(screen.getByText('Manage your Vault and DVE vault assets')).toBeInTheDocument();
  });
  ...
  it('displays the USD value', () => {
    renderWithRouter(<VaultPage />);
    expect(screen.getByText('≈ $4567.89 USD')).toBeInTheDocument();
  });

  it('renders the vault address section', () => {
    renderWithRouter(<VaultPage />);
  ...
  it('opens the send modal when click "Send NRN" button', () => {
    renderWithRouter(<VaultPage />);
    const sendButton = screen.getByRole('button', { name: 'Send NRN' });
    fireEvent.click(sendButton);
    expect(screen.getByRole('heading', { name: 'Send NRN' })).toBeInTheDocument();
