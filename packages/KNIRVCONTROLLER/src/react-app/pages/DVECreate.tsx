...
const renderConfirm = (state: DVECreateState) => {
  ...
  // Derived mock DVE vault address
  const derivedVault = `0x${Array.from({ length: 40 }, () =>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-slate-900/80 border border-slate-700 rounded-xl p-3.5 space-y-1.5">
        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">DVE Vault Address</label>
        <div className="flex items-center space-x-2">
          <div className="w-7 h-7 bg-blue-600/20 rounded-lg flex items-center justify-center">
            <Cpu className="w-3.5 h-3.5 text-blue-400" />
          </div>
          <span className="text-xs font-mono text-blue-300 font-bold truncate">{derivedVault}</span>
        </div>
