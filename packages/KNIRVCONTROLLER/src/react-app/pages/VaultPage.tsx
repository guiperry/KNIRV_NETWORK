...
import { Shield, ArrowUpRight, ArrowDownLeft, Zap, TrendingUp, Copy, ExternalLink, Lock, Loader, RefreshCw } from 'lucide-react';
      ...
      <div className="p-4 pb-24 space-y-6 relative">
        ...
        <div className="text-center py-4 relative">
          ...
          <p className="text-slate-400 text-sm font-mono uppercase tracking-wider">
            Manage your Vault and DVE vault assets
          </p>
          <button 
            onClick={lockVault}
        ...
        <div className="relative group">
          ...
          <div className="relative glass-panel p-6 glow-hover">
            <div className="flex items-center justify-between mb-4">
              ...
              {vaultData && (
                <div className={`px-3 py-1 rounded-full ${vaultData.change24h >= 0 ? 'bg-green-500/20 border border-green-500/30' : 'bg-red-500/20 border border-red-500/30'}`}>
                  <div className="flex items-center space-x-1">
                    <TrendingUp className={`w-3 h-3 ${vaultData.change24h >= 0 ? 'text-green-400' : 'text-red-400 rotate-180'}`} />
                    <span className={`text-xs font-medium font-mono ${vaultData.change24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {vaultData.change24h >= 0 ? '+' : ''}{vaultData.change24h}%
                    </span>
                  </div>
                </div>
              )}
              ...
              {isLoading ? (
                ...
                <>
                  <div className="text-3xl font-bold text-white">
                    {vaultData?.nrnBalance.toLocaleString() ?? '0'} NRN
                  </div>
                  <div className="text-lg text-slate-300 font-mono">
                    ≈ ${vaultData?.usdValue.toFixed(2) ?? '0.00'} USD
                  </div>
                </>
              )}
            </div>
