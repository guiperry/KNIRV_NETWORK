...
function formatValidationResponse(skillName: string): string {
  return (
    `✓ Validation task queued for skill "${skillName}".\n\n` +
...
export default function CognitiveEngineChat() {
  ...
  const simulateAgentResponse = useCallback((userInput: string) => {
    ...
    if (lower.includes('fleet') || lower.includes('dve status') || lower.includes('fleet status')) {
      ...
      const skillName = match ? match[1] : 'unknown-skill';
      responseContent = formatValidationResponse(skillName);
