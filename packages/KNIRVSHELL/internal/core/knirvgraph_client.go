...
func (c *KNIRVGraphClient) CreateIncidentNode(ctx context.Context, inc *incident.Incident, stagedPath, digest string) (string, error) {
	nodeID := fmt.Sprintf("incident-%s", inc.ID)
	errorNode := &ErrorNode{
		ID:          nodeID,
		ErrorType:   inc.ErrorSignature,
		Description: inc.TriggerLine,
		Severity:    1,
		Timestamp:   time.Now().UTC().Format(time.RFC3339),
		Context: map[string]interface{}{
			"agent_name":    inc.AgentName,
			"agent_type":    inc.AgentType,
			"exit_code":     inc.ExitCode,
			"staged_path":   stagedPath,
			"staged_sha256": digest,
			"incident_id":   inc.ID,
		},
		ResolutionStatus: "captured",
	}

	if err := c.SubmitErrorNode(ctx, errorNode); err != nil {
		return "", err
	}
