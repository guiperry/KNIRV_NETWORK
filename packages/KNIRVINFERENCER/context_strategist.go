...
// splitIntoChunks splits text into chunks using the configured strategy and
// applies cross-chunk overlap so context is preserved across window boundaries.
func (cm *ContextStrategist) splitIntoChunks(text string) []string {
	return cm.applyChunkOverlap(cm.splitIntoChunksRaw(text))
}

// splitIntoChunksRaw performs the actual chunking without overlap post-processing.
func (cm *ContextStrategist) splitIntoChunksRaw(text string) []string {
	switch cm.strategy {
...
// applyChunkOverlap prepends the trailing sentences of each chunk to the start
// of the following chunk so the token-budget window can see context across
// chunk boundaries. Overlap is measured in tokens and only applied when the
// boundary text is not already present in the receiving chunk.
func (cm *ContextStrategist) applyChunkOverlap(chunks []string) []string {
	...
	for i := 1; i < len(chunks); i++ {
		prev := chunks[i-1]
		next := chunks[i]
		tail, tailTokens := cm.trailingOverlap(prev, cm.chunkOverlap)
		if tailTokens <= 0 || strings.Contains(next, strings.TrimSpace(tail)) {
			overlapped[i] = next
			continue
		}
		overlapped[i] = tail + "\n\n" + next
...
// trailingOverlap returns up to 'budget' tokens worth of trailing sentences
// from text, plus the estimated token count of the returned tail.
func (cm *ContextStrategist) trailingOverlap(text string, budget int) (string, int) {
	sentenceRegex := regexp.MustCompile(`[.!?]\s+`)
	...
	for i := len(parts) - 1; i >= 0; i-- {
		...
		toks := estimateTokens(trimmed, cm.modelName)
		if used+toks > budget && len(tail) > 0 {
			break
		}
