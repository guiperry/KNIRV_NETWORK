...
// tiktokenEncodingCache memoises *tiktoken.Tiktoken instances by encoding
// name. tiktoken.GetEncoding is expensive: every call rebuilds the BPE regex
// (regexp2.Compile) and decoder maps. Under the concurrent load observed in
// production this made token estimation the dominant cost (16 goroutines x 10
// estimates took ~48s). The cached *tiktoken.Tiktoken is safe for concurrent
// use because its CoreBPE fields are read-only during Encode and regexp2 uses a
// runner pool under the hood.
var tiktokenEncodingCache sync.Map
...
func getEncodingForModel(model string) (*tiktoken.Tiktoken, error) {
	...
	if name, ok := tiktoken.MODEL_TO_ENCODING[lowerModel]; ok {
		encodingName = name
	} else {
		// Check for prefixes from our custom map
		for prefix, name := range modelToEncoding {
	...
	if cached, ok := tiktokenEncodingCache.Load(encodingName); ok {
	...
	actual, _ := tiktokenEncodingCache.LoadOrStore(encodingName, enc)
...
// coTAnswerMarkers separate the final answer from the reasoning trace in a
// chain-of-thought response.
var coTAnswerMarkers = []string{"Final answer:", "Final Answer:", "ANSWER:", "Answer:"}

// parseCoTResponse extracts the final answer from a chain-of-thought response,
// stripping the reasoning trace. When no answer marker is present the response
// is returned unchanged.
func parseCoTResponse(response string) string {
	lower := strings.ToLower(response)
	idx := -1
	markerLen := 0
	for _, m := range coTAnswerMarkers {
		if i := strings.LastIndex(lower, strings.ToLower(m)); i > idx {
			idx = i
			markerLen = len(m)
		}
	}
	...
	answer := strings.TrimSpace(response[idx+markerLen:])
	if answer == "" {
		return strings.TrimSpace(response)
	}
	return answer
...
func (d *DelegatorService) GenerateWithCoT(ctx context.Context, promptText string) (string, error) {
	...
	if d.moa != nil {
		...
		if err != nil {
			...
			return parseCoTResponse(response), nil
		}
	}
	...
	return parseCoTResponse(fullResponse), nil
}
