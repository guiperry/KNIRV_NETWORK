...
// streamEndpoint returns the Gemini streaming endpoint for the provider. It
// mirrors Endpoint() but targets :streamGenerateContent with the SSE alt.
func (p *GeminiProvider) streamEndpoint() string {
	...
	return fmt.Sprintf("%smodels/%s:streamGenerateContent?alt=sse&key=%s", base, model, apiKey)
}
...
// applyGenerationConfig propagates gollm option keys onto the Gemini
// GenerationConfig and stream flag. Supported keys:
//
//	temperature, top_p, top_k, max_tokens, presence_penalty,
//	frequency_penalty, seed, candidate_count, stop_sequences,
//	response_mime_type, stream.
func applyGenerationConfig(cfg *GeminiGenerationConfig, stream *bool, options map[string]interface{}) {
	if options == nil {
		return
	}
	...
	if v, ok := options["candidate_count"].(float64); ok {
		i := int32(v)
		cfg.CandidateCount = &i
	}
	if v, ok := options["stop_sequences"].([]interface{}); ok {
		for _, item := range v {
			if s, ok := item.(string); ok {
				cfg.StopSequences = append(cfg.StopSequences, s)
			}
		}
	}
	if v, ok := options["response_mime_type"].(string); ok {
		cfg.ResponseMIMEType = v
	}
	if v, ok := options["stream"].(bool); ok && stream != nil {
		*stream = v
	}
}
...
func (p *GeminiProvider) PrepareRequest(prompt string, options map[string]interface{}) ([]byte, error) {
	...
	applyGenerationConfig(reqBody.GenerationConfig, &reqBody.Stream, options)
...
// doGenerateRequest posts a prepared request body to the Gemini generateContent
// endpoint and hands the response to ParseResponse.
func (p *GeminiProvider) doGenerateRequest(ctx context.Context, reqBytes []byte) (string, error) {
	p.mutex.Lock()
	apiKey := p.apiKey
	httpClient := p.client
	p.mutex.Unlock()
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.Endpoint(), bytes.NewReader(reqBytes))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", fmt.Errorf("gemini API returned status %d: %s", resp.StatusCode, msg)
	}
...
func (p *GeminiProvider) GenerateContent(ctx context.Context, prompt string) (string, error) {
	reqBytes, err := p.PrepareRequest(prompt, nil)
	if err != nil {
		return "", fmt.Errorf("failed to prepare Gemini request: %w", err)
	}
	return p.doGenerateRequest(ctx, reqBytes)
}
...
func (p *GeminiProvider) GenerateContentFromMessages(ctx context.Context, messages []types.MemoryMessage) (string, error) {
	reqBytes, err := p.PrepareRequestWithMessages(messages, nil)
	if err != nil {
...
func (p *GeminiProvider) StreamContent(ctx context.Context, prompt string) (chan string, chan error) {
	...
	go func() {
		...
		// Read streaming response line by line. Gemini streams the payload as
		// server-sent events (SSE), so each non-empty line is "data: {json}".
		...
		for scanner.Scan() {
			...
			// Strip the SSE "data: " prefix before JSON parsing.
