...
// localLlamaAPIKey satisfies gollm's generic provider validation. The local
// llama provider never adds it to request headers.
const (
	localLlamaAPIKey         = "knirv-local-llama-no-auth"
	localLlamaMaxTokens      = 256
	localLlamaRequestTimeout = 15 * time.Minute
)
...
type InferenceService struct {
	...
	// dreamAttempts are isolated from interactive request routing. They are
	// selected explicitly by GenerateTextWithProvider for background work.
	dreamAttempts     []LLMAttempt
...
// --- ADDED: GenerateTextWithProviderAndSchema ---
// GenerateTextWithProviderAndSchema is the schema-aware variant of
// GenerateTextWithProvider.  It requests JSON-shaped output from the named
// provider (which llama.cpp's server honors via `response_format:
// {"type":"json_object"}`) and unmarshals the response into `result` before
// returning it.  Phase D of the llama cognitive-engine plan: the cognitive
// engine uses this path so it can validate and gate dream-task output
// instead of logging raw model prose.
func (s *InferenceService) GenerateTextWithProviderAndSchema(providerName, promptText string, schema interface{}, result interface{}) error {
	raw, err := s.GenerateTextWithProvider(providerName, promptText)
	if err != nil {
		return err
	}
	if result == nil {
	...
	if err := json.Unmarshal([]byte(raw), result); err != nil {
		return fmt.Errorf("provider %q returned non-JSON response: %w", providerName, err)
	}
	return nil
}
...
func (s *InferenceService) findLLMInstance(providerName string) llm.LLM {
	for _, attempt := range s.dreamAttempts {
...
// RegisterDreamProvider registers a local provider for explicit background
// inference. It deliberately does not alter the primary/fallback lists used by
// interactive chat and orchestration.
func (s *InferenceService) RegisterDreamProvider(providerName, model, endpoint string) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()

	opts := []config.ConfigOption{
		config.SetProvider(providerName),
		// gollm validates that every provider has a key. The embedded provider
		// never sends this sentinel value over the network.
		config.SetAPIKey(localLlamaAPIKey),
		config.SetModel(model),
		// Dream tasks are background summaries, not interactive unbounded
		// completions. Bound their output and allow enough time for a CPU-only
		// local model to finish while other slots are busy.
		config.SetMaxTokens(localLlamaMaxTokens),
		config.SetTimeout(localLlamaRequestTimeout),
	}
	llmInstance, err := gollm.NewLLM(opts...)
	if err != nil {
		return fmt.Errorf("failed to create dream LLM instance for provider '%s': %w", providerName, err)
	}
	initializedLLM, ok := llmInstance.(llm.LLM)
	if !ok {
		return fmt.Errorf("initialized instance for provider '%s' is not of type llm.LLM", providerName)
	}
	s.dreamAttempts = append(s.dreamAttempts, LLMAttempt{
		Instance: initializedLLM,
		Config:   LLMAttemptConfig{ProviderName: providerName, ModelName: model, MaxTokens: localLlamaMaxTokens, IsPrimary: true},
		Opts:     opts,
	})
	log.Printf("InferenceService: Registered local provider '%s' (%s) for dream tasks", providerName, model)
	return nil
}

// RegisterLocalProvider registers a local provider instance (e.g. llama.cpp)
// without requiring an API key. The provider is added as a fallback attempt.
func (s *InferenceService) RegisterLocalProvider(providerName, model, endpoint string) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	...
	llmInstance, err := gollm.NewLLM(opts...)
	if err != nil {
		return fmt.Errorf("failed to create local LLM instance for provider '%s': %w", providerName, err)
	}
	...
	if initializedLLM, ok := llmInstance.(llm.LLM); ok {
		attempt := LLMAttempt{
			...
			Config: LLMAttemptConfig{
				...
				ModelName:    model,
				APIKeyEnvVar: "",
				MaxTokens:    localLlamaMaxTokens,
				IsPrimary:    false,
			},
			Opts: opts,
		}
		s.fallbackAttempts = append(s.fallbackAttempts, attempt)
		log.Printf("InferenceService: Registered local provider '%s' (%s) as fallback", providerName, model)
		return nil
	}
