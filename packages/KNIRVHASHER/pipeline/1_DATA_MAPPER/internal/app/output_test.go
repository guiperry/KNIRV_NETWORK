...
func TestWriteJSONOutputEmptyData(t *testing.T) {
	...
	content := string(data)
	// writeJSONOutput writes compactly (no indentation), so an empty result
	// set is just an empty array with no records between the brackets.
	if content != "[]" {
		t.Errorf("Empty JSON format mismatch. Expected '[]', got: %q", content)
	}
}
