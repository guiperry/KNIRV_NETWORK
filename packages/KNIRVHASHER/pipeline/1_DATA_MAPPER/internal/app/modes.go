...
func RunApplication(config *Config) error {
	...
	if files, err := stagedInputFiles(stagedInputDir(config)); err == nil && len(files) > 0 {
		fmt.Println("📥 Processing Stage 0 connector records")
		return ProcessStagedRecords(context.Background(), config)
	}
	...
	fmt.Println("🚀 Starting Data Mapper with continuous workflow...")
...
func RunDemoFrameGenerator(config *Config) error {
	...
	scriptCandidates := []string{
		filepath.Join("pipeline", "1_DATA_MAPPER", "scripts", "generate_hello_world.py"),
