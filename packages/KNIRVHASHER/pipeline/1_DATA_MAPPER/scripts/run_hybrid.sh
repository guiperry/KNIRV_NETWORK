...
# Optimized Data Mapper Runner with Hybrid Embeddings
...
echo "Running Data Mapper with Hybrid Embeddings..."
...
# Check if data-mapper binary exists
DATAMINER_BINARY="$PROJECT_ROOT/cmd/data-mapper/data-mapper"
if [ ! -f "$DATAMINER_BINARY" ]; then
    echo "🔨 Dataminer binary not found, building..."
    cd "$PROJECT_ROOT/cmd/data-mapper" && go build -o data-mapper main.go
fi
