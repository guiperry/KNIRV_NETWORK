...
import (
	"github.com/knirv/hasher/pipeline/3_DATA_TRAINER/pkg/training"
)
...
type TrainingScheduler struct {
	onDemand  chan *training.UserSecurityGates
	violation chan *GuardrailViolation
	scheduler *CronScheduler
	trainer   *training.UserTrainer
	client    *grpc.Client
}
...
func NewTrainingScheduler(cfg *SchedulerConfig) *TrainingScheduler {
	ts := &TrainingScheduler{
		onDemand:  make(chan *training.UserSecurityGates, 10),
...
func (ts *TrainingScheduler) handleViolations() {
	for {
		select {
		case v := <-ts.violation:
			gates := &training.UserSecurityGates{
				OrgID:  v.NodeID,
				UserID: v.NodeID,
				Constraints: []training.SecurityConstraint{
					{
...
func (ts *TrainingScheduler) runTraining(gates *training.UserSecurityGates) {
...
func (ts *TrainingScheduler) saveTrainedGates(gates *training.TrainedGates) error {
	return nil
}

func (ts *TrainingScheduler) TriggerOnDemand(gates *training.UserSecurityGates) {
	ts.onDemand <- gates
}
