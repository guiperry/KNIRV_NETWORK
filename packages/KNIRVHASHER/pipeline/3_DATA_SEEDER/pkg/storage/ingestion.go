...
func (di *DataIngestor) SetFiles(files []string) {
		...
		if len(fields) >= 15 { // We have at least source_file, chunk_id, asic_slots_0-11, target_token_id
			...
			if len(fields) >= 15 {
				...
				if targetID, err := strconv.ParseInt(strings.Trim(fields[14], "\""), 32, 64); err == nil {
					...
					record.TokenSequence = tokenSequenceOrTarget(nil, record.TargetToken)
				}
...
func tokenSequenceOrTarget(tokenSequence []int32, targetToken int32) []int32 {
