...
func TestVarianceAnalyzerSampleFromRecordJSONNumberAndInvalidValue(t *testing.T) {
	va := NewVarianceAnalyzer()

	decoderEmbedding := make([]interface{}, 768)
	for i := range decoderEmbedding {
		decoderEmbedding[i] = json.Number("0.125")
	}

	if err := va.SampleFromRecord(decoderEmbedding); err != nil {
		t.Fatalf("sampleFromRecord json.Number failed: %v", err)
	}

	badEmbedding := make([]interface{}, 768)
	for i := range badEmbedding {
		badEmbedding[i] = float64(i)
	}
	badEmbedding[42] = map[string]interface{}{"bad": "value"}

	if err := va.SampleFromRecord(badEmbedding); err == nil {
		t.Fatal("expected error for invalid embedding element")
	}
}
