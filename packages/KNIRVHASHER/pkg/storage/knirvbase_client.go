...
type KNIRVBASEClient struct {
	...
	storage         map[string]map[string]string // collection -> id -> data
}
...
func (c *KNIRVBASEClient) insert(collection, id, data string) error {
	// Initialize collection map if it doesn't exist
	if _, ok := c.storage[collection]; !ok {
		c.storage[collection] = make(map[string]string)
	}
	// Store the data
	c.storage[collection][id] = data
	return nil
}

func (c *KNIRVBASEClient) findAll(collection string) ([]string, error) {
	// Return all documents in the collection
	col, ok := c.storage[collection]
	if !ok {
		return []string{}, nil
	}

	results := make([]string, 0, len(col))
