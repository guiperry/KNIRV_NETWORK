// pkg/hashing/neural/mining_neuron.go
// Mining-based neuron activation using BM1382 ASIC via HashMethod interface
//
...
type MiningNeuron struct {
	...
	// HashMethod interface for hardware acceleration
...
func (n *MiningNeuron) SetHashMethod(method core.HashMethod) {
	n.mu.Lock()
	defer n.mu.Unlock()
	n.hashMethod = method
}
...
func (n *MiningNeuron) mine(header []byte) (uint32, error) {
	if n.hashMethod != nil && n.hashMethod.IsAvailable() {
		return n.mineASIC(header)
	}
	return n.mineSoftware(header)
}

// mineASIC uses the hardware ASIC for mining via HashMethod interface
func (n *MiningNeuron) mineASIC(header []byte) (uint32, error) {
	// Use HashMethod.MineHeader for hardware-accelerated mining
	result, err := n.hashMethod.MineHeader(header, n.NonceStart, n.NonceEnd)
	if err != nil {
