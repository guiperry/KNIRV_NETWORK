...
if [ ! -f "data/testnet-gateway/package.json" ]; then
    ...
    if [ -d "../packages/KNIRVGATEWAY" ]; then
        cp ../packages/KNIRVGATEWAY/package.json data/testnet-gateway/ 2>/dev/null || true
        cp ../packages/KNIRVGATEWAY/package-lock.json data/testnet-gateway/ 2>/dev/null || true
        cp -r ../packages/KNIRVGATEWAY/assets data/testnet-gateway/ 2>/dev/null || true
        cp -r ../packages/KNIRVGATEWAY/netlify data/testnet-gateway/ 2>/dev/null || true
        cp -r ../packages/KNIRVGATEWAY/scripts data/testnet-gateway/ 2>/dev/null || true
        cp -r ../packages/KNIRVGATEWAY/graphchain-explorer data/testnet-gateway/ 2>/dev/null || true
        cp -r ../packages/KNIRVGATEWAY/developer-portal data/testnet-gateway/ 2>/dev/null || true
