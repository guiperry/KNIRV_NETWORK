...
if [ "$RENDER" = "true" ] || [ -n "$RENDER_SERVICE_ID" ]; then
    ...
    DOCKERFILES=(
        ...
        "../packages/KNIRVORACLE/Dockerfile"
        "../packages/KNIRVCHAIN/Dockerfile"
        "../packages/KNIRVGRAPH/Dockerfile"
        "../packages/KNIRVSERVER/Dockerfile"
        "../packages/KNIRVROUTER/Dockerfile"
    )
    ...
    services=("KNIRVORACLE" "KNIRVCHAIN" "KNIRVGRAPH" "KNIRVSERVER" "KNIRVROUTER")
    for service in "${services[@]}"; do
        print_status "Building $service image..."
        if docker build -f "../packages/$service/Dockerfile" -t "knirv-${service,,}:latest" "../packages/$service/" 2>&1; then
