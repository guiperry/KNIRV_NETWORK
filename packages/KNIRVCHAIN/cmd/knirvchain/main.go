...
func main() {
	...
	managedByServer := func() bool {
		switch strings.ToLower(strings.TrimSpace(os.Getenv("KNIRV_MANAGED_BY_SERVER"))) {
		case "1", "true", "yes", "on":
			return true
		default:
			return false
	...
	cfg.ManagedByServer = managedByServer

	// Fetch and store public IP info for standalone launches or root nodes.
	// When KNIRVSERVER owns the bootnode lifecycle, the gateway process handles
	// bootnode registration and external IP discovery instead.
	if nodeRole == config.Root || !managedByServer {
		...
		log.Printf("[%s][%s] Skipping public IP discovery because KNIRVSERVER manages bootnode registration", nodeRole.String(), cfg.ChainID)
	}
	...
	if !cfg.IsPeer {
		...
		// Follow KNIRVSERVER's network-mode convention when -testnet was not
		// explicitly passed: KNIRV_NETWORK_MODE is propagated by the parent
		// process's manager. "": no signal (standalone run) — keep the
		// flag's own default rather than forcing testnet.
		if !flagsSet["testnet"] {
			switch os.Getenv("KNIRV_NETWORK_MODE") {
			case "testnet":
				*testnetMode = true
		...
		// Set testnet flag in config if testnet mode is active
		if *testnetMode {
		...
		// (no-op: KNIRVORACLE now owns NRN disbursement; see internal/oracle/payment)
...
func startNodeWithComponents(
	...
	go func() {
		...
		configureCheckpointRuntime(bc, db, blockchainSrv, cfg.MinersAddress)
