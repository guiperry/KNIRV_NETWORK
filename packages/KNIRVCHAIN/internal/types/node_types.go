...
// ErrorNodeRecord is KNIRVCHAIN's *storage* form of an error node.
//
// It is deliberately not the wire type. shared-proto/graph/v1/graph.proto
// (internal/protocol/proto) is the single canonical definition, shared with
// KNIRVGRAPH's drq/nrv. This record exists because the chain persists nodes as
// plain JSON via internal/graph/node_store.go, and two fields genuinely differ
// from the wire form: Context is a plain `map[string]interface{}` here (JSON
// object) versus google.protobuf.Struct on the wire, and Status is a string in
// both. Aliasing the storage type to the wire type would silently change the
// persisted shape and break every previously stored row.
//
// Convert at the boundaries with ToCanonical / ErrorNodeRecordFromCanonical.
type ErrorNodeRecord struct {
	ID             string                 `json:"id"`
...
func NewErrorNodeRecord(id, errorType, errorSignature, modelOrigin string, context map[string]interface{}) (*ErrorNodeRecord, error) {
	if id == "" {
...
func (en *ErrorNodeRecord) IncrementFailureCount() {
	en.FailureCount++
}

// UpdateStatus updates the node status with validation
func (en *ErrorNodeRecord) UpdateStatus(status NodeStatus) error {
...
// ToCanonical converts a stored record into the canonical wire node.
func (r *ErrorNodeRecord) ToCanonical() (*proto.ErrorNode, error) {
	if r == nil {
		return nil, nil
	}
	var context *structpb.Struct
	if len(r.Context) > 0 {
		encoded, err := structpb.NewStruct(r.Context)
		if err != nil {
			return nil, fmt.Errorf("encode error node context: %w", err)
		}
		context = encoded
	}
	return &proto.ErrorNode{
		Id:             r.ID,
		ErrorType:      r.ErrorType,
		ErrorSignature: r.ErrorSignature,
		ModelOrigin:    r.ModelOrigin,
		Context:        context,
...
// ErrorNodeRecordFromCanonical converts a canonical wire node into the chain's
// storage form, preserving the persisted JSON shape. An unrecognised status is
// rejected rather than stored, so a bad wire value cannot corrupt the store.
func ErrorNodeRecordFromCanonical(node *proto.ErrorNode) (*ErrorNodeRecord, error) {
	if node == nil {
		return nil, nil
	}
	...
	if !IsValidNodeStatus(status) {
		return nil, errors.NewValidationError("unknown node status: " + node.GetStatus())
	}
	return &ErrorNodeRecord{
		ID:             node.GetId(),
		ErrorType:      node.GetErrorType(),
		ErrorSignature: node.GetErrorSignature(),
		ModelOrigin:    node.GetModelOrigin(),
		Context:        node.GetContext().AsMap(),
		FailureCount:   node.GetFailureCount(),
