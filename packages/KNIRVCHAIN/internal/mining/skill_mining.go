...
type MiningProposal struct {
	...
	EstimatedCostNRN uint64                    `json:"estimated_cost_nrn"`
}
...
func (sm *SkillMiner) SubmitMiningProposal(errorNodeID, minerAddress string, loraAdapter *types.LoRAAdapterPointer) (*MiningProposal, error) {
	...
	// Calculate mining cost using pricing engine
	estimatedCost := sm.pricingEngine.CalculateSkillMintCost(
		len(loraAdapter.AdapterID),
		false,
		loraAdapter.Rank,
	)
	...
	proposal := &MiningProposal{
		...
		EstimatedCostNRN: estimatedCost,
	}
...
func (sm *SkillMiner) createMiningProposalTransaction(proposal *MiningProposal) (*blockchain.Transaction, error) {
	...
	txFee := proposal.EstimatedCostNRN
	if txFee == 0 {
		txFee = 100
	}
...
func (sm *SkillMiner) ValidateSkillProposal(proposal *MiningProposal) (*types.SkillNode, error) {
	...
	if validationResult.IsValid {
		...
		if sm.metrics != nil {
			sm.metrics.MiningDuration.Observe(time.Since(startTime).Seconds())
			sm.metrics.MiningRewards.Add(float64(skillNode.NRNReward))
			sm.metrics.SkillNodesTotal.Inc()
		}
