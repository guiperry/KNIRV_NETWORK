...
import (
	...
	"KNIRVCHAIN/config"
	// Added to access constants
	chromem "github.com/philippgille/chromem-go"
	// Removed leveldb import as we no longer use it directly
)

// EmbeddingClient interface to avoid circular imports
type EmbeddingClient interface {
	GenerateEmbedding(ctx context.Context, text string) ([]float32, error)
}

// DummyEmbeddingClient provides dummy embeddings for testing
type DummyEmbeddingClient struct{}

func (d *DummyEmbeddingClient) GenerateEmbedding(ctx context.Context, text string) ([]float32, error) {
...
type ChromemManager struct {
	...
	embeddingClient                EmbeddingClient       // Embedding client interface to avoid circular imports
}

// GetTransactionCollection returns the underlying transaction chromem collection.
// Used by packages that wrap ChromemManager for type-specific storage.
func (m *ChromemManager) GetTransactionCollection() *chromem.Collection {
	return m.transactionCollection
}

// GetContextCollection returns the underlying context record chromem collection.
// Used by packages that wrap ChromemManager for type-specific storage.
func (m *ChromemManager) GetContextCollection() *chromem.Collection {
	return m.contextRecordCollection
}
...
func NewChromemManager(cfg *config.ChromemConfig) (*ChromemManager, error) {
	...
	return &ChromemManager{
		...
		embeddingClient:                embeddingClient, // Store the embedding client
	}, nil
}
...
func (m *ChromemManager) OnNewBlockConfirmed(block interface{}) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	log.Printf("ChromemDB Manager: Processing new confirmed block")

	// Try to extract block information using type assertion
	if blockMap, ok := block.(map[string]interface{}); ok {
		// Extract block number and hash if available
		blockNumber, hasBlockNumber := blockMap["number"]
		blockHash, hasBlockHash := blockMap["hash"]

		if hasBlockNumber && hasBlockHash {
			log.Printf("ChromemDB Manager: Confirmed block #%v with hash %v", blockNumber, blockHash)

			// Process transactions in the block if available
			if transactions, hasTxs := blockMap["transactions"]; hasTxs {
				if txList, ok := transactions.([]interface{}); ok {
					for _, tx := range txList {
						if txMap, ok := tx.(map[string]interface{}); ok {
							if err := m.processBlockTransaction(txMap); err != nil {
								log.Printf("ChromemDB Manager: Error processing transaction in block: %v", err)
							}
						}
					}
				}
		...
		log.Printf("ChromemDB Manager: Block type assertion failed, received type: %T", block)
	}

	return nil
}

// processBlockTransaction processes a transaction from a confirmed block
func (m *ChromemManager) processBlockTransaction(tx map[string]interface{}) error {
	// Extract transaction hash and type
	txHash, hasTxHash := tx["hash"]
	txType, hasTxType := tx["type"]

	if !hasTxHash || !hasTxType {
		return fmt.Errorf("transaction missing hash or type")
	}

	log.Printf("ChromemDB Manager: Processing block transaction %v of type %v", txHash, txType)

	// Index the transaction in ChromemDB for searchability
	if _, err := json.Marshal(tx); err == nil {
		document := fmt.Sprintf("Transaction %v of type %v", txHash, txType)
		metadataStr := map[string]string{
			"transaction_hash": fmt.Sprintf("%v", txHash),
			"transaction_type": fmt.Sprintf("%v", txType),
			"source":           "confirmed_block",
		}

		// Use the correct ChromemDB Add method signature
		ids := []string{fmt.Sprintf("block_tx_%v", txHash)}
		documents := []string{document}
		metadatas := []map[string]string{metadataStr}

		if err := m.transactionCollection.Add(context.Background(), ids, nil, metadatas, documents); err != nil {
			log.Printf("ChromemDB Manager: Error indexing block transaction: %v", err)
		}
	}
...
func (m *ChromemManager) OnBlockOrphaned(block interface{}) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	log.Printf("ChromemDB Manager: Handling orphaned block")
	...
	if blockMap, ok := block.(map[string]interface{}); ok {
		...
		if hasBlockNumber && hasBlockHash {
			log.Printf("ChromemDB Manager: Orphaning block #%v with hash %v", blockNumber, blockHash)

			// Remove transactions from the orphaned block if available
			if transactions, hasTxs := blockMap["transactions"]; hasTxs {
				if txList, ok := transactions.([]interface{}); ok {
					for _, tx := range txList {
						if txMap, ok := tx.(map[string]interface{}); ok {
							if err := m.removeOrphanedTransaction(txMap); err != nil {
								log.Printf("ChromemDB Manager: Error removing orphaned transaction: %v", err)
							}
						}
					}
				}
		...
		log.Printf("ChromemDB Manager: Orphaned block type assertion failed, received type: %T", block)
	}

	return nil
}

// removeOrphanedTransaction removes a transaction from an orphaned block
func (m *ChromemManager) removeOrphanedTransaction(tx map[string]interface{}) error {
	...
	log.Printf("ChromemDB Manager: Removing orphaned transaction %v", txHash)

	// Remove the transaction from ChromemDB
	txID := fmt.Sprintf("block_tx_%v", txHash)
	// Use the correct ChromemDB Delete method signature
	if err := m.transactionCollection.Delete(context.Background(), nil, nil, txID); err != nil {
		log.Printf("ChromemDB Manager: Error removing orphaned transaction: %v", err)
		return err
	}
...
func (m *ChromemManager) StoreAgent(agentData *Agent) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	content := fmt.Sprintf("Agent ID: %s, Name: %s, Description: %s, Owner: %s, Type: %s, Created: %s",
		agentData.ID, agentData.Name, agentData.Description, agentData.Owner, agentData.AgentType, agentData.CreatedAt.Format(time.RFC3339))
...
func (m *ChromemManager) GetAgentResourceCapabilities(agentID string) ([]*Capability, error) {
	...
	for _, cap := range agent.Capabilities {
		if cap.CapabilityType == "resource" {
			resourceCapabilities = append(resourceCapabilities, cap)
		}
	}
