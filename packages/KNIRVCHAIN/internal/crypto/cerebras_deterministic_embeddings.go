...
// deterministic embeddings that provide good semantic similarity for KNIRVCHAIN memory
...
func (c *CerebrasEmbeddingClient) GenerateEmbeddings(ctx context.Context, texts []string) ([]float32, error) {
	...
	fmt.Printf("DEBUG: KNIRVCHAIN generated deterministic embedding for text: %.50s... (dimension: %d)\n", text, len(embedding))
	return embedding, nil
}

// CreateCerebrasEmbeddingFunction creates an embedding function for chromem-go
// This function provides deterministic, high-quality embeddings for KNIRVCHAIN memory
