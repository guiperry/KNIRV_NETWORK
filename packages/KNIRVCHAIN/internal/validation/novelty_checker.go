...
// ValidateIdeaType uses the classifier to validate idea type classification
func (nc *NoveltyChecker) ValidateIdeaType(ctx context.Context, ideaType string) classifier.ClassificationResult {
	if nc.classifier == nil {
		return classifier.ClassificationResult{}
	}
	return nc.classifier.ClassifyWithDetails(ctx, ideaType)
}
...
func (nc *NoveltyChecker) AssessNovelty(ideaNode *types.IdeaNode) (*NoveltyResult, error) {
	...
	if len(existingIdeas) == 0 {
		...
		if nc.metrics != nil {
			nc.metrics.NoveltyChecks.Inc()
			nc.metrics.NoveltyCheckDuration.Observe(time.Since(startTime).Seconds())
		}
		return result, nil
	}
