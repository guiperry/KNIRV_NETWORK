...
**Source**: KNIRVROOT/ansible/ansible_bootnode_deployments.md

```markdown
# Ansible Playbook for KNIRVROOT Bootnode Deployment on AWS

This Ansible playbook automates the deployment of a KNIRVROOT bootnode on Amazon Web Services (AWS). It handles both the provisioning of the necessary AWS infrastructure and the configuration of the bootnode software.
...
*   The KNIRVROOT executable (`KNIRVROOT_executable`) is pre-compiled and available on the Ansible control node.
...
```yaml
---
- name: Provision KNIRVROOT Bootnode Infrastructure on AWS
  hosts: localhost
  gather_facts: false
  vars:
    ...
    security_group_name: "KNIRVROOT-bootnode-sg"
    p2p_port: 6050 # Your KNIRVROOT P2P port
    api_port: 6000 # Your KNIRVROOT API port
    ssh_port: 22
    instance_tags:
      Name: "KNIRVROOT-bootnode"
      Project: "KNIRVROOT"
  ...
  tasks:
    - name: Create Security Group for Bootnode
      amazon.aws.ec2_security_group:
        ...
        description: "Security group for KNIRVROOT Bootnode"
        region: "{{ aws_region }}"
        rules:
          - proto: tcp
            ports:
...
- name: Configure KNIRVROOT Bootnode Software
  hosts: KNIRVROOT_bootnode_host # Target the dynamically added host
  become: true # Most tasks require sudo
  vars:
    KNIRVROOT_executable_local_path: "/path/to/your/local/KNIRVROOT_executable" # CHANGE THIS
    KNIRVROOT_config_local_path: "/path/to/your/local/bootnode_config.json"     # CHANGE THIS
    KNIRVROOT_wallet_local_path: "/path/to/your/local/wallet.dat"           # CHANGE THIS
    KNIRVROOT_master_wallet_local_path: "/path/to/your/local/master_wallet.dat" # CHANGE THIS

    remote_user: "{{ ansible_ssh_user }}" # e.g., 'ubuntu'
    remote_KNIRVROOT_bin_dir: "/opt/KNIRVROOT"
    remote_KNIRVROOT_app_config_dir: "/home/{{ remote_user }}/.config/KNIRVROOT"
    remote_KNIRVROOT_data_dir_base: "/home/{{ remote_user }}/.config/KNIRVROOT/data" # Base for data, actual path might include ChainID
  ...
  tasks:
    ...
    - name: Create KNIRVROOT binary directory
      ansible.builtin.file:
        path: "{{ remote_KNIRVROOT_bin_dir }}"
        state: directory
        mode: '0755'

    - name: Create KNIRVROOT application config directory
      ansible.builtin.file:
        path: "{{ remote_KNIRVROOT_app_config_dir }}"
        state: directory
        owner: "{{ remote_user }}"
        group: "{{ remote_user }}"
        mode: '0750' # Restrict access to config files

    - name: Create KNIRVROOT application data base directory
      ansible.builtin.file:
        path: "{{ remote_KNIRVROOT_data_dir_base }}"
        state: directory
        owner: "{{ remote_user }}"
        group: "{{ remote_user }}"
        mode: '0750'
    ...
    - name: Copy KNIRVROOT config.json for bootnode
      ansible.builtin.copy:
        src: "{{ KNIRVROOT_config_local_path }}"
        dest: "{{ remote_KNIRVROOT_app_config_dir }}/config.json"
        owner: "{{ remote_user }}"
        group: "{{ remote_user }}"
        mode: '0640'

    - name: Copy KNIRVROOT wallet.dat
      ansible.builtin.copy:
        src: "{{ KNIRVROOT_wallet_local_path }}"
        dest: "{{ remote_KNIRVROOT_app_config_dir }}/wallet.dat"
        owner: "{{ remote_user }}"
        group: "{{ remote_user }}"
        mode: '0600' # Restrict access

    - name: Copy KNIRVROOT master_wallet.dat
      ansible.builtin.copy:
        src: "{{ KNIRVROOT_master_wallet_local_path }}"
        dest: "{{ remote_KNIRVROOT_app_config_dir }}/master_wallet.dat"
        owner: "{{ remote_user }}"
        group: "{{ remote_user }}"
        mode: '0600' # Restrict access

    - name: Create systemd service file for KNIRVROOT bootnode
      ansible.builtin.template:
        src: templates/KNIRVROOT-bootnode.service.j2
        dest: "/etc/systemd/system/{{ service_name }}.service"
        mode: '0644'
      notify: Reload systemd and restart KNIRVROOT-bootnode

  handlers:
    - name: Reload systemd and restart KNIRVROOT-bootnode
      ansible.builtin.systemd:
        daemon_reload: true

    - name: Enable and start KNIRVROOT-bootnode service
      ansible.builtin.systemd:
        name: "{{ service_name }}"
        enabled: true
...
## Systemd Service Template (`templates/KNIRVROOT-bootnode.service.j2`)

Create a directory named `templates` in the same directory as your playbook. Inside it, create the file `KNIRVROOT-bootnode.service.j2`:

```jinja
[Unit]
Description=KNIRVROOT Bootnode Service
After=network.target

[Service]
User={{ remote_user }}
Group={{ remote_user }}
WorkingDirectory=/home/{{ remote_user }} # Or {{ remote_KNIRVROOT_bin_dir }}
ExecStart={{ remote_KNIRVROOT_bin_dir }}/KNIRVROOT_executable --bootnode
# If your application needs a specific config path when run as a service:
# ExecStart={{ remote_KNIRVROOT_bin_dir }}/KNIRVROOT_executable --bootnode --config {{ remote_KNIRVROOT_app_config_dir }}/config.json
...
1.  **Replace Placeholders:**
    ...
    *   **In the playbook (`deploy_bootnode_aws.yml`):**
        ...
        *   `KNIRVROOT_executable_local_path`
        *   `KNIRVROOT_config_local_path`
        *   `KNIRVROOT_wallet_local_path`
        *   `KNIRVROOT_master_wallet_local_path`

    *   **In `templates/KNIRVROOT-bootnode.service.j2`:**
        *   Review `WorkingDirectory`. The default `WorkingDirectory=/home/{{ remote_user }}` should allow the application to find its config in `~/.config/KNIRVROOT/` correctly when run as `{{ remote_user }}`.
...
1.  Save the playbook as `deploy_bootnode_aws.yml` and the template in `templates/KNIRVROOT-bootnode.service.j2`.
...
This playbook provides a solid foundation for deploying KNIRVROOT bootnodes on AWS. You can extend it further with more sophisticated configuration templating, logging setup, monitoring, etc., as your needs evolve. Remember to test thoroughly in a non-production environment first.  Pay close attention to the AWS region and AMI ID; these are critical for a successful deployment.
```
