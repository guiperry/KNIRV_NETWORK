...
func (m *MockEBPFManager) GetProcessMetrics() (map[uint32]*ebpf.ProcessStats, error) {
	args := m.Called()
	return args.Get(0).(map[uint32]*ebpf.ProcessStats), args.Error(1)
}
...
func (w *ebpfManagerWrapper) GetProcessMetrics() (map[uint32]*ebpf.ProcessStats, error) {
	return w.mock.GetProcessMetrics()
}
...
func (m *minimalEBPFMock) GetProcessMetrics() (map[uint32]*ebpf.ProcessStats, error) {
	return nil, nil
}
