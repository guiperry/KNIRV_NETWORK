...
start_network_server() {
    log "Starting mobile-tool for local network access..."
...
build_and_serve() {
    log "Building mobile-tool for production..."
...
build_android_app() {
    ...
    if [ ! -f "capacitor.config.ts" ]; then
        error "Capacitor config not found. Run this script from the mobile-tool directory"
    fi
...
main() {
    ...
    if [ ! -f "package.json" ]; then
        error "Please run this script from the mobile-tool directory"
    fi
