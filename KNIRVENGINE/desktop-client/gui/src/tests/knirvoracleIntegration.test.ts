...
describe('KNIRVORACLE Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    knirvoracleService = new KNIRVOracleService({
      baseURL: process.env.VITE_KNIRVORACLE_URL || 'http://localhost:8080',
      apiKey: process.env.VITE_KNIRVORACLE_API_KEY || 'test-key',
      timeout: 10000,
    });
  ...
  describe('KNIRVOracleService', () => {
    it('should initialize with correct configuration', () => {
      ...
      expect(knirvoracleService.getConfig().baseURL).toBe(
        process.env.VITE_KNIRVORACLE_URL || 'http://localhost:8080'
      );
    });
