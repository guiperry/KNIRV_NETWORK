...
func (s *AgentInferencerService) CreateTerminal(ctx context.Context, sessionID string, rows, cols int) (string, error) {
	s.mutex.RLock()
	if !s.isRunning {
		s.mutex.RUnlock()
		return "", errors.New("service not running")
	...
	return s.inferencer.CreateTerminal(ctx, sessionID, rows, cols)
}

// ResizeTerminal resizes a terminal session
func (s *AgentInferencerService) ResizeTerminal(ctx context.Context, sessionID string, terminalID string, rows, cols int) error {
	s.mutex.RLock()
	if !s.isRunning {
		s.mutex.RUnlock()
		return errors.New("service not running")
	...
	return s.inferencer.ResizeTerminal(ctx, sessionID, terminalID, rows, cols)
}

// WriteToTerminal writes data to a terminal session
func (s *AgentInferencerService) WriteToTerminal(ctx context.Context, sessionID string, terminalID string, data []byte) error {
	s.mutex.RLock()
	if !s.isRunning {
		s.mutex.RUnlock()
		return errors.New("service not running")
	...
	return s.inferencer.WriteToTerminal(ctx, sessionID, terminalID, data)
}

// ReadFromTerminal reads data from a terminal session
func (s *AgentInferencerService) ReadFromTerminal(ctx context.Context, sessionID string, terminalID string) ([]byte, error) {
	s.mutex.RLock()
	if !s.isRunning {
		s.mutex.RUnlock()
		return nil, errors.New("service not running")
	...
	return s.inferencer.ReadFromTerminal(ctx, sessionID, terminalID)
}

// CloseTerminal closes a terminal session
func (s *AgentInferencerService) CloseTerminal(ctx context.Context, sessionID string, terminalID string) error {
	s.mutex.RLock()
	if !s.isRunning {
		s.mutex.RUnlock()
		return errors.New("service not running")
	...
	return s.inferencer.CloseTerminal(ctx, sessionID, terminalID)
}
