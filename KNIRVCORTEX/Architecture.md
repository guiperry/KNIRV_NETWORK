...
Cognitive Shell (Outer WASM): A generic container that provides the core agent logic, lifecycle, and communication capabilities. This is a standard component of your system.
...
The "forging" process happens within a new agent-forge crate, and the entire build is localized to the KNIRVCORTEX directory.

Proposed Directory Structure within KNIRVCORTEX
This structure keeps all agent-related Rust code and build artifacts neatly organized.

plaintext
KNIRVCORTEX/
├── agent-forge/         # (New) The "forge" for compiling models into inner WASM.
│   ├── Cargo.toml
│   ├── README.md
│   └── src/
...
Here are the files for the agent-forge:
...
The Agent Forge is a Rust crate for compiling pre-trained models (in ONNX format) into KNIRV-compatible WASM modules. The output `.wasm` file is a "Forged Model" that can be uploaded and executed by the KNIRV Cognitive Shell.
...
1.  Place your quantized `model.onnx` file in the `src/` directory.
...
The final agent will be located at `KNIRVCORTEX/dist/forged_agent_example.wasm`. This is the file you upload through the KNIRV Controller UI.
...
/// Input structure for the agent's inference function.
...
/// Output structure for the agent's inference function.
#[derive(Serialize, Deserialize, Debug)]
...
pub extern "C" fn init() {
    INIT.call_once(|| {
        // In a real scenario, you would `include_bytes!` your model.
...
The shell is the "outer" WASM. It defines the agent's behavior and imports the infer function, which will be provided at runtime by the host linking it to the "inner" WASM.
...
.PHONY: build-forge
build-forge:
	@echo "Building Agent Forge..."
	@cd agent-forge && cargo build --target $(TARGET) --release
	@cp agent-forge/target/$(TARGET)/release/agent_forge.wasm $(DIST_DIR)/forged_agent_example.wasm
	@echo "✓ Agent Forge built at $(DIST_DIR)/forged_agent_example.wasm"
...
test: ## Run tests for all cortex crates
	...
	@cd agent-forge && cargo test

.PHONY: clean
clean: ## Clean all build artifacts
	@cd cognitive-shell && cargo clean
	@cd agent-forge && cargo clean
	@rm -rf $(DIST_DIR)
...
Build: A developer runs make inside the KNIRVCORTEX directory. This produces dist/cognitive_shell.wasm (which is deployed with your host application) and dist/forged_agent_example.wasm (which is the distributable agent).

Upload: In the KNIRVCONTROLLER UI, a user navigates to the AgentManager. The "Upload WASM Agent" button, powered by the handleAgentUpload function in AgentManager.tsx, sends the forged_agent_example.wasm file to your backend.
...
It loads both cognitive_shell.wasm and the user-uploaded forged_agent.wasm from storage.
It instantiates the forged_agent.wasm first.
It creates a Linker and finds the exported init, infer, alloc, and dealloc functions from the forged agent instance.
