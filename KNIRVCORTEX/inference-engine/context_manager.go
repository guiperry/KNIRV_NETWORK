...
func (cm *ContextManager) splitByTokenCount(text string) []string {
	...
	for _, paragraph := range paragraphs {
		...
		if paragraphTokens > cm.maxChunkSize {
			...
			// First try using findSplitIndex for optimal splitting
			splitAt := findSplitIndex(trimmed, cm.maxChunkSize, cm.modelName)
			if splitAt > 0 && splitAt < len(trimmed) {
				firstPart := trimmed[:splitAt]
				secondPart := trimmed[splitAt:]
				chunks = append(chunks, cm.splitByTokenCount(firstPart)...)
				chunks = append(chunks, cm.splitByTokenCount(secondPart)...)
				continue
			}

			// Fall back to sentence splitting if findSplitIndex didn't find a good split
...
func (cm *ContextManager) processSequentially(_ context.Context, llm TextGenerator, chunks []string, instructionPerChunk string) (string, error) {
	...
	for chunkIndex, chunk := range chunks {
		log.Printf("ContextManager: Processing chunk %d/%d sequentially...", chunkIndex+1, len(chunks))
		...
		log.Printf("ContextManager: Sequential Prompt for Chunk %d:\n%s\n", chunkIndex+1, chunkPrompt)

		result, err := llm.GenerateText(chunkPrompt)
		if err != nil {
			// If an error occurs, return the results obtained so far and the error
			log.Printf("ContextManager: Error on chunk %d: %v", chunkIndex+1, err)
			results = append(results, fmt.Sprintf("[ERROR PROCESSING CHUNK %d]", chunkIndex+1))
			return strings.Join(results, "\n\n---\n\n"),
				fmt.Errorf("error processing chunk %d: %w", chunkIndex+1, err)
		}

		results = append(results, result)
		log.Printf("ContextManager: Chunk %d processed.", chunkIndex+1)
		...
		if adapter, ok := llm.(*LLMAdapter); ok { // Check if it's our adapter
			...
			if adapter.ProviderName != "" { // Check if provider name is available
				log.Printf("ContextManager: Adding 10s delay after chunk %d (Provider: %s)...", chunkIndex+1, adapter.ProviderName)
...
func findSplitIndex(text string, tokenBudget int, modelName string) int {
	...
	for i := 0; i < len(text); i++ {
		...
		if currentTokens > tokenBudget {
			...
			// Clamp the index to valid range [0, len(text)-1]
			return max(0, min(i-1, len(text)-1))
		}
...
func min(a, b int) int {
	...
	return b  // b is equal or smaller
}
