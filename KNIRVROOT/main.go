...
import (
	...
	"KNIRVROOT/dataengine"
	"KNIRVROOT/enum/StatusType"
	"KNIRVROOT/errors"
	"KNIRVROOT/inference"
	"KNIRVROOT/inference/agentify"

	"github.com/joho/godotenv"
	provider "github.com/mouuff/go-rocket-update/pkg/provider"
	updater "github.com/mouuff/go-rocket-update/pkg/updater"

	"KNIRVROOT/config" // Use your actual module path
...
// convertRelayConfig converts config.RelayConfig to main.RelayConfig
func convertRelayConfig(cfg config.RelayConfig) RelayConfig {
	return RelayConfig{
		Enabled:            cfg.Enabled,
...
func main() {
	...
	var logFilePath = "KNIRVROOT.log" // You can make this configurable
	...
	bootnode := flag.Bool("bootnode", false, "Run as a bootnode for the KNIRVROOT network")
	...
	if *logLevelFlag != "" {
		...
		// TODO: Implement actual log level configuration
	}
	...
	if nodeRole != config.Root {
		...
		if cfg.MinersAddress == utils.BLOCKCHAIN_ADDRESS || cfg.MinersAddress == "KNIRVROOT_Faucet" {
	...
	log.Println("Exiting main KNIRVROOT function.")
}
...
func startNodeWithComponents(
	...
	go func() {
		...
		// 7.1. Node.js services (for root and bootnode roles)
		var nodejsManager *NodeJSManager // Declare nodejsManager
		if (cfg.IsRoot || cfg.IsBootnode) && cfg.NodeJSServices.Enabled {
			var errNodeJS error
			nodejsManager, errNodeJS = initNodeJSServices(&cfg, discoveryMgr) // Use the passed discoveryMgr
			if errNodeJS != nil {
				log.Printf("[%s][%s] ERROR: Failed to initialize Node.js services: %v", cfg.ChainID, nodeRole.String(), errNodeJS)
		...
		go func() {
			...
			// --- Update the altgui backend.config file with the actual HTTP port ---
			altguiEnvPath := filepath.Join(".", "altgui", "backend.config") // Use filepath.Join for cross-platform compatibility
			...
			if updateErr := utils.UpdateEnvVariable(altguiEnvPath, "NEXT_PUBLIC_BACKEND_URL", backendURL); updateErr != nil {
				log.Printf("[%s][%s] WARNING: Failed to update GUI backend URL for %s: %v", nodeRole.String(), cfg.ChainID, altguiEnvPath, updateErr)
				// This is a non-fatal error, node can continue
			} else {
				log.Printf("[%s][%s] Successfully updated GUI backend URL for %s", nodeRole.String(), cfg.ChainID, altguiEnvPath)
			}
			close(serverStopped)
		}()
...
func startNode(ctx context.Context, wg *sync.WaitGroup, cfg config.Config, role config.Role, disableWallet bool, isNetworkMode bool) {
	...
	go func() {
		...
		// 7.1. Node.js services (for root and bootnode roles)
		var nodejsManager *NodeJSManager
		if (cfg.IsRoot || cfg.IsBootnode) && cfg.NodeJSServices.Enabled {
			nodejsManager, err = initNodeJSServices(&cfg, discoveryMgr)
			if err != nil {
				log.Printf("[%s][%s] ERROR: Failed to initialize Node.js services: %v", role.String(), cfg.ChainID, err)
				// Continue execution even if Node.js services fail to start
			}
			if nodejsManager != nil {
				defer func() {
					log.Printf("[%s][%s] Stopping Node.js services...", role.String(), cfg.ChainID)
					nodejsManager.StopAllServices()
					log.Printf("[%s][%s] Node.js services stopped", role.String(), cfg.ChainID)
				}()
			}
		}
...
func integrateInferenceEngineWithDHT(discoveryMgr *DiscoveryManager, cfg *config.Config) error {
	...
	metricsKey := fmt.Sprintf("/KNIRVROOT/inference-metrics/%s", nodeID)
