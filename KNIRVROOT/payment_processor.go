...
// PaymentProcessor handles the payment processing functionality
type PaymentProcessor struct {
	config             config.PaymentProcessorConfig
	disbursementWallet *Wallet // The loaded wallet for sending tokens
...
func NewPaymentProcessor(cfg config.PaymentProcessorConfig, masterWallet *Wallet) (*PaymentProcessor, error) {
...
func (pp *PaymentProcessor) disburseTokens(recipientAddress string, amount *big.Int, paymentID string) (string, error) {
	...
	log.Printf("Constructed transaction: From %s To %s Amount %d", tx.From, tx.To, tx.Value)
...
// broadcastTransaction sends the signed transaction to the KNIRVROOT node.
func (pp *PaymentProcessor) broadcastTransaction(tx *Transaction) (string, error) {
...
func (pp *PaymentProcessor) handleStripeWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if eventType == "charge.succeeded" { // Adjust based on your Stripe integration
		...
		// 3. *** IMPORTANT: Get the recipient's KNIRVROOT address ***
		...
		if okMeta {
			addr, ok := metadata["KNIRVROOT_address"].(string)
			if ok && addr != "" {
		...
		if !okAddr {
			log.Printf("ERROR: KNIRVROOT_address missing in metadata for charge %s", chargeID)
...
// isValidKNIRVROOTAddress checks if an address is a valid KNIRVROOT address
func isValidKNIRVROOTAddress(addr string) bool {
...
func LoadPaymentProcessorConfig(cfg *config.PaymentProcessorConfig) {
	...
	if nodeRPC := os.Getenv("KNIRVROOT_NODE_RPC"); nodeRPC != "" {
		cfg.NodeRPC = nodeRPC
	}
