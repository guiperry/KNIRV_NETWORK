...
*   **MCP:** A set of standards for defining, discovering, and interacting with AI capabilities. KNIRVROOT serves as a decentralized backend for MCP.
*   **Capabilities:** These are the core building blocks of the MCP ecosystem on KNIRVROOT:
...
The `ContextRecord` is central to KNIRVROOT's MCP, providing an on-chain, verifiable log for:
...
cd /path/to/KNIRVROOT_GO_ROOT

# Build the executable
go build -o KNIRVROOT_node .
```
...
2.  Path specified by the `KNIRVROOT_CONFIG_PATH` environment variable.
3.  User config directory (`~/.config/KNIRVROOT/config.json` on Linux).
...
1.  Run the `KNIRVROOT_node` on your chosen bootstrap machines. Note their full multiaddresses (e.g., `/ip4/x.x.x.x/tcp/4001/p2p/12D3Koo...`).
...
3.  **Single Node Mode:** Starts a single node, typically using root configuration unless overridden. Useful for development or specific setups.
    ...
    ./KNIRVROOT_node -port 5005 -p2p.port 4005 -wallet_port 6005 -shared_database_path database_node5005/agent.db -miners_address your_miner_address
    ```
...
KNIRVROOT uses a custom URI scheme for resource identification within its private network:
...
Resolution involves parsing the URI, deriving a resource key (e.g., `myPeer.chain`), generating a CID, querying the private KNIRVROOT DHT for providers using the CID, connecting to a provider via libp2p, and making the request over a P2P stream.
