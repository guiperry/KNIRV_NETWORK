...
*   **Plugin System (Preferred):** Use Go's plugin system (`plugin` package) to dynamically load and execute the code templates. This allows you to load the templates at runtime without recompiling the Inference Engine.
    ...
    *   **Execution:** The Inference Engine calls on the KNIRVROOT to process NRN transactions within the capability (e.g., `ValidateAsset`, `ImportAsset`) when parsing functions.
...
7.  **Transaction Processed (or Rejected):** If the asset is valid, the Inference Engine sends the transaction to the KNIRVROOT for an NRN URI minted. If the asset is invalid, the blockchain core rejects the transaction.

### agent Client (Inference Engine):

An application that enables llm inference within custom capability functionalities.

*   **Discovers capabilities:** Queries the KNIRVROOT API.
...
*   **Logs capability usage:** Submits an `MCPInvokeCapabilityTransaction` to the KNIRVROOT, including a `ContextRecord` and the NRN fee.
*   **Capability Hosting Client:** Makes the capability binary/code available for download at the `LocationHints` specified in the `ResourceDescriptor` via the KNIRVROOT DHT. Receives NRN token fees when clients log usage of their registered capabilities.
...
2.  **Registration (On-Chain):**
    ...
    *   `Owner`: The developer's KNIRVROOT address.
    ...
    *   `LocationHints`: Array of URIs (e.g., `agent://<dev_id>.chain/capabilities/<capability_id>`, IPFS CID) where the capability binary can be downloaded.
    ...
    This transaction is submitted to the KNIRVROOT network.
3.  **Discovery (By Client):**

    The agent Client queries the KNIRVROOT API (e.g., `GET /mcp/capabilities?resourceType=TOOL&name=AssetValidator`) to find suitable capabilities.
...
6.  **Usage Logging & Fee Payment (By Client):**
    ...
    *   `Initiator`: The client's KNIRVROOT address.
    ...
    *   `Details`: Other relevant metadata (e.g., capability version used, execution duration).
    ...
    The client signs and submits this transaction to the KNIRVROOT network. The fee is ultimately credited to the capability's `Owner`.
