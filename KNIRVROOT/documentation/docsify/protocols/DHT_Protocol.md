# KNIRVROOT Decentralized Discovery Implementation Plan (DHT)
...
*   **DHT Integration:** Each participating KNIRVROOT node will run a Kademlia DHT client/server instance.
...
*   **Chain Identifier:** For simplicity, we'll assume the `chainID` serves as both the primary identifier for a KNIRVROOT instance and the key under which it registers its multiaddress(es) in the DHT.
...
The URI scheme for KNIRVROOT will be updated to reflect the decentralized nature of the network and provide a clear structure resembling familiar domain patterns. The general format will be:
...
*   **`agent://`**: Protocol prefix indicating a KNIRVROOT-specific URI.
...
1.  Ensure each KNIRVROOT node generates or loads a persistent cryptographic key pair (e.g., Ed25519) on startup.
