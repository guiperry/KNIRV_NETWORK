...
The following mutexes are identified within the KNIRVROOT system. They are listed in the order they should be acquired (i.e., you can acquire `M1` then `M2`, but not `M2` then `M1`).
...
*   **Transaction Processing (`BlockchainStruct.AddTransactionToTransactionPool`):**
    ...
    3.  Performs `transaction.VerifyTxn()` (which involves crypto, but no KNIRVROOT mutexes).
...
By strictly following this Mutex Hierarchy Protocol, KNIRVROOT aims to minimize the risk of deadlocks and ensure robust concurrent operation. Developers must be diligent in understanding and applying these rules.
