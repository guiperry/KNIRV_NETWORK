# KNIRVROOT Build System

This document describes the build system for KNIRVROOT, including role-specific builds and cross-compilation.

## Node Roles

KNIRVROOT supports multiple node roles, each with specific functionality:
...
KNIRVROOT now supports role-specific builds using Go build tags. This allows for creating binaries that are optimized for specific roles without requiring runtime configuration.

### Building for Specific Roles

You can build KNIRVROOT for specific roles using the Makefile targets:
...
You can cross-compile KNIRVROOT for multiple platforms and roles:
...
go build -tags=client -o KNIRVROOT

# Build for root role
go build -tags=root -o KNIRVROOT_root

# Build for bootnode role
go build -tags=bootnode -o KNIRVROOT_bootnode

# Build for developer role
go build -tags=developer -o KNIRVROOT_developer
```

## User Interfaces

KNIRVROOT supports multiple user interfaces:
...
KNIRVROOT supports the following command-line flags:
