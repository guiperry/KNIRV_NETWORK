# KNIRVROOT Node Registry & STUN Service Documentation

## Overview

This service provides two key functions for the KNIRVROOT network: a **STUN Server** for public IP discovery and an **HTTP Registry API** for bootnode registration and lookup, designed to work with a Wildcard DNS setup.

Prospective network bootnodes first use the STUN service to discover their public IP address. Then, they register this IP and their listening port with the HTTP Registry API. When new dev nodes need to connect to the KNIRVROOT Network, instead of connecting through a centralized relay or bootstrap listing, it queries the Bootnode Registry API for alist of all active bootnodes. Each node then attempts to connect directly to each listed bootnode to retrieve the correct IP address and port, enabling a direct dev-to-dev connection to execute the bootstrapping process.

---

## STUN Service (IP Discovery)

*   **Purpose:** Allows a KNIRVROOT node to discover its public IP address and port as seen by the internet, which is crucial for nodes operating behind NAT (Network Address Translation). Nodes should query this service *before* registering via the HTTP API.
...
*   **Purpose:** Allows a KNIRVROOT bootnode to register or update its network location (IP address and port) with the registry. Bootnodes should call this endpoint upon startup (after STUN discovery) and potentially periodically to signal they are still active.
*   **Method:** `POST`
*   **Path:** `/register`
*   **Request Body (JSON):**
    ```json
...
*   **Purpose:** Allows a KNIRVROOT node to query the central registry for the current IP address and port of a specific boot node, identified by its `chainID`.
*   **Method:** `GET`
*   **Path:** `/lookup/:chainID`
