# KNIRVROOT Payment Processor

The KNIRVROOT Payment Processor is a component that enables root nodes to accept payments in fiat currencies (USD, EUR, etc.) and cryptocurrencies (ETH, BTC, etc.) and automatically disburse KNIRVROOT tokens to customers.

## Overview

The payment processor integrates with popular payment gateways like Stripe and Coinbase Commerce to process payments and automatically disburse tokens to customers' KNIRVROOT wallets. This enables roots to monetize their content and services using KNIRVROOT tokens without having to handle payment processing themselves.
...
To enable the payment processor, start your KNIRVROOT node with the `--root` flag:
...
1. Configure your payment gateways (Stripe, Coinbase Commerce) to send webhook events to your KNIRVROOT node
2. Include the customer's KNIRVROOT wallet address in the payment metadata
...
This component is part of the KNIRVROOT project and is subject to the same license terms.