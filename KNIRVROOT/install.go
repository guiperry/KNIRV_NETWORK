...
func init() {
	...
	walletManager, err = NewWalletManager(encryptionKey)
	if err != nil {
		panic(fmt.Sprintf("failed to initialize wallet manager: %v", err))
	}
}

// URIResponse represents the response from the KNIRVROOT Root's /uriGenerator endpoint
...
func Install(configPath string, IsBootnode bool, role config.Role, nonInteractive bool, walletPath string) (*config.Config, error) {
	...
	if role == config.Root {
		...
		if role != config.Root {
			fmt.Println("1. Connect to the KNIRVROOT Bootnode")
			fmt.Println("2. Generate a unique chain URI for this node")
		}

		if role == config.RoleBootnode {
			fmt.Println("3. Register with KNIRVROOT Bootnode Registry")
		}
	...
	if role != config.Root && role != config.RoleClient { // Skip URI generation for Root AND Client roles
		// Step 1: Connect to KNIRVROOT Root
		...
		chainURI = fmt.Sprintf("agent://KNIRVROOT-BOOT:%d.chain/", currentCfg.Port) // Port might be default here
	...
	case config.Root:
		...
		if chainID == "" {           // Fallback if not set in currentCfg
			chainID = fmt.Sprintf("KNIRVROOT-ROOT:%d", currentCfg.Port) // Default for root
		}
	...
	var devWallet, masterWallet *Wallet
	...
	// Step 4: Register with KNIRVROOT Node Registry (only for Bootnode role)
	if role == config.RoleBootnode {
		...
		nodeID := dm.host.ID().String()

		// Register with node registry using public IP and P2P port
		fmt.Println("Registering bootnode with KNIRVROOT Node Registry...")
		fmt.Printf("Registering node %s at %s:%s (PeerID: %s) with KNIRVROOT Node Registry\n",
		...
		if err != nil {
			...
			fmt.Println("Successfully registered with KNIRVROOT Node Registry")
		}
	...
	if role != config.Root {
		...
		_ = fmt.Sprintf("KNIRVROOT-%s", strings.ToLower(role.String())) // Used in log messages
	...
	case config.Root:
		...
		configToSave.ChainID = chainID // This is KNIRVROOT-ROOT<port> or similar
	...
	switch role {
	case config.Root:
		fmt.Println("Your KNIRVROOT ROOT Node is now configured.")
		fmt.Println("This node will be able to create and manage chains.")
	case config.RoleBootnode:
		fmt.Println("Your KNIRVROOT Bootnode is now configured with a unique chain URI.")
		fmt.Println("This node will help with dev discovery and network coordination.")
	case config.RolePeer:
		fmt.Println("Your KNIRVROOT Peer Node is now configured with a unique chain URI.")
		fmt.Println("This node will participate in the network and process transactions.")
	case config.RoleClient:
		fmt.Println("Your KNIRVROOT Client Node is now configured.")
		fmt.Println("This node will connect to the network with reduced resource usage.")
	}

	fmt.Println("Finalizing installation and launching KNIRVROOT Node...")
...
// promptForRootEndpoint prompts the user for the KNIRVROOT Root endpoint
...
func promptForRootEndpoint(nonInteractive bool, currentCfg *config.Config) string {
	...
	fmt.Printf("Enter the KNIRVROOT Root endpoint [default: %s]: ", defaultEndpoint)
	var endpoint string
	fmt.Scanln(&endpoint)
...
// GenerateChainURI connects to the KNIRVROOT Root and generates a unique chain URI
...
func GenerateChainURI(rootEndpoint string, desiredURI string) (string, string, string, error) {
	...
	req.Header.Set("User-Agent", "KNIRVROOT-Node-Installer/1.0")

	// Send the request
	resp, err := client.Do(req)
	if err != nil {
		log.Printf("HTTP request failed: %v", err)
		return "", "", "", fmt.Errorf("failed to connect to KNIRVROOT Root: %w", err)
	}
...
func RegisterURIHandlers(chainURI string) error {
	...
	schemes := []uri.URIScheme{
		{
			Name:        "agent",
			Description: "KNIRVROOT Decentralized MCP Network",
		},
	}
...
// RegisterWithNodeRegistry registers this node with the KNIRVROOT Node Registry
...
func configureLinuxService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	serviceUser := "KNIRVROOT"
	workingDir := "/var/lib/KNIRVROOT"
	logPath := "/var/log/KNIRVROOT.log"

	// Determine service name and command line based on service type
	serviceName := fmt.Sprintf("KNIRVROOT-%s", strings.ToLower(role.String()))
	...
	if err != nil {
		executablePath = "/usr/local/bin/KNIRVROOT" // Fallback if we can't determine the path
	}

	if role == config.RoleBootnode {
		description = fmt.Sprintf("KNIRVROOT Bootnode Service for chain %s", cfg.ChainID)
		...
		description = fmt.Sprintf("KNIRVROOT Peer Node Service for chain %s", cfg.ChainID)
...
`, description, serviceUser, execStart, workingDir, logPath, logPath)
	...
	if os.Geteuid() != 0 {
		...
		if err != nil {
			...
			dataDir = filepath.Join(homeDir, ".config", "KNIRVROOT")
		}
...
func configureWindowsService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	displayName := "KNIRVROOT Node"
	description := "KNIRVROOT Node Service"
	logPath := "C:\\ProgramData\\KNIRVROOT\\logs\\KNIRVROOT.log"
	...
	executablePath, err := os.Executable()
	if err != nil {
		executablePath = os.Getenv("ProgramFiles") + "\\KNIRVROOT\\KNIRVROOT.exe" // Fallback
	}

	var binPath string

	if role == config.RoleBootnode {
		displayName = "KNIRVROOT Bootnode"
		description = fmt.Sprintf("KNIRVROOT Bootnode Service for chain %s", cfg.ChainID)
	...
	if err != nil || !isAdmin {
		...
		if err != nil {
			...
			dataDir = filepath.Join(appDataDir, "AppData", "Local", "KNIRVROOT")
		}
...
echo Installing KNIRVROOT %s Service
...
func checkWindowsAdminPrivileges() (bool, error) {
	...
	testFile := "C:\\Windows\\Temp\\KNIRVROOT_admin_test.tmp"
...
func configureMacOSService(configPath string, cfg *config.Config, role config.Role) error {
	// Set default values
	workingDir := "/Library/KNIRVROOT"
	logPath := "/Library/Logs/KNIRVROOT/KNIRVROOT.log"

	// Determine service name and command line based on service type
	serviceName := fmt.Sprintf("com.KNIRVROOT.%s", strings.ToLower(role.String()))

	// Get the executable path
	executablePath, err := os.Executable()
	if err != nil {
		executablePath = "/usr/local/bin/KNIRVROOT" // Fallback
	}
...
<dict>
	...
	if os.Geteuid() != 0 {
		...
		if err != nil {
			...
			dataDir = filepath.Join(homeDir, "Library", "Application Support", "KNIRVROOT")
		}
