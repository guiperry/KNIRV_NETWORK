...
func NewConfigManager(configPath string) (*ConfigManager, error) {
	...
	if configPath == "" {
		...
		configPath = filepath.Join(homeDir, ".KNIRVROOT", "config.yaml")
	}
...
func setDefaultConfig(v *viper.Viper) {
	// Node settings
	v.SetDefault("node.name", "KNIRVROOT Node")
	v.SetDefault("node.role", string(config.RoleClient))
	v.SetDefault("node.chain_id", "KNIRVROOT-testnet")
	...
	v.SetDefault("logging.file", "KNIRVROOT.log")
}
...
func (cm *ConfigManager) SetConfig(cfg *config.Config) {
	cm.cfg = cfg
}
