...
func Uninstall() {
	...
	fmt.Printf("=== KNIRVROOT %s Node Uninstallation ===\n", role.String())
	...
	fmt.Printf("KNIRVROOT %s Node has been uninstalled.\n", role.String())
}
...
func UnregisterURIHandlers() error {
	...
	schemes := []uri.URIScheme{
		{
			Name:        "agent",
			Description: "KNIRVROOT Decentralized Protocol",
		},
	}
...
func getServiceName(role config.Role) string {
	return fmt.Sprintf("KNIRVROOT-%s", strings.ToLower(role.String()))
}
