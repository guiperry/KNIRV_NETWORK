...
func RegisterWithRegistry(registryURL string, nodeChainID string, nodeP2PPort int, nodeIP string, nodePeerID string) error {
	...
	payload := RegistrationPayload{
		ChainID: nodeChainID, // This is the logical ChainID (e.g., KNIRVROOT-5000)
		IP:      nodeIP,
	...
	req.Header.Set("User-Agent", "KNIRVROOT-Node/1.0")

	// Send the request
	log.Printf("Registering node %s (PeerID: %s) at %s:%d with KNIRVROOT Node Registry", nodeChainID, nodePeerID, nodeIP, nodeP2PPort)
...
// err := registerWithRegistry("http://registry.knirv.com:3003", "KNIRVROOT-[chainID]]", Port, "detectedPublicIP")
// if err != nil {
//     log.Printf("[Bootnode][%s] WARNING: Failed to register with registry: %v", "KNIRVROOT-5000", err)
// }
