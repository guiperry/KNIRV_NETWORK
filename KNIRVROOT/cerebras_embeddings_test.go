...
func TestCerebrasEmbeddingGeneration(t *testing.T) {
	...
	apiKey := "test-api-key"
	text := "This is a test text for embedding generation in KNIRVROOT"
	...
	t.Logf("Successfully generated consistent 384-dimensional embedding for KNIRVROOT")
}
...
func TestCerebrasEmbeddingFunction(t *testing.T) {
	ctx := context.Background()
	apiKey := "test-api-key"
	text := "KNIRVROOT blockchain transaction embedding test"
	...
	t.Logf("Successfully generated normalized embedding function for KNIRVROOT")
}
...
func TestEmbedDocuments(t *testing.T) {
	...
	t.Logf("Successfully generated embeddings for %d documents in KNIRVROOT", len(texts))
}
