...
func getGraphChainBinaryPath() string {
	// Get the directory where the KNIRVSHELL binary is located
	...
	// Fallback: check in the same directory as KNIRVSHELL
...
func showGraphChainStatus() {
	...
	if info, err := os.Stat(binaryPath); err != nil {
		...
		fmt.Println("2. Or build manually: cd KNIRVGRAPH && go build -o ../KNIRVSHELL/bin/graphchain-cli ./cmd/cli/main.go")
		return
	} else {
