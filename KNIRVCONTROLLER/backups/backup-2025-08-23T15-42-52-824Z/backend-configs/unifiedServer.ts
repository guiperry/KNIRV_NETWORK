    ...
    this.app.get('*', (req, res, next) => {
      ...
      res.sendFile(indexPath, (err) => {
        if (err) {
          ...
          res.status(404).json({
            success: false,
            error: 'Frontend not built. Run "npm run build:receiver" first.'
          });
        }
      });
    });
  ...
  public async start() {
    try {
      ...
      this.server.listen(this.port, () => {
        ...
        logger.info('📱 Receiver frontend available at: http://localhost:' + this.port);
        ...
        logger.info('  GET  / - Receiver Frontend');
