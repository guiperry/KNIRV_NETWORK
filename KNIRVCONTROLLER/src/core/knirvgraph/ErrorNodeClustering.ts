...
export class ErrorNodeClustering {
  ...
  private async kMeansClustering(errorNodes: ErrorNode[], featureVectors: ErrorFeatureVector[]): Promise<ErrorCluster[]> {
    ...
    while (iterations < maxIterations) {
      ...
      const assignments = errorNodes.map((node, index) => {
      ...
      clusters = [];
      for (let i = 0; i < k; i++) {
        const clusterNodes = errorNodes.filter((_, index) => assignments[index].clusterIndex === i);
        const clusterVectors = featureVectors.filter((_, index) => assignments[index].clusterIndex === i);
