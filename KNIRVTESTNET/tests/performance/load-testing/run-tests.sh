...
if [ ! -f "go.mod" ]; then
    ...
    go mod init load-testing
    go mod tidy
fi

# Run the Go tests
echo "🧪 Executing performance load test suite..."
go test -v -timeout=15m ./... 2>&1 | tee test_results.log

# Check test results
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ Performance load tests completed successfully!"
...
echo "🎉 All performance load tests passed!"
