...
if [ ! -f "go.mod" ]; then
    ...
    go mod init cross-service-integration
    go mod tidy
fi

# Run the Go tests
echo "🧪 Executing cross-service integration test suite..."
go test -v -timeout=10m ./... 2>&1 | tee test_results.log

# Check test results
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo "✅ Cross-service integration tests completed successfully!"
    ...
    echo "❌ Cross-service integration tests failed!"
