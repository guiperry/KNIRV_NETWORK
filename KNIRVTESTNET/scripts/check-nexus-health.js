...
function checkUnifiedBinary() {
  const binaryPath = path.join(__dirname, '..', 'bin', 'knirvnexus');
  ...
  if (!fs.existsSync(sourcePath)) {
    console.log('⚠️  KNIRVNEXUS source directory not found - cannot verify freshness');
...
async function testNexusService() {
  ...
  const pidFile = path.join(__dirname, '..', 'data', 'knirvnexus.pid');
...
async function checkNexusHealth() {
  ...
  const configFiles = [
    ...
    { path: 'data/knirvnexus/config.yaml', name: 'Legacy Configuration' }
  ];
  ...
  const dataDir = path.join(__dirname, '..', 'data', 'knirvnexus');
  ...
  // Check KNIRVNEXUS source availability
  const sourcePath = path.join(__dirname, '..', '..', 'KNIRVNEXUS');
  if (fs.existsSync(sourcePath)) {
    console.log('✅ KNIRVNEXUS source directory available');

    // Check if source has package.json
    const sourcePackageJson = path.join(sourcePath, 'package.json');
    if (fs.existsSync(sourcePackageJson)) {
      console.log('✅ KNIRVNEXUS source has package.json');
    } else {
      warnings.push('KNIRVNEXUS source missing package.json');
    }

    // Check if source has main.go
    const sourceMainGo = path.join(sourcePath, 'main.go');
    if (fs.existsSync(sourceMainGo)) {
      console.log('✅ KNIRVNEXUS source has main.go');
    } else {
      warnings.push('KNIRVNEXUS source missing main.go');
    }
  } else {
    issues.push('KNIRVNEXUS source directory not found - cannot build');
  }
  ...
  console.log('   Start service:    ./scripts/start-knirvnexus.sh');
  console.log('   Check service:    curl http://localhost:8084/health');
  console.log('   View logs:        tail -f logs/knirvnexus.log');
