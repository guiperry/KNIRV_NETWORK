...
# KNIRV TESTNET - Unified KNIRVCONTROLLER Starter
# Automatically detects and starts either real or demo KNIRVCONTROLLER
...
CONTROLLER_PID_FILE="$TESTNET_ROOT/data/knirvcontroller.pid"
CONTROLLER_LOG_FILE="$TESTNET_ROOT/logs/knirvcontroller.log"
...
# Check if real KNIRVCONTROLLER is available
...
# Start real KNIRVCONTROLLER
start_real_controller() {
    print_info "🚀 Starting Real KNIRVCONTROLLER..."
    
    local controller_dir="$PROJECT_ROOT/KNIRVCONTROLLER"
    local config_script="$TESTNET_ROOT/config/knirvcontroller/start-real-controller.sh"
    
    if [ ! -f "$config_script" ]; then
        print_error "Real KNIRVCONTROLLER not built. Run: ./scripts/build-knirvcontroller.sh"
        return 1
    fi
    ...
    if curl -s --max-time 10 "http://localhost:$REAL_CONTROLLER_PORT" > /dev/null 2>&1; then
        print_success "Real KNIRVCONTROLLER started successfully!"
        ...
        print_warning "Real KNIRVCONTROLLER failed to start, falling back to demo..."
...
start_demo_controller() {
    print_info "🎮 Starting Demo KNIRVCONTROLLER..."
    
    # Use the demo controller script
    if "$SCRIPT_DIR/start-demo-knirvcontroller.sh" start; then
...
auto_start_controller() {
    print_info "🔍 Auto-detecting KNIRVCONTROLLER configuration..."
    
    # Check if any controller is already running
    if curl -s --max-time 2 "http://localhost:$REAL_CONTROLLER_PORT" > /dev/null 2>&1; then
        print_success "Real KNIRVCONTROLLER already running on port $REAL_CONTROLLER_PORT"
        return 0
    fi
    
    if curl -s --max-time 2 "http://localhost:$DEMO_CONTROLLER_PORT" > /dev/null 2>&1; then
        print_success "Demo KNIRVCONTROLLER already running on port $DEMO_CONTROLLER_PORT"
        return 0
    fi
    
    # Try real controller first if available
    if check_real_controller_available; then
        print_choice "Real KNIRVCONTROLLER available - attempting to start..."
    ...
    print_choice "Using Demo KNIRVCONTROLLER for testnet development"
    start_demo_controller
}

# Stop any running controller
stop_controller() {
    print_info "🛑 Stopping KNIRVCONTROLLER services..."
    ...
    if [ -f "$CONTROLLER_PID_FILE" ]; then
        ...
        if kill -0 "$pid" 2>/dev/null; then
            kill "$pid"
            print_success "Real KNIRVCONTROLLER stopped (PID: $pid)"
        fi
        rm -f "$CONTROLLER_PID_FILE"
    fi
    
    # Stop demo controller
    "$SCRIPT_DIR/start-demo-knirvcontroller.sh" stop
...
show_status() {
    ...
    # Show availability
    if check_real_controller_available; then
        print_info "Real KNIRVCONTROLLER: Available for deployment"
    else
        print_warning "Real KNIRVCONTROLLER: Not available (not built or missing dependencies)"
    fi
}
...
case "${1:-auto}" in
    ...
    "real")
        if check_real_controller_available; then
            ...
            print_error "Real KNIRVCONTROLLER not available. Run: ./scripts/build-knirvcontroller.sh"
            exit 1
        fi
        ;;
    "demo")
    ...
    "status")
        ...
        ;;
    "help"|"--help"|"-h")
        echo "KNIRV TESTNET - KNIRVCONTROLLER Management"
        echo ""
        echo "Usage: $0 [COMMAND]"
        echo ""
        echo "Commands:"
        echo "  auto    Auto-detect and start appropriate controller (default)"
        echo "  real    Start real KNIRVCONTROLLER (requires build)"
        echo "  demo    Start demo KNIRVCONTROLLER"
        echo "  stop    Stop all KNIRVCONTROLLER services"
        echo "  status  Show controller status"
        echo "  help    Show this help message"
        echo ""
        echo "Examples:"
        echo "  $0                    # Auto-start (real if available, demo otherwise)"
        echo "  $0 real              # Force start real KNIRVCONTROLLER"
        echo "  $0 demo              # Force start demo KNIRVCONTROLLER"
        ...
        echo ""
        echo "Setup:"
        echo "  1. Build real controller: ./scripts/build-knirvcontroller.sh"
