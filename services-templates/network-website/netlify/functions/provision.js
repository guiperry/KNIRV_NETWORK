 ...
 * In serverless mode, this proxies requests to the persistent Render oracle.
 */
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Fetch from persistent Render oracle
    console.log('Fetching peers from persistent oracle:', RENDER_GATEWAY_INTERNAL_API);
    
    const response = await axios.get(RENDER_GATEWAY_INTERNAL_API, {
      headers: {
        'Authorization': `Bearer ${INTERNAL_API_KEY}`,
        'User-Agent': 'KNIRVORACLE-Netlify/1.0.0'
      },
      timeout: 5000
    });
    ...
    if (!Array.isArray(dhtPeers)) {
      throw new Error('Invalid response format from persistent oracle');
    }

    // Cache the result
    cache.set('dht_peers', dhtPeers);

    console.log(`Successfully fetched ${dhtPeers.length} peers from persistent oracle`);
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
      statusCode = 503;
      errorMessage = 'Persistent oracle unavailable';
