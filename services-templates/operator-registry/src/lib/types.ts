...
export interface OperatorSignature {
  ...
  verificationMethod: string; // DID URL of the public key (e.g., operatorDID#key-1)
...
// Represents the detailed information about an operator
export interface OperatorFacts {
  id: string; // Operator's own DID, primary key (NANDA Identifier)
  ansName?: string; // Full ANS name, e.g., "a2a://myTranslator.DocumentTranslation.LinguaCorp.v1.2.certified" (ANS Capability Address)
  name: string; // User-friendly display name for the operator
  ...
  signature?: OperatorSignature; // Signature by operator or publisher
  avatarUrl?: string; // Optional URL for operator's avatar image
  dataAiHint?: string; // Keywords for AI-assisted placeholder image generation
  registeredAt?: string; // ISO date string
}

// Represents the pointer to OperatorFacts stored in the NANDA+ANS registry
export interface OperatorAddr {
  operator_id: string; // DID, should match OperatorFacts.id
  facts_url: string; // NANDA pointer to the OperatorFacts URL
  private_facts_url?: string;
  adaptive_router_url?: string;
  ttl: number; // Time-to-live for this NANDA record
  signature: OperatorSignature; // Signature from the registry shard
}

// Combined Operator type primarily used for UI display and interactions.
// It merges essential fields from OperatorFacts and OperatorAddr.
export interface Operator extends OperatorFacts {
  // OperatorFacts fields are inherited.
  // We can add specific fields from OperatorAddr if needed directly.
  addr_ttl?: number; // TTL from NANDA OperatorAddr
  addr_facts_url?: string; // facts_url from NANDA OperatorAddr, distinguishing it if OperatorFacts also had a facts_url.
