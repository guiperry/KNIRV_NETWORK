...
// Import Economics Engine
const { initializeEconomicsSystem } = require('./lib/economics');
const AdminAPI = require('./lib/economics/AdminAPI');
...
// Economics Engine initialization (will be started in the async init function)
let economicsSystem = null;
...
app.get('/health', (req, res) => {
  const statusData = {
    ...
    features: {
      ...
      economics: economicsSystem !== null,
      economicsRunning: economicsSystem ? economicsSystem.tokenEconomics.isRunning : false
    }
  };
...
async function startServer() {
  try {
    ...
    console.log('Initializing KNIRV Economics Engine...');
    economicsSystem = await initializeEconomicsSystem({
      nrnContract: process.env.NRN_CONTRACT || '',
      xionRPC: process.env.XION_RPC || 'https://rpc.xion-testnet-1.burnt.com:443',
      knirvchainURL: process.env.KNIRVCHAIN_URL || 'http://localhost:8080',
      knirvnexusURL: process.env.KNIRVNEXUS_URL || 'http://localhost:8081',
      knirvoracleURL: process.env.KNIRVORACLE_URL || 'http://localhost:8082',
      knirvgraphURL: process.env.KNIRVGRAPH_URL || 'http://localhost:8083'
    });

    // Mount Economics API routes
    app.use('/api/economics', economicsSystem.router);
    console.log('Economics API routes mounted at /api/economics');

    // Mount Admin API routes (for webgui)
    const adminAPI = new AdminAPI(economicsSystem.tokenEconomics, economicsSystem.integration);
    app.use('/api/admin/economics', adminAPI.getRouter());
    console.log('Admin API routes mounted at /api/admin/economics');

    // Start Express server
    const PORT = process.env.PORT || 3000;
    app.listen(PORT, () => {
      console.log('=====================================');
      console.log('KNIRV Payment Gateway & Economics Engine');
      console.log('=====================================');
      console.log(`Server running on port ${PORT}`);
      console.log(`Open http://localhost:${PORT} in your browser`);
      console.log('');
      console.log('Available endpoints:');
      console.log('  - Payment Gateway: /api/faucet/*, /api/create-*');
      console.log('  - Economics API: /api/economics/*');
      console.log('  - Admin API: /api/admin/economics/*');
    ...
    process.on('SIGTERM', async () => {
      ...
      if (economicsSystem) {
        await economicsSystem.tokenEconomics.stop();
        await economicsSystem.integration.stop();
      }
      process.exit(0);
    });
